...
**Progress (session-182):** `/test-suite` shipped as global skill. 3-phase protocol (generate → verify → revise) shaped by contrarian review — Phase 2 (verify) is the skill's core value, structurally separated and non-skippable. Three mandatory self-checks: echo-chamber detection (specification-based assertions, not implementation-mirroring), error-path balance (weighted guidance with domain sensitivity), mutation mindset (single-character mutation coverage). Auto-detects pytest/vitest/jest/playwright/go/rust. Framework-specific idioms inline. No MCP dependency (optional governance enhancement gated on availability).

**Progress (session-182):** `/doc-gen` shipped as global skill. 3-phase protocol (classify+generate → verify → revise). Key differentiator shaped by contrarian: **tool-assisted verification** — grep/read commands as oracle to mechanically confirm every factual claim, not self-attestation. Output distinguishes mechanically-verified claims (grep-confirmed) from self-reviewed claims (semantic, not verifiable). Four checks: claim verification (tool-assisted), example validation (tool-assisted), padding/value (self-review), completeness/omission (non-obvious behavior gaps). Source material: documentation-writer subagent + industry research (Diátaxis framework, Mintlify/Swimm verification patterns, hallucination taxonomy).

**Progress (session-183):** `/refactor-audit` shipped as global skill. 3-phase protocol (map → assess → report). Single mode: pre-refactor readiness assessment ("Is this code ready to be safely refactored?"). Core value shaped by contrarian: **structural contract mapping** (what grep CAN confirm) + **invisible contracts warning** (what grep CANNOT detect — ordering, invariants, state machines — labeled UNVERIFIED). Never outputs bare "SAFE" — strongest verdict is "STRUCTURALLY READY (invisible contracts not verified)." Dynamic dispatch detector for blast radius completeness. Test coverage prerequisite as structural gate (STOP, not advice). Clear skill composition: `/refactor-audit` (ready?) → refactor → `/code-review` (did it work?). Source: CodeScene ACE (37% correctness rate), Fowler's prerequisites, blast radius analysis tools, code-reviewer subagent. Remaining skills (`/pre-commit`, `/architecture-review`) stay in Discussion.
