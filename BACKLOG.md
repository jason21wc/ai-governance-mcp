...
#### 123. Derived Map Freshness Enforcement (Pre-Commit / CI Gate) `D1 Capability`

**Filed:** 2026-04-23 (session-123, post-commit 2nd-pass contrarian HIGH).

**What.** `documents/test-failure-mode-map.md` is a static committed artifact generated by `scripts/generate-test-failure-map.py`. There is no pre-commit hook, CI workflow, or Makefile target that invokes the generator and fails on diff. Freshness depends on whoever remembers to run the script. Plan v3 called the map "rot-immune"; it isn't — it's regen-on-demand with no enforcement.

**Fix (choose one).** (a) Add pre-commit hook that runs `generate-test-failure-map.py` and fails if the output differs from the committed file — same pattern as ruff autofix in the existing pre-commit config. (b) Add CI step (`.github/workflows/`) that regenerates and `git diff --exit-code`s. (c) Delete the committed artifact and generate on demand only (simpler, but loses the diff-in-PR benefit for reviewers).

**Trigger.** Before any broader annotation sweep (BACKLOG #121) to prevent the map from lagging during the sweep.
