...
| Version | Supported          |
| ------- | ------------------ |
| 2.1.x   | :white_check_mark: (server/package) |
| < 2.1   | :x:                |

> **Note:** ARCHITECTURE.md tracks its own document version separately from the package version in pyproject.toml (currently v2.1.0). Security support follows the package version.
...
| **Tool Description Poisoning** | Injection payloads in individual tool descriptions (AML.T0110) | SERVER_INSTRUCTIONS scanned at module load; individual tool descriptions unscanned (partial) |
| **Supply Chain Rug Pull** | Server passes vetting then mutates tool descriptions after update (AML.T0109) | Version-locked descriptions, agent template hash verification (advisory, not blocking) |
| **Reputation Inflation / Namesquatting** | Malicious package using a confusingly similar name (AML.T0111) | Unique name, official repo verification, PyPI/npm registration |
| **Dynamic Response Injection** | Served content contains injection patterns at runtime (AML.T0108) | CI content scanning, extraction-time scanning, NFKC normalization (partial — no runtime response scanning) |
| **CI Workflow Injection** | Shell injection via workflow_dispatch inputs | Pinned GHA SHAs |
| **Cross-Project Confused Deputy** | CE allows arbitrary directory indexing within $HOME | Path resolution controls (partial — no project-ownership gate) |
| **Secret Exclusion Override** | `.contextignore` negation patterns can un-exclude secrets (e.g., `!.env`) | `SECURITY_IGNORE_PATTERNS` applied after user patterns — negation cannot override (#319) |
| **Governance Content Egress** | Keyword adjudication sends planned_action/context externally to Codex | Codex CLI opt-in (partial — no per-invocation confirmation or content preview) |
...
- **Tree-sitter grammar safety**: `tree-sitter-language-pack` loads compiled C grammars via FFI; mitigated by version pinning (`>=0.7.0,<1.0`), limited to 6 priority grammars in use (Python, JS, TS, Go, Rust, Java), signed build provenance. Non-priority languages use line-based parsing (no FFI).
