...
The MCP initialization message provides concise tool-use, assessment, safety and
communication guidance. It fits the observed 2,048-character host delivery window
(and a 2,048-byte UTF-8 budget); this is not a universal MCP protocol limit.
Detailed behavioral guidance remains available through
`get_principle('meta-method-behavioral-floor-directives')`. Retrieval reports
availability and relevant guidance, not permission or proof of compliance.
...
| `index_loaded` | boolean | Whether retrieval loaded a governance index |
| `semantic_available` | boolean | Whether semantic search was available; false with a loaded index means keyword-only retrieval |
| `retrieval_warning` | string or null | Explicit coverage limitation for unavailable/degraded retrieval; retained independently of principle-body limits |
| `relevant_principles` | array | Principles relevant to the action, each with `id`, `title`, `content` (string **or `null`**), `relevance`, `score`, `series_code`, `domain`. `content` is **bounded to a body-size budget** to reduce payload; this does not bound the complete response or guarantee every host's limit: triggered S-Series + top-scoring bodies are inline; lower-ranked are reference-only (`content: null`) — fetch via `get_principle(id)`. |
...
| `compliance_evaluation` | array | Empty for retrieval-only guidance: surfacing a principle does not establish action compliance. Reserved for actual evaluations. |
...
Missing-index and keyword-only retrieval route to `REVIEW` with AI judgment guidance,
including when nothing matches. Genuine safety escalation retains precedence. Incomplete
retrieval alone does not withdraw existing user authorization or require another permission
request. Healthy retrieval with no principles above the review threshold may return `PROCEED`;
this is a routing result, not proof that an action complies with every principle.
...
- Omission markers and exact IDs for full-detail retrieval when content is withheld

**Principle bodies are returned in full when space permits.** Bodies are
...
a paragraph boundary with an inline marker naming the same call. In the ordinary format the ID also
appears in the footer. Compact responses retain inline pointers and an explicit notice. A unit whose indexed body is empty says so explicitly rather than rendering blank.
...
- **It is not pure score order.** Any principle with `series_code == "S"` allocates ahead
  ...
  match's body. A retained entry names its withheld body with a fetch call. The corpus bounds
...
**The complete response is capped at 32,000 Python characters**, including headings,
metadata, omission notices and the appended governance reminder. This is a product
limit, not a token count, serialized wire-byte limit, or universal MCP/host guarantee.
The 20,000-character body budget still applies within it. Ordinary results below the
complete limit retain their format.

Larger results use compact metadata: long titles are marked as clipped; scores,
match reasons and reference summaries are omitted. Reference status and maturity,
confidence, safety and retrieval-health warnings remain. Exact IDs and body-withheld
pointers are reserved before inline bodies. Selection favors S-Series principles,
then score, while display preserves the original hierarchy. This affects delivery
only; retrieval ranking, logged matches and stored content are unchanged.

If even that entry list cannot fit, whole entries are omitted and counted explicitly;
IDs are never clipped into unusable fetch pointers. Narrow the query or specify a
domain to discover entries absent from that response. Use `get_principle('<id>')`
for full principle, method or reference detail. The limit applies to `query_governance`,
including its input-error responses; it does not newly cap `get_principle` or other tools.
...
**Purpose:** Check consultation evidence for a completed action by matching words against
the in-memory audit log. This does not verify behavior, whether guidance was followed, or
action compliance. A matching record is evidence of consultation only.
...
| `status` | string | Legacy `COMPLIANT`, `NON_COMPLIANT`, or `PARTIAL` consultation-evidence status |
| `verification_scope` | string | Always `consultation_evidence` |
| `limitation` | string | Explains word-overlap matching and that behavior/compliance are not verified |
