...
from ..path_resolution import looks_like_project, safe_cwd
...
# Load-time embedding-space canary gate thresholds (BACKLOG #59).
# Values mirror retrieval.py CANARY_COSINE_FLOOR / CANARY_COSINE_WARN (#58):
# a same-space re-encode is deterministic (cosine ≈ 1.0); a divergent space
# scores near-orthogonal (≈ 0.0). 0.95 tolerates backend numeric drift while
# catching any real space change.
...
class ProjectManager:
    ...
    def __init__(
        ...
        self._loaded_code_edges: dict[str, list[dict]] = {}
    ...
    def query_project(
        ...
        if project_path is None:
            ...
            if cwd is None:
                raise ValueError(
                    "The process working directory is unavailable (deleted?). "
    ...
    def find_references(
        ...
        """Find code references for a symbol.

        Args:
            symbol: Symbol name to search for.
            project_path: Project to search. Defaults to CWD.
            direction: "callers" (who references this), "callees" (what this references), or "all".

        Returns:
            Dict with callers, callees, and metadata.
        """
        if project_path is None:
            project_path = safe_cwd()
            if project_path is None:
                raise ValueError(
                    "The process working directory is unavailable (deleted?). "
                    "Pass project_path explicitly."
        ...
        for edge in edges:
            if direction in ("all", "callers"):
                if edge.get("target_symbol") == symbol or edge.get(
                    "target_path", ""
                ).endswith(f"/{symbol}.py"):
                    callers.append(
                        {
            ...
            if direction in ("all", "callees"):
                if edge.get("source_symbol") == symbol:
                    callees.append(
                        {
    ...
    async def build_knowledge_graph(self, project_path: Path) -> dict:
        if self.readonly:
            raise RuntimeError(
                "Cannot build knowledge graph in read-only mode. "
                "Build from a writable environment first."
            )
        ...
        if not self.storage.project_exists(project_id):
            raise ValueError(
                f"Project not indexed yet. Run index_project first: {project_path}"
            )

        index_path = self.storage.get_index_path(project_id)
        adapter = CogneeAdapter(project_index_path=index_path)
        adapter.configure()

        try:
            chunks_data = self.storage.load_chunks(project_id)
            chunks = [ContentChunk(**c) for c in chunks_data] if chunks_data else []
        except Exception as exc:
            raise ValueError(f"Failed to load chunks for KG build: {exc}") from exc

        await adapter.add_chunks(chunks, dataset_name=project_id)
        result = await adapter.cognify(dataset_name=project_id)
        return result
    ...
    async def query_knowledge_graph(
        ...
        if not adapter.has_knowledge_graph():
            return {
                "query": query,
                "results": [],
        ...
        adapter.configure()
        results = await adapter.search(query, search_type=search_type, top_k=top_k)
        return {
            "query": query,
            "results": results,
            "search_type": search_type,
    ...
    def _load_project(self, project_id: str) -> ProjectIndex:
        ...
        self._loaded_indexes[project_id] = index
        # skip_embeddings (not just the pop above) is what actually disables
        # semantic search on mismatch: the pop alone made the reload guard in
        # _load_search_indexes TRUE, so the incompatible embeddings were
    ...
    def _load_search_indexes(
        self, project_id: str, skip_embeddings: bool = False
    ) -> None:
        """Load embeddings and BM25 index for a project.

        skip_embeddings=True (model mismatch in _load_project) loads only
        BM25/code-edges, leaving semantic search disabled until re-index.
        Also validates embeddings/chunks length consistency and runs the
        embedding-space canary gate (BACKLOG #59).
        """
        if not skip_embeddings and project_id not in self._loaded_embeddings:
            embeddings = self.storage.load_embeddings(project_id)
            if embeddings is not None:
                # A1 FIX: Validate embeddings/chunks length consistency.
                # Invariant: every call site populates _loaded_indexes before
                # calling here, so index is None should be unreachable — if a
                # future call site breaks that, the embeddings store below
                # would bypass both the length check and the canary gate.
                index = self._loaded_indexes.get(project_id)
                ...
                elif index is not None and not self._canary_check_passes(
                    project_id, index
                ):
                    # Gate failed — leave _loaded_embeddings unpopulated so
                    # _semantic_search returns empty and queries degrade to
                    # BM25-only (loud, never silent).
                    pass
                else:
        ...
        code_edges = self.storage.load_code_edges(project_id)
        if code_edges is not None:
            self._loaded_code_edges[project_id] = code_edges

    def _canary_check_passes(self, project_id: str, index: ProjectIndex) -> bool:
        """Behavioral embedding-space canary gate (BACKLOG #59, port of #58).

        The structural checks (model-label in _load_project, length above)
        cannot detect a divergent embedding space — vectors built by a
        different-direction encoder are structurally indistinguishable from
        good ones (the governance session-205→209 silent-BM25 incident
        class). Re-encode the build-time canary texts via the REAL query
        encoder and compare to their stored build vectors.

        Runs ONCE at load, never per-query — the in-process encoder is
        cached after first resolution and cannot change space mid-session,
        so per-query re-checks would only add daemon roundtrips. Encoding
        under _index_lock inherits the existing _semantic_search precedent
        (query-time encode already runs under the same lock); the daemon
        roundtrip is timeout-bounded and the canary set is ≤3 short texts.

        Returns True when embeddings are safe to use (including the legacy
        no-canaries case); False degrades the project to BM25-only.
        """
        canaries = getattr(index, "embedding_canaries", None)
        if not canaries:
            return True  # legacy index (pre-#59) — gate skipped, mirrors #58
        try:
            encoder = self._indexer.embedding_model
            if encoder is None:
                # Readonly mode never loads an encoder; semantic search is
                # already dead per-query, so discard with an accurate message
                # rather than letting the encode raise and misdiagnose this
                # as build/query divergence.
                logger.warning(
                    "Embedding-space canary check skipped for %s: no encoder "
                    "available (read-only mode). Using BM25-only.",
                    project_id,
                )
                return False
            ...
            norms = np.linalg.norm(encoded, axis=1) * np.linalg.norm(stored, axis=1)
            with np.errstate(divide="ignore", invalid="ignore"):
                cosines = np.where(
                    norms > 0, (encoded * stored).sum(axis=1) / norms, 0.0
                )
            min_cos = float(cosines.min()) if len(cosines) else 1.0
            if min_cos < CANARY_COSINE_FLOOR:
                logger.error(
                    "Embedding-space canary check FAILED for %s (min cosine "
                    "%.4f < %.2f): the stored vectors do not match the query "
                    "encoder's space (build/query embedder divergence, "
                    "BACKLOG #59). Falling back to BM25-only — re-index the "
                    "project to fix.",
                    project_id,
                    min_cos,
                    CANARY_COSINE_FLOOR,
                ...
                return False
            if min_cos < CANARY_COSINE_WARN:
                logger.warning(
                    "Embedding-space canary check passed but low for %s (min "
                    "cosine %.4f); possible backend/precision drift between "
                    "build and query encoders.",
                    project_id,
                    min_cos,
                )
            return True
        except Exception as e:  # noqa: BLE001 — gate must never crash a load
            logger.error(
                "Embedding-space canary check errored for %s (%s); treating "
                "as failure and falling back to BM25-only to stay fail-safe.",
                project_id,
                e,
            )
            return False
    ...
    def _start_watcher(self, project_path: Path, project_id: str) -> None:
        ...
        def on_change(changed_files: list[Path]) -> None:
            ...
            try:
                ...
                new_code_edges = self.storage.load_code_edges(project_id)
                ...
                with self._index_lock:
                    ...
                    # No canary gate on this path (deliberate, BACKLOG #59):
                    # these vectors were just built by the same in-process
                    # encoder that serves queries, so build==query space by
                    # construction; the load-time gate in _load_search_indexes
                    # re-verifies at the next process boundary. Only guard
                    # length consistency (mirror _load_search_indexes).
                    if new_embeddings is not None and len(new_embeddings) == len(
                        new_index.chunks
                    ):
                        ...
                        if new_embeddings is not None:
                            logger.warning(
                                "Embeddings/chunks length mismatch for %s after "
                                "incremental update (%d embeddings vs %d chunks). "
                    ...
                    if new_bm25 is not None:
                        ...
                        self._loaded_bm25.pop(project_id, None)
                    if new_code_edges is not None:
                        self._loaded_code_edges[project_id] = new_code_edges
                    else:
                        self._loaded_code_edges.pop(project_id, None)
                    self._watcher_failures.pop(project_id, None)
