...
class EmbeddingCanary(BaseModel):
    ...
    `text` is the EXACT string fed to encode() at build (import-context
    prefix + truncation applied), and `vector` is the embedding row the
    build produced for it — witnessing the actual build space. At load the
    project manager re-encodes `text` via the query encoder and asserts
    cosine(re-encoded, vector) is high; a low value means build-space !=
    query-space — the silent BM25-only failure class that the model-label /
    length checks cannot detect (BACKLOG #59, port of #58).
    """

    text: str = Field(..., description="Exact string encoded at build time")
    vector: list[float] = Field(
        ..., description="Embedding the build produced for text"
    )
...
class ProjectIndex(BaseModel):
    ...
    embedding_canaries: list[EmbeddingCanary] = Field(
        ...
        description="Build-time (text, vector) probes for load-time "
        "build-space==query-space verification (BACKLOG #59). Optional — "
        "absent in legacy indexes, in which case the load-time canary gate "
        "is skipped.",
    )
