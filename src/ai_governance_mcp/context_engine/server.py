...
- find_references: Structural code references (imports, calls, inheritance)
- build_knowledge_graph: Build LLM-based KG from indexed content (opt-in)
- query_knowledge_graph: Query the knowledge graph for entities/relationships
...
from ..path_resolution import is_within_allowed_scope, looks_like_project, safe_cwd
...
The knowledge graph tools (`build_knowledge_graph`, `query_knowledge_graph`) require the `cognee` package (`pip install -e ".[knowledge-graph]"`). Unlike indexing (free, deterministic), KG construction uses LLM calls for entity extraction — recommended local providers: LM Studio (MLX, free) or Ollama (GGUF, free). Cloud providers (anthropic, openai) incur API charges. Configure via `AI_CONTEXT_ENGINE_COGNEE_*` env vars (LLM provider, model, endpoint, temperature, embedding model/dimensions). See README for model recommendations and cost estimates. Use `build_knowledge_graph` after `index_project` to construct the KG. Use `query_knowledge_graph` for entity relationships and concept connections that semantic search might miss. `project_status` shows whether a KG has been built.
    ...
    async def list_tools() -> list[Tool]:
        return [
            ...
            Tool(
                ...
                description=(
                    "Find code references for a symbol — who imports it, "
                    "calls it, or extends it. Returns structural relationships "
                    "from the code reference graph built during indexing."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        "symbol": {
                            ...
                            "description": (
                                ...
                                "Examples: 'CodeConnector', 'parse', 'BaseStorage'"
                            ),
                            "minLength": 1,
                            "maxLength": 200,
                        },
                        ...
                        "direction": {
                            ...
                            "description": (
                                "Direction of references: 'callers' (who references this symbol), "
                                "'callees' (what this symbol references), or 'all' (both). "
                                "Default: 'all'."
                            ),
                            "default": "all",
                        },
            ...
            Tool(
                ...
                description=(
                    "Build a knowledge graph for the current project using Cognee. "
                    "Extracts entities and relationships from indexed content via LLM. "
                    "IMPORTANT: This uses LLM calls with real cost — default provider "
                    "is LM Studio or Ollama (free/local). Cloud providers (anthropic, openai) "
                    "incur API charges. Configure model via AI_CONTEXT_ENGINE_COGNEE_* "
                    "env vars (see README for model recommendations). "
                    "Requires project to be indexed first via index_project."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            Tool(
                ...
                description=(
                    "Query the knowledge graph for entity and relationship information. "
                    "Requires build_knowledge_graph to have been run first. "
                    "Use for: entity relationships, concept connections, "
                    "structural knowledge that semantic search might miss."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": (
                                "Natural language query about entities, "
                                "relationships, or concepts in the project."
                            ),
                            "minLength": 1,
                            "maxLength": MAX_QUERY_LENGTH,
                        },
                        "search_type": {
                            ...
                            "description": (
                                "Type of knowledge graph search. "
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        try:
            if name == "query_project":
                ...
                return await _handle_find_references(manager, arguments)
            elif name == "build_knowledge_graph":
                return await _handle_build_knowledge_graph(manager, arguments)
            elif name == "query_knowledge_graph":
                return await _handle_query_knowledge_graph(manager, arguments)
            else:
                return [
                    TextContent(
                        type="text",
...
def _resolve_project_path(arguments: dict) -> Path | None:
    ...
    cwd = safe_cwd()
    if cwd is None:
        # Working directory deleted out from under the process. Falling through
        # to the explicit-path guidance is exactly right: there is no project to
        # infer, and query_project is hook-enforced, so raising here would take
        # the CE gate down the way a dead cwd once took the governance gate down.
        logger.info(
            "Working directory is unavailable — cannot use as fallback. "
...
async def _handle_project_status(
    ...
    try:
        ...
        logger.warning("Could not determine KG status: %s", e)
...
async def _handle_find_references(
    ...
    if not symbol or len(symbol) > 200:
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"error": "symbol is required (1-200 characters)"},
                    indent=2,
                ),
            )
        ]
    ...
    if direction not in ("callers", "callees", "all"):
        direction = "all"
    ...
    result = await loop.run_in_executor(
        None,
        lambda: manager.find_references(
            symbol=symbol,
...
async def _handle_build_knowledge_graph(
    manager: ProjectManager, arguments: dict
) -> list[TextContent]:
    """Handle build_knowledge_graph tool call."""
    if not CogneeAdapter.is_available():
        return [
            TextContent(
                ...
                text=json.dumps(
                    {
                        "error": "cognee is not installed",
                        "install": "pip install -e '.[knowledge-graph]'",
                    },
                    indent=2,
                ),
            )
    ...
    if not _check_index_rate_limit():
        return [
            TextContent(
                ...
                text=json.dumps(
                    {
                        "error": "Rate limit exceeded for knowledge graph build. "
    ...
    try:
        result = await manager.build_knowledge_graph(project_path)
        return [
            TextContent(
                type="text",
                text=json.dumps(result, indent=2),
...
async def _handle_query_knowledge_graph(
    ...
    """Handle query_knowledge_graph tool call."""
    ...
    if not query or len(query) > MAX_QUERY_LENGTH:
        return [
            TextContent(
                ...
                text=json.dumps(
                    {
                        "error": f"query is required (1-{MAX_QUERY_LENGTH} characters)",
                    },
                    indent=2,
                ),
            )
    ...
    valid_search_types = {"GRAPH_COMPLETION", "SUMMARIES", "INSIGHTS", "CHUNKS"}
