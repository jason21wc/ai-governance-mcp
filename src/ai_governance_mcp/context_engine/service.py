...
from .._exec import resolve_python
...
def _find_watcher_executable() -> str:
    ...
    if path:
        return path
    # Fallback: absolute interpreter + module invocation (PATH-independent).
    return f"{resolve_python()} -m ai_governance_mcp.context_engine.watcher_daemon"
...
def _generate_macos_measurement_plist(script_path: Path) -> str:
    ...
    Writes ~/.context-engine/PHASE2_TRIGGERED on a Phase-2 regression/runaway
    (R1 = embedding daemon socket absent; T3 = watcher peak past the runaway
    ceiling — re-aimed session-215 from the old memory-level thresholds).
    Compliance Review Check 6b.2 reads that marker.
