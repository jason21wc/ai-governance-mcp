...
from .models import ContentChunk, EmbeddingCanary, FileMetadata, ProjectIndex
...
# Applied AFTER user patterns so negation (`!.env`) cannot override them.
...
class Indexer:
    ...
    def index_project(
        ...
        # Resolve code references (imports, calls, inheritance)
        code_edges = self._resolve_code_references(project_path, all_chunks)
        ...
        project_index = ProjectIndex(
            ...
            embedding_canaries=self._make_canaries(all_chunks, embeddings),
        )
        ...
        if code_edges is not None:
            self.storage.save_code_edges(
                project_id, [e.model_dump() for e in code_edges]
            )
        ...
        logger.info(
            "Project indexed successfully: %d chunks, %d files, %d edges",
            len(all_chunks),
            len(all_metadata),
            len(code_edges) if code_edges else 0,
        )
    ...
    def incremental_update(
        ...
        project_index = ProjectIndex(
            ...
            # Refresh canaries from the final matrix on every incremental
            # save. For an unchanged-chunk canary the vector is the reused
            # old row and the text transform is deterministic from stored
            # content, so the pair stays consistent; if the old rows are from
            # a divergent space, the load-time gate catches it at the next
            # process boundary (BACKLOG #59 — deliberately NO build-side
            # space check here: in-process, build and query share the same
            # cached encoder, and a re-encode guard wired to a full-re-index
            # fallback is a re-index-storm surface under a flapping daemon).
            embedding_canaries=self._make_canaries(all_chunks, embeddings),
        )

        # Resolve code references with the full updated chunk set
        code_edges = self._resolve_code_references(project_path, all_chunks)

        # Atomic save — ordered: chunks → embeddings → bm25 → edges → metadata → manifest LAST
        ...
        logger.info(
            "Incremental update complete: %d total chunks (%d reused, %d new), %d edges",
            len(all_chunks),
            n_unchanged,
            len(new_chunks),
            len(code_edges) if code_edges else 0,
    ...
    def _get_chunking_version(self) -> str:
        ...
        if code_connector and code_connector._tree_sitter_available:
            return "tree-sitter-v3"
        return "line-based-v2"

    def _resolve_code_references(self, project_path: Path, chunks: list) -> list | None:
        """Resolve cross-file code references into typed edges.

        Gracefully degrades: returns None on failure so indexing
        succeeds even if reference resolution encounters errors.
        """
        try:
            from .reference_resolver import ReferenceResolver

            resolver = ReferenceResolver(project_path)
            edges = resolver.resolve_all(chunks)
            logger.info("Resolved %d code reference edges", len(edges))
            return edges
        except Exception as e:
            logger.warning("Code reference resolution failed: %s", e)
            return None
    ...
    def _reconstruct_index(
        ...
        return ProjectIndex(
            ...
            embedding_canaries=metadata.get("embedding_canaries", []),
        )
    ...
    def load_ignore_patterns(self, project_path: Path) -> pathspec.GitIgnoreSpec:
        ...
        patterns.extend(SECURITY_IGNORE_PATTERNS)
    ...
    @staticmethod
    def _embedding_input_text(chunk: ContentChunk) -> str:
        """The exact string fed to encode() for a chunk.

        Single source of truth for the embed-text transform — canaries store
        this same string so the load-time gate's re-encode reproduces the
        build's input byte-for-byte (BACKLOG #59). Deterministic from stored
        chunk fields, so it is reproducible for unchanged chunks across
        incremental updates.
        """
    ...
    def _make_canaries(
        self, chunks: list[ContentChunk], embeddings: np.ndarray
    ) -> list[EmbeddingCanary]:
        """Pick ≤3 spread (text, vector) canaries from the final matrix.

        Vectors are REUSED from the just-built embeddings, witnessing the
        ACTUAL build space (whatever encoder produced it). Unlike the
        governance extractor (#58), no force-local canonicality is required:
        this index is machine-local and queried by the same process's
        encoder, so the invariant is build-space == query-space
        self-consistency — a self-consistent daemon space is valid for
        search. A few spread probes suffice: a directional flip is a
        property of the space, not of which texts probe it.
        """
        if len(chunks) == 0 or embeddings is None or len(embeddings) != len(chunks):
            return []
        indices = sorted({0, len(chunks) // 2, len(chunks) - 1})
        return [
            EmbeddingCanary(
                text=self._embedding_input_text(chunks[i]),
    ...
    def _generate_embeddings(self, chunks: list[ContentChunk]) -> np.ndarray:
        ...
        texts = [self._embedding_input_text(chunk) for chunk in chunks]
