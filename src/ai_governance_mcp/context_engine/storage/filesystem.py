...
class FilesystemStorage(BaseStorage):
    ...
    def save_code_edges(self, project_id: str, edges: list[dict]) -> None:
        path = self._ensure_dir(project_id)
        _atomic_write_json(path / "code_edges.json", edges)

    def load_code_edges(self, project_id: str) -> list[dict] | None:
        json_path = self.get_index_path(project_id) / "code_edges.json"
        if json_path.exists():
            if json_path.stat().st_size > MAX_JSON_FILE_SIZE_BYTES:
                logger.warning(
                    "Code edges file exceeds %d byte limit, skipping: %s",
                    MAX_JSON_FILE_SIZE_BYTES,
                    project_id,
                )
                return None
            try:
                ...
                logger.warning(
                    "Corrupt code edges file for project %s, removing: %s",
                    project_id,
                    e,
                )
                try:
...
class ReadOnlyFilesystemStorage(FilesystemStorage):
    ...
    def save_code_edges(self, project_id: str, edges: list[dict]) -> None:
        raise ReadOnlyStorageError("Cannot save code edges in read-only mode.")
    ...
    def load_code_edges(self, project_id: str) -> list[dict] | None:
        ...
        if json_path.exists():
            ...
            try:
                ...
                logger.warning(
                    "Corrupt code edges for project %s (read-only, cannot remove): %s",
                    project_id,
                    e,
                )
                return None
