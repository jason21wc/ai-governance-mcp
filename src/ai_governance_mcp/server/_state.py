...
def get_domain_names() -> list[str]:
    ...
    """
    if _engine is not None and _engine.index is not None:
...
def _load_tiers_config() -> dict | None:
    ...
    if not isinstance(_tiers_config, dict):
        logger.warning(
            "tiers.json has wrong shape: expected dict, got %s — "
            "universal floor disabled",
            type(_tiers_config).__name__,
        )
        _tiers_config = None
        _tiers_loaded = True
        return None
    ...
    for key in _SECTION_KEYS:
        if key in _tiers_config and not isinstance(_tiers_config[key], dict):
            logger.warning(
                "tiers.json section %r has wrong shape: expected dict, got %s — "
                "universal floor disabled",
                key,
                type(_tiers_config[key]).__name__,
            )
            _tiers_config = None
            _tiers_loaded = True
            return None
...
def _build_universal_floor(tiers_config: dict) -> list[dict]:
    ...
    for d in behavioral.get("directives", []):
        ...
        if d.get("principle_ref"):
            item["principle_ref"] = d["principle_ref"]
        items.append(item)
