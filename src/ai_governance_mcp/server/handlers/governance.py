...
async def _handle_evaluate_governance(
    ...
    - Returns relevant guidance, retrieval coverage, and a routing assessment
    """
    ...
    # First pass: identify S-Series triggers and best score.
    ...
    # Retrieval coverage is independent of match quality. No matches from an
    # unavailable index cannot support the healthy no-match PROCEED route.
    # Keep all safety routing above intact, including keyword adjudication.
    retrieval_warning = None
    if not result.index_loaded:
        retrieval_warning = (
            "Governance index unavailable: neither semantic nor keyword retrieval "
            "could run. Missing results do not mean no principles apply."
        )
    elif not result.semantic_available:
        retrieval_warning = (
            "Degraded coverage: keyword-only retrieval; semantic search was unavailable. "
            "Relevant principles may be missing, including when no keywords matched."
        )
    if retrieval_warning:
        rationale = f"{retrieval_warning} {rationale}"
        if not s_series_triggered:
            assessment = AssessmentStatus.REVIEW
            requires_ai_judgment = True
            # Replace any healthy no-match permission claim, retaining benign
            # adjudication and matched-principle guidance where present.
            if ai_judgment_guidance is None:
                rationale = retrieval_warning
            coverage_guidance = (
                "Use AI judgment with the available instructions, source documents, "
                "and any surfaced principles; do not infer clearance from missing results. "
                "Existing user authorization remains in effect. Retrieval unavailability "
                "alone does not require another permission request; genuine safety and "
                "authority boundaries still apply."
            )
            ai_judgment_guidance = " ".join(
                part for part in (ai_judgment_guidance, coverage_guidance) if part
            )

    confidence = _determine_confidence(best_score, s_series_triggered)
    if retrieval_warning and not s_series_triggered:
    ...
    governance_assessment = GovernanceAssessment(
        ...
        index_loaded=result.index_loaded,
    ...
    auto_reasoning = GovernanceReasoningLog(
        ...
        if auto_reasoning_entries
            ...
            ReasoningEntry(
                principle_id="none",
                status="EVALUATED",
                reasoning=retrieval_warning
                or "No strongly relevant principles found for this action",
            )
        ],
...
async def _handle_verify_governance(args: dict) -> list[TextContent]:
    ...
    - Returns legacy COMPLIANT, NON_COMPLIANT, or PARTIAL consultation statuses
    - Does not verify behavior or whether surfaced guidance was followed
    """
    ...
    if expected_principles:
        ...
        if missing:
            verification = VerificationResult(
                ...
                finding=(
                    f"A matching consultation record was found (audit_id: {best_match.audit_id}), "
                    f"but not all expected principles appear in that record. "
    ...
    verification = VerificationResult(
        ...
        matching_audit_id=best_match.audit_id,
        finding=(
            f"A matching consultation record was found. "
