    ...
    if domain is not None and domain not in valid_domains:
        return [
            TextContent(
                type="text",
                text=(
                    f"Error: Invalid domain '{_clip(domain, QUERY_ECHO_MAX_CHARS)}'. "
                    f"Valid: {_clip(', '.join(sorted(valid_domains)), QUERY_ECHO_MAX_CHARS)}"
                ),
            )
        ]
...
def _allocate_result_content(result) -> tuple[dict[int, str | None], list[str]]:
    ...
    rendered into a single response; budgeting them separately would double the
    body allowance. The complete response also has a separate product limit.
...
def _retrieval_header(result, *, compact: bool = False) -> list[str]:
    """Shared health/safety warnings; compact mode also bounds query metadata."""
    lines = []
    if result.s_series_triggered:
    ...
    domains = ", ".join(result.domains_detected) or "None (Constitution only)"
    if compact:
        domains = _clip(domains, QUERY_ECHO_MAX_CHARS)
    lines.append(f"**Domains Detected:** {domains}")
    if result.domain_scores and not compact:
...
def _format_retrieval_result(result) -> str:
    ...
    lines.extend(_retrieval_header(result))
...
def _compact_retrieval_result(result, bodies: dict[int, str | None], limit: int) -> str:
    """Fit complete blocks, retaining exact IDs and health warnings, never slicing output.

    Reserve discovery pointers before spending on bodies. Selection and body upgrades
    use the existing safety-first/score policy; display keeps the original hierarchy.
    Even IDs can exceed the allowance in malformed user data: omit the whole entry,
    count it explicitly and offer a narrower query rather than a broken fetch ID.
    """
    header = "\n".join(_retrieval_header(result, compact=True))
    entries = []
    principles = list(result.constitution_principles) + list(result.domain_principles)
    for i, sp in enumerate(principles):
        p = sp.principle
        heading = f"### [{sp.confidence.value.upper()}] `{p.id}`: {_clip(p.title, 160)}"
        # An empty indexed body is an extraction defect, not a budget omission.
        brief = _render_body(None if p.content.strip() else "", p.id)
        body = _render_body(bodies.get(i), p.id)
        entries.append((p.series_code == "S", sp.combined_score, heading, brief, body))
    for sm in result.methods:
        m = sm.method
        entries.append(
            (
    ...
    for sr in result.references:
        ...
        entries.append(
            (
                False,
                sr.combined_score,
                f"- Reference `{r.id}`: {_clip(r.title, 160)} "
                f"[{r.maturity}/{r.status}] ({sr.confidence.value})",
                "",
                "",
            )
        )
    ...
    def notice(omitted: int) -> str:
        return (
            ...
            f"**Result entries omitted for space: {omitted}.** "
