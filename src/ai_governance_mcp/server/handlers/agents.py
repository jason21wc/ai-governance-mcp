...
async def _resolve_caller_project_path(arguments: dict) -> tuple[Path | None, bool]:
    ...
    cwd = safe_cwd()
    if cwd is None:
        return None, False  # No working directory — force an explicit path
...
def _detect_claude_code_environment(project_path: Path | None = None) -> bool:
    ...
    An unavailable working directory with no explicit ``project_path`` returns
    False — "not detected" is the correct answer when there is nowhere to look,
    and this is a detector, so a negative is a valid result rather than an error.
    """
    if project_path is not None:
        cwd: Path | None = project_path
    else:
        cwd = safe_cwd()
        if cwd is None:
            return False
...
def _get_agent_install_path(
    ...
    Raises:
        ValueError: If agent_name is not in AVAILABLE_AGENTS, path traversal is
            detected, or a project-scoped install has no directory to install
            into (no ``project_path`` and no working directory).
    """
    ...
    if scope == "user":
        ...
        target = project_path if project_path is not None else safe_cwd()
        if target is None:
            # A WRITE with no destination. Unlike the detectors above, guessing
            # would install into whatever directory happened to be inferable —
            # the arbitrary-directory bug (#50, #86) that the marker checks exist
            # to prevent. Demand an explicit path instead.
            raise ValueError(
                "Cannot resolve a project-scoped install path: no project_path was "
                "given and the process working directory is unavailable. Pass "
                "project_path explicitly."
            )
        base_path = target / ".claude" / "agents"
...
async def _handle_install_agent(args: dict) -> list[TextContent]:
    ...
    if scope not in VALID_SCOPES:
        error = ErrorResponse(
            ...
            message=f"Invalid scope: {scope!r}. Must be 'project' or 'user'.",
            suggestions=["Use scope='project' or scope='user'"],
        )
