    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "Returns guidance and routing assessment with PROCEED, REVIEW, or ESCALATE; "
                "retrieval alone does not establish compliance. Missing or keyword-only "
                "retrieval is disclosed and routes to REVIEW unless safety requires ESCALATE. "
                "Retrieval unavailability alone does not withdraw existing user authorization. "
        ...
        Tool(
            ...
            description=(
                ...
                "Returns legacy COMPLIANT, NON_COMPLIANT, or PARTIAL statuses for consultation evidence only. "
                "Does not verify behavior, whether guidance was followed, or action compliance. "
        ...
        Tool(
            ...
            description=(
                ...
                "Pass mode='sync' on an ALREADY-scaffolded project to get a "
                "report-only staleness check — which kit files were added to the "
                "template since it was scaffolded, and which template changes it "
                "predates (with the reason for each). Sync never writes."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "mode": {
                        "type": "string",
                        "description": (
                            "'create' (default) initializes a project, skipping files "
                            "that already exist. 'sync' is REPORT-ONLY for an existing "
                            "project: it lists missing kit files and template changes "
                            "made since the project was scaffolded, and never writes or "
                            "overwrites anything. Sync cannot be combined with confirmed "
                            "or show_manual."
                        ),
                        "enum": ["create", "sync"],
                    },
        ...
        Tool(
            ...
            description=(
                ...
                "the tool handles formatting and file creation. Pass target_root "
                "to redirect the write into an alternate checkout (e.g. a git "
                "worktree) of this same corpus."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "domain": {
                        "type": "string",
                        "description": (
                            "Registered governance domain this entry belongs to. "
                            "Must be one of the registered domains — an unregistered "
                            "folder is never scanned by the extractor (BACKLOG #220)."
                        ),
                        "enum": get_domain_names(),
                        "maxLength": 50,
                    },
                    ...
                    "applies_to": {
                        "type": "array",
                        "description": (
                            "Optional stack/platform/language tokens this entry is "
                            "relevant to, e.g. ['python', 'nextjs']. Omit for "
                            "universal patterns. Used by search_references' 'stack' "
                            "filter — distinct from tags (which affect content search)."
                        ),
                        "items": {"type": "string", "maxLength": 50},
                        "maxItems": 10,
                    },
                    ...
                    "target_root": {
                        "type": "string",
                        "description": (
                            "Optional. Redirect the write into an alternate checkout "
                            "(e.g. a git worktree) of THIS corpus repository — useful "
                            "when self-developing the framework in a worktree. Must be "
                            "the SAME git repository as the configured corpus (validated "
                            "by git identity, not directory shape); a look-alike or "
                            "unrelated project is refused. Omit to write to the "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "stack": {
                        "type": "array",
                        "description": (
                            "Boost entries whose 'applies_to' matches the current "
                            "project's stack/platform/language, e.g. ['python'] or "
                            "['nextjs', 'supabase']. Entries without applies_to are "
                            "universal and unaffected. Boost-only — a mismatch is "
                            "never penalized."
                        ),
                        "items": {"type": "string", "maxLength": 50},
                        "maxItems": 10,
                ...
                "properties": {
                    "section": {
                        ...
                        "description": (
                            ...
                            "'dead_methods', 'maturity_proposals', 'false_positives', "
                            "'retrieval_gaps', 'actionable_recommendations')"
                        ),
                        "maxLength": 50,
                    },
                },
