...
class DocumentExtractor:
    ...
    def _scan_file_for_suspicious_content(
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            views = security_views(line)

            # Check each pattern against both views
            for pattern_type, pattern in SUSPICIOUS_PATTERNS.items():
                matches = [m for v in views for m in pattern.findall(v)]
                if matches:
                    ...
                    # prompt_injection and hidden_instruction have no legitimate place in
                    # indexed governance content, and a wrapper that suppresses them is a
