...
# Emitted only after an index generation has been accepted into live state.
INDEX_READINESS_FORMAT = "Index readiness: accepted_semantic_rows=%d canary_count=%d"
...
# A failed hot reload gets one self-correction attempt on the next request.
# After that, the same committed generation is ignored until global_index.json
# changes again. This preserves the last known-good index without turning every
# retrieval call into another expensive load attempt (BACKLOG #354).
INDEX_RELOAD_MAX_ATTEMPTS = 2
...
class RetrievalEngine:
    ...
    def __init__(self, settings: Settings):
        ...
        # Last generation accepted into the in-memory index. Failed reloads
        # must not advance this marker: observed-on-disk and accepted-in-memory
        # are different states.
        self._index_mtime: float = 0.0
        self._reload_attempt_mtime: Optional[float] = None
        self._reload_attempts: int = 0
    ...
    def _load_index(self) -> None:
        ...
        try:
            ...
            self._record_reload_failure(current_mtime)
            level = (
                logging.ERROR
                if self._reload_attempts >= INDEX_RELOAD_MAX_ATTEMPTS
                else logging.WARNING
            )
            logger.log(
                level,
                "Failed to parse index JSON, keeping previous state "
                "(mtime: %.2f; %s): %s",
                current_mtime,
        ...
        if new_index and new_content_emb is not None:
            ...
            if new_content_emb.ndim != 2:
                logger.warning(
                    "Content embeddings must be a 2D matrix, got shape %s. "
                    "Discarding embeddings.",
                    new_content_emb.shape,
                )
                new_content_emb = None
                new_domain_emb = None
            elif new_content_emb.shape[0] != expected_items:
                logger.warning(
        ...
        # Domain routing is optional, so an invalid domain matrix disables
        # routing without discarding an otherwise healthy content matrix.
        if new_index and new_domain_emb is not None:
            if new_domain_emb.ndim != 2:
                logger.warning(
                    "Domain embeddings must be a 2D matrix, got shape %s. "
                    "Discarding domain embeddings.",
                    new_domain_emb.shape,
                )
                new_domain_emb = None
            elif (
                new_index.embedding_dimensions
                and new_domain_emb.shape[1] != new_index.embedding_dimensions
            ):
                logger.warning(
                    "Domain embedding dimension mismatch: %d vs expected %d. "
                    "Discarding domain embeddings.",
                    new_domain_emb.shape[1],
                    new_index.embedding_dimensions,
                )
                new_domain_emb = None
        ...
        if (
            ...
            logger.log(
                level,
                "Index reload would lose semantic embeddings — keeping previous state "
                "to prevent degradation (mtime: %.2f; consecutive failures: %d; %s).",
                current_mtime,
        ...
        self._reload_attempt_mtime = None
        ...
        logger.info(
            ...
            len(self.content_embeddings) if self.content_embeddings is not None else 0,
            len(self.index.embedding_canaries or []) if self.index is not None else 0,
        )
    ...
    def _check_index_freshness(self) -> None:
        ...
        A refused generation is retried once on the next request. Two failed
        attempts exhaust that generation until the JSON commit signal changes.
        """
        index_path = self.settings.index_path / "global_index.json"
        try:
            current_mtime = index_path.stat().st_mtime
        ...
        if self._index_reload_due(current_mtime):
            with self._index_lock:
                ...
                try:
                    ...
                    return
                if self._index_reload_due(recheck_mtime):
                    logger.info(
    ...
    def _index_reload_due(self, current_mtime: float) -> bool:
        """Return whether a disk generation should be loaded or retried."""
        if current_mtime == self._index_mtime:
            return False
        return not (
            current_mtime == self._reload_attempt_mtime
            and self._reload_attempts >= INDEX_RELOAD_MAX_ATTEMPTS
        )

    def _record_reload_failure(self, current_mtime: float) -> None:
        """Count a failed attempt for one disk generation without accepting it."""
        if current_mtime == self._reload_attempt_mtime:
            self._reload_attempts += 1
        else:
            self._reload_attempt_mtime = current_mtime
            self._reload_attempts = 1

    def _reload_retry_state(self) -> str:
        """Describe whether the refused generation can be attempted again."""
        if self._reload_attempts < INDEX_RELOAD_MAX_ATTEMPTS:
            return "one automatic retry remains"
        return "automatic retry exhausted; waiting for global_index.json to change"
