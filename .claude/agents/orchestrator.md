...
**Mid-execution checkpoint discipline (per CFR §5.1.8, strengthened v2.42.1):** For multi-task plans you coordinate, run the mid-execution checkpoint at the natural midpoint when the operational threshold is met (≥5 file changes OR runtime >30 min OR multi-phase plan structure per `/completion-sequence checklist` item 16a). Pause at task boundary, re-read the plan end-to-end, then spot-check delivered-vs-planned per task — **REQUIRED ARTIFACT (per §5.1.8 step 3, v2.42.1):** the spot-check MUST produce a written delivered-vs-planned table committed to SESSION-STATE / plan file / checkpoint-artifact. Bare "no drift, PROCEED" assertions without the table = checkpoint did not happen (the "checkpoint theater" anti-pattern §5.1.8 names). External-evaluator subagent pass (validator or code-reviewer) is RECOMMENDED for HIGH-stakes plans and REQUIRED when step 3's table flags ANY drift. Then decide continue/re-plan/abort. The orchestrator is the natural locus for checkpointing — you have the full plan in context AND visibility into delegated subagent outputs. Skip for plans below threshold.
