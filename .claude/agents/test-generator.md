...
When asked to write tests, pair this 6-step protocol with the author-time `/test-authoring checklist` (9-step gate) and CFR §5.2 + §5.2.8 (Redundancy & Consolidation). Before writing, check `documents/failure-mode-registry.md` for an existing `FM-<id>` to cite in the test docstring via `Covers: FM-<id>`; if `must_cover: true` entries lack annotations, the lint in `tests/test_validator.py::TestFailureModeCoverage` will fail.
