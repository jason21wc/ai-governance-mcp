...
# BACKLOG retained-entry review runs once, at the host-neutral Git pre-push
# stage in .pre-commit-config.yaml (#367). That stage reads the actual published
# range, prints advisory findings with verbose=true, and blocks comparison errors.
# The former duplicate Check 13b used a weaker inferred range, swallowed successful
# notices, and prescribed re-deletion. Do not restore that conflicting consumer.
...
try:
    for line in open(path):
        ...
        if rec.get("commit") != sha:
            continue
        # A record DESCRIBES this commit only when the tree was clean and HEAD did
        # not move under the run. Otherwise the results may belong to different
        # content -- observed 2026-09-01, when a run begun before a fix recorded
        # against the commit that contained the fix.
        #
        # Non-describing red records are DOWNGRADED, never dropped. Blocking on
        # them risks a false deny, and a false deny trains QUALITY_GATE_SKIP=true,
        # which disables the secret scanner in Check 6. Dropping them silently
        # would delete a real signal -- asserting less than was measured, which is
        # the same defect in the other direction.
        # ONLY an explicit dirty/head_moved flag downgrades. A record predating
        # these fields is left on its historical blocking path: absence of the
        # field is not evidence the run was untrustworthy, and changing behaviour
        # where no information was gained is not a fix.
        if rec.get("dirty") is True or rec.get("head_moved") is True:
            unattributed = rec
        else:
            worst = rec          # last describing record for this commit wins
except OSError:
    sys.exit(0)
if worst and worst.get("fail", 0) > 0:
    failed = [c.get("check") for c in worst.get("checks", []) if c.get("status") == "fail"]
    print(f'"'"'BLOCK|{worst["fail"]} failing: {", ".join(x for x in failed if x)[:120]}'"'"')
elif worst is None and unattributed and unattributed.get("fail", 0) > 0:
    failed = [c.get("check") for c in unattributed.get("checks", []) if c.get("status") == "fail"]
    print(f'"'"'WARN|{unattributed["fail"]} failing: {", ".join(x for x in failed if x)[:120]}'"'"')
' "$HEAD_SHA" "$REPO_ROOT/logs/check-runs.jsonl" 2>/dev/null || echo "")
        case "$LAST_RED" in
            BLOCK\|*)
                debug "Known-red local check run describing HEAD: ${LAST_RED#BLOCK|}"
                emit_deny "QUALITY GATE: your own local check run at this commit reported failures and you are pushing anyway — ${LAST_RED#BLOCK|} . Re-run  bash scripts/check.sh  and fix, or set QUALITY_GATE_SKIP=true to override (audited)."
                ;;
            WARN\|*)
                echo "[check-run-evidence] WARN — a check.sh run recorded against HEAD reported ${LAST_RED#WARN|}, but it ran over a dirty tree or its HEAD moved mid-run, so it may not describe what you are pushing. Re-run  bash scripts/check.sh  on a clean tree to settle it." >&2
                ;;
        esac
    fi
fi
...
try:
    for line in open(path):
        ...
        try:
            ...
            continue
        if rec.get("commit") == sha:
            # Same test as Check 11, and for the same reason in the other
            # direction: a run over a dirty tree, or one whose HEAD moved
            # mid-run, is not evidence that THIS commit was checked. Records
            # written before these fields existed carry no claim either way and
            # are accepted, so this stays advisory rather than retroactively
            # warning on every historical push.
            if rec.get("dirty") is True or rec.get("head_moved") is True:
                continue
            print("true")
            sys.exit(0)
...
' "$CHECK12_SHA" "$REPO_ROOT/logs/check-runs.jsonl" 2>/dev/null || echo "false")
        ...
        if [ "$HAS_RECORD" = "false" ]; then
            echo "[check-run-evidence] WARN — no check.sh run record DESCRIBING HEAD ($CHECK12_SHA). A run over a dirty tree, or one whose HEAD moved mid-run, does not describe this commit. Run  bash scripts/check.sh  before pushing (advisory; V-014 tracking)." >&2
        fi
    fi
fi
