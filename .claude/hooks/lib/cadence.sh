...
# text_field_from_operations <operations_file> <anchor> <field>
#   Echoes the text after a Markdown field label inside the named section, or ''
#   when absent. Example: `**Count basis:** independent-context` returns
#   `independent-context`. Section boundaries and accepted heading spellings are
#   identical to date_field_from_operations; keep both parsers aligned.
text_field_from_operations() {
    python3 -c "
import re, sys
path, anchor, field = sys.argv[1], sys.argv[2], sys.argv[3]
hdr_re = re.compile(r'^#{2,3}\\s')
