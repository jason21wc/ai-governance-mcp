...
if [ -r "$HOOK_DIR/lib/audit-bypass.sh" ]; then
    ...
    echo "[pre-exit-plan-mode-gate] WARNING: lib/audit-bypass.sh missing — degraded, bypasses will not be audited" >&2
    audit_bypass() { :; }
fi
...
# Measured session-272: a must-deny probe returned rc=1 / 0 bytes with HOME
# unset, versus 303 bytes of deny JSON with it set. The `trap 'exit 2' ERR`
# below does NOT fire on an unbound-variable abort.
#
# The root cause is not the variable, it is that a LOGGING path was allowed to
# become a precondition of the VERDICT. HOME is needed only to place this log;
# every write to it is already `|| true`-guarded in _audit_log. So an empty
# value costs a diagnostic line and nothing else. BACKLOG #299 Stage A.
...
emit_deny() {
    ...
    # pre-test-oom-gate.sh. The old guard fired only on a NONZERO exit, so a
    # python3 that exits 0 printing nothing emitted no JSON and fell through to
    # `exit 0`. This file's own header calls that out as the worst case: "a
    # fail-open here defeats every other fail-closed guarantee." BACKLOG #298
    # at the emission point. Deny-path only — cannot brick a session.
    local _deny_json
...
" "$reason" 2>/dev/null) || _deny_json=""
    ...
    echo "[plan-contrarian-gate] python3 unavailable, failed, or emitted no JSON while writing the deny — falling back to exit 2 (hard deny). Set PLAN_CONTRARIAN_SKIP_HOOK=1 to bypass if persistent." >&2
    exit 2
}

# Emit an allow (no JSON; harness defaults to allow on silent exit-0).
#
# The allow is LOGGED, and that is not cosmetic. OPERATIONS V-012 measures a
# *rate* — gate denials over plan-mode invocations — but this log recorded only
# `deny`, `semantic-bypass` and `structural-bypass`, so the denominator was
# never in it. The threshold V-012 states (CONFIRM <=40% / REFUTE >=60%) was
# not computable from the source it names. Logging the allow supplies the
# denominator; without it the instrument can only ever count numerators.
# `bypass` records still route through their own tags, so the outcome classes
# stay distinguishable. NOTE THE BOOKKEEPING: a bypass emits its own tag AND
# then an `allow` (bypasses exit through emit_allow), so a bypass event is TWO
# lines. The convention for anyone computing a rate: **allow + deny = the
# denominator; bypass lines are annotations on an allow, never counted again.**
# (2026-07-25.)
emit_allow() {
    local note="${1:-}"
    if [ -n "$note" ]; then
...
" <<<"$INPUT" 2>/dev/null || echo "")
    ...
    # A third invocation used to sit here, capturing into `atom_status`, which was
    # then never read — the two runs below supersede it. Removed 2026-08-15: it was
    # a dead subprocess on every plan-mode exit, found when shellcheck (SC2034) was
    # first run against this repo.
