...
|--------|------------|---------|----------|
| **Constitution** | 24 | 332 | Universal AI behavior, safety, quality |
| **AI Coding** | 16 | 340 | Software development, testing, deployment |
| **Multi-Agent** | 17 | 54 | Agent orchestration, handoffs, autonomous operation |
| **Storytelling** | 15 | 35 | Creative writing, narrative, voice preservation |
...
| Tool | Purpose |
|------|---------|
| `evaluate_governance` | **Pre-action** compliance check — PROCEED/REVIEW/ESCALATE |
...
- `evaluate_governance` evaluates planned actions against principles BEFORE execution, auto-detects S-Series (safety) concerns, and returns PROCEED, REVIEW, or ESCALATE. REVIEW means relevant principles were surfaced — read and apply them. S-Series violations force the verdict to ESCALATE. **"Force" means the verdict, not the outcome:** ESCALATE instructs the model to stop and surface to you, and nothing downstream consumes it — no code path reads the verdict, and the enforcement proxy gates call order only. Every call logs an `audit_id`.
...
pip install -e .
python -m ai_governance_mcp.extractor   # Build the search index
```

Then use the config generator to set up your AI client:
...
Test: *"Query governance for handling incomplete specifications"*.
...
Use `get_principle` for principle IDs. Methods like §7.8 are *found* through `query_governance`, which lists them by id and title — method bodies are not inlined, so fetch the procedure itself with `get_principle` using the id the query returned:

```
get_principle("meta-core-systemic-thinking")
query_governance("proportional rigor")          # lists method ids + titles
get_principle("meta-method-progressive-application-proportional-response")   # §7.8 itself
```
...
Use the config generator for platform-specific setup — it resolves an **absolute interpreter path** (so GUI hosts like Claude Desktop launch it without needing your shell PATH) and writes the `AI_GOVERNANCE_*` environment variables. It defaults to **advisory** (the server alone); pass `--enforce` to wrap it in the governance proxy (soft mode):
...
python -m ai_governance_mcp.config_generator --json claude --enforce     # Wrap in the proxy (soft mode)
```

> The config generator automatically includes `AI_GOVERNANCE_INDEX_PATH` and `AI_GOVERNANCE_DOCUMENTS_PATH` environment variables. These are required when running the server from a different directory than the project root.
...
The enforcement proxy (`ai-governance-proxy`) wraps the governance MCP server at the protocol level, intercepting JSON-RPC tool calls. In **hard mode** it blocks an action tool that lacks a prior `evaluate_governance()` call; in **soft mode** it only appends a warning and lets the call through. Important: this gates the model's *call order* — it is model-satisfiable (the model can call `evaluate_governance()` itself), **not** a human approval gate. On auto-running GUI hosts (Claude Desktop) a soft warning is post-hoc, so the host's own per-tool approval prompt is the real human gate (see this note — `search_references("ref-ai-coding-connect-local-mcp-server-to-claude-surfaces")`). Works with any MCP client: Claude App, Cursor, Gemini CLI, ChatGPT Desktop, and others.

**The config generator defaults to advisory (no proxy); pass `--enforce` to opt in.**

| Mode | How to configure | What it does | When to use |
|------|-----------------|------------|-------------|
| **Advisory** (config-gen default) | Default — `python -m ai_governance_mcp.server` (no proxy) | Model cooperation only; the host's own approval prompt is the gate | Any MCP host, incl. GUI |
| **Proxy, soft** | `--enforce` (config gen) | Appends a warning, does **not** block; post-hoc on GUI auto-run | A nudge for hookless clients |
| **Proxy, hard** | proxy + `GOVERNANCE_ENFORCEMENT_SOFT_MODE=false` | Blocks an action tool until `evaluate_governance()` is called (protocol-level; model-satisfiable, not human approval) | Clients without their own gate |

**Graduating to hard mode:** enable the proxy (`--enforce`) and set `GOVERNANCE_ENFORCEMENT_SOFT_MODE=false` in your MCP config's `env` block. Action tools are then blocked (not just warned) until `evaluate_governance()` is called — a call-order gate, still model-satisfiable rather than a human approval step.
...
| `GOVERNANCE_ENFORCEMENT_SOFT_MODE` | `false` (`true` in configs generated with `--enforce`) | Warn instead of block |
...
**How to choose** — the trade-offs are stable even though the models and prices are not, so this section describes the shape of the decision and sends you to the vendor for current values. (Model names, prices, and benchmark scores were previously tabulated here and rotted on four vendors' independent release cadences; see `rules-of-procedure` §10.1.4.)

**Local — zero marginal cost, recommended for development.** Any instruction-tuned open model that supports grammar-constrained JSON will work. The axis that matters is memory: a mixture-of-experts model in the ~20 GB class gives the best quality-per-second because only a fraction of its parameters activate per token, a dense model of similar footprint reasons better per token but runs slower, and a large dense model gives the highest local quality at the cost of needing 64 GB+ of unified memory. Constrain JSON output via Outlines (LM Studio) or GBNF (Ollama) — extraction quality depends far more on that than on model choice.

```bash
# Local via LM Studio (MLX acceleration on Apple Silicon)
export AI_CONTEXT_ENGINE_COGNEE_LLM_PROVIDER=custom
export AI_CONTEXT_ENGINE_COGNEE_LLM_MODEL=<your-local-model-id>
export AI_CONTEXT_ENGINE_COGNEE_LLM_ENDPOINT=http://127.0.0.1:1234/v1
export AI_CONTEXT_ENGINE_COGNEE_LLM_API_KEY=.

# Local via Ollama (GGUF)
export AI_CONTEXT_ENGINE_COGNEE_LLM_PROVIDER=ollama
export AI_CONTEXT_ENGINE_COGNEE_LLM_MODEL=<your-ollama-model-tag>
```

**Cloud.** A knowledge-graph build is a high-volume, low-difficulty extraction job — many small structured-output calls, not deep reasoning. **The small/fast tier of any major vendor is the right default**, and the frontier tier is not worth its multiple here. Cost scales with corpus size: a build over a few hundred chunks lands in single-digit dollars on the small tier and roughly an order of magnitude more on the frontier tier. Read current per-token prices from the vendor's own pricing page — they change more often than this file does.

```bash
# Cloud — small/fast tier of any provider
export AI_CONTEXT_ENGINE_COGNEE_LLM_PROVIDER=openai
export AI_CONTEXT_ENGINE_COGNEE_LLM_MODEL=<small-tier-model-id>
export AI_CONTEXT_ENGINE_COGNEE_LLM_API_KEY=sk-...
```

**Before you commit to a model, test structured output on ten chunks.** Structured-output reliability varies by vendor and generation and is the failure mode that actually bites here — a model that reasons well but emits malformed JSON is useless for this job. That check costs pennies and is worth more than any recommendation this file could carry.
...
**"0 domains" or empty principles:** CLI and Desktop configs are separate systems. Check both have `AI_GOVERNANCE_DOCUMENTS_PATH` and `AI_GOVERNANCE_INDEX_PATH` set.

**S-Series false positives:** `evaluate_governance` flags safety-adjacent keywords in `planned_action` text. Most keyword-only triggers (a CRITICAL keyword matched but no S-Series principle retrieved) are now handled server-side: the keyword adjudicator (BACKLOG #73, active by default) routes `benign` topic mentions to REVIEW instead of ESCALATE. If you still see `s_series_check.triggered=true` with empty `principles`, check `keyword_adjudication`: `genuine` or `floor` means a real concern — do NOT override; `unavailable` (judge unreachable) is the fail-safe heuristic case (document the override in your reasoning trace and proceed).
...
│   └── context_engine/      # Context Engine MCP (7 tools)
...
├── .claude/skills/          # Project skills (completion-sequence-aigov, compliance-review, test-authoring)
├── global-skills/           # Canonical source for user-level skills (content-enhancer, code-review, dream, …)
├── reference-library/       # Accumulating applied patterns (secondary authority)
├── .claude/
│   ├── agents/              # Installed subagents
│   └── hooks/               # Enforcement hooks (PreToolUse, UserPromptSubmit, SessionStart, pre/post-push gates)
├── index/                   # Generated index + embeddings
└── tests/                   # Test suite (governance + context engine; run pytest for the current count)
```

## Dogfooding

This project is built using its own governance framework. Development follows the AI Coding framework (SPECIFY → PLAN → TASKS → IMPLEMENT) encoded in [`documents/title-10-ai-coding.md`](documents/title-10-ai-coding.md), with explicit gate criteria per phase. Decisions are logged through `evaluate_governance` + `log_governance_reasoning`. The framework's own hooks enforce governance consultation on source edits to this repo. Lessons from applying the framework to itself are captured in [`LEARNING-LOG.md`](_ai-context/LEARNING-LOG.md) — including corrections when the framework fails its own standards.
...
- [x] pip install (editable and wheel)
...
- [ ] Governance effectiveness measurement (see [BACKLOG.md](_ai-context/BACKLOG.md) #22 for scope)

**Content**
- [x] Shipped domains (see the domain table above) + modular custom domain support (drop in `title-NN-name.md`, rebuild index)
- [ ] Visual communication domain (presentations, reports, print design — see BACKLOG #6)
- [ ] Autonomous operations domain (see BACKLOG #11)

Full roadmap discussion and open questions live in [BACKLOG.md](_ai-context/BACKLOG.md).
