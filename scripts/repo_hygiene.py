...
def collect_local_facts(repo: Path) -> dict:
    ...
    # HOW FAR BEHIND ITS REMOTE IS THE BASE WE JUST PICKED? (BACKLOG #365)
    #
    # resolve_base_ref tries `refs/heads/<base>` BEFORE `refs/remotes/origin/<base>`,
    # and that order is deliberate and correct: a stale local base reads landed work as
    # UNLANDED, which withholds a removal command, and withholding is the safe direction.
    # But safe is not the same as true. Measured 2026-09-01: the primary sat one commit
    # behind `origin/main` and this tool reported "Worktree whose work never landed:
    # ... 1 commit(s) not on refs/heads/main" about `backlog3`, whose work HAD landed.
    #
    # A HIGH finding that is false is how a channel earns being ignored -- which is the
    # subject of #365 itself, so this is not a detour. Record the lag and let the
    # landedness arm below say what was actually measured. -1 means undeterminable, and
    # is treated exactly like "no lag" so an unmeasurable remote never changes behaviour.
    _origin_base = f"refs/remotes/origin/{facts['default_branch']}"
    facts["default_ref_lag"] = -1
    facts["origin_base_ref"] = None
    if _base and _base != _origin_base:
        _rc, _ = _git(repo, "rev-parse", "--verify", "--quiet", _origin_base)
        if _rc == 0:
            facts["origin_base_ref"] = _origin_base
            _rc, _out = _git(
                repo, "rev-list", "--count", _origin_base, "--not", _base, "--"
            )
            facts["default_ref_lag"] = (
    ...
    for w in facts["worktrees"]:
        ...
        # Second opinion, ONLY when the local base is measurably behind its remote.
        # Never replaces `unlanded_commits` -- that stays the conservative number the
        # removal command keys on. This one exists so the TITLE can stop asserting
        # "never landed" about work that is sitting on origin. -1 = not computed.
        w["unlanded_vs_origin"] = -1
        if facts["origin_base_ref"] and facts["default_ref_lag"] > 0:
            _rc, _out = _git(
                repo,
                "rev-list",
                "--count",
                _branch,
                "--not",
                facts["origin_base_ref"],
                "--",
            )
            w["unlanded_vs_origin"] = (
                int(_out.strip()) if _rc == 0 and _out.strip().isdigit() else -1
            )
...
def classify(local: dict, remote: dict | None) -> list[Finding]:
    ...
    def keep_aliases(f: Finding) -> set[str]:
        """Use the same identity for suppression and stale-marker detection.

        Worktree display names are lossy. Preserve their legacy marker spelling,
        but match namespaced markers against the collected full ref, never against
        a guessed prefix or another branch with the same final path component.
        """
        aliases = {f.ref, f.ref.removeprefix("origin/"), f"PR #{f.ref}"}
        if branch_ref := f.evidence.get("branch_ref"):
            branch = branch_ref.removeprefix("refs/heads/")
            aliases.update((branch, f"origin/{branch}"))
        return aliases
    ...
    # Both default to the "nothing measured" values so classify() stays pure over
    # `local` and a caller that predates these facts behaves exactly as before.
    origin_base = local.get("origin_base_ref") or ""
    default_ref_lag = local.get("default_ref_lag", -1)

    # THE STALE PRIMARY IS ITSELF A FINDING (BACKLOG #362 candidate (a), #365).
    #
    # Reported rather than merely used, because a local default behind its remote is
    # not only a measurement problem here: `~/.claude/skills/*` and `~/.codex/hooks/*`
    # are absolute symlinks into the primary checkout, so every project on this machine
    # runs whatever version it holds. Nothing else on the machine notices, and this tool
    # already had to compute the number for the landedness arm above.
    #
    # WARN, never HIGH, and never blocking: being behind is the ordinary state between a
    # sibling's publish and your next fetch. The command is offered ONLY from the
    # primary, because a fast-forward run from a worktree would move the wrong checkout.
    if default_ref_lag > 0:
        findings.append(
            Finding(
                check="stale_default_ref",
                ref=base_branch or "default",
                severity="warn",
                title=(
                    f"Local {base_branch} is {default_ref_lag} commit(s) behind"
                    f" {origin_base} — landedness and any skill or hook symlinked into"
                    " this checkout are measured against stale content"
                ),
                disposition="fix",
                evidence={
                    "default_branch": base_branch,
                    "origin_base_ref": origin_base,
                    "lag_commits": default_ref_lag,
                    "acting_is_primary": bool(local.get("acting_is_primary")),
                },
                command=(
                    f"git merge --ff-only {origin_base}"
                    if local.get("acting_is_primary")
                    else None
                ),
            )
        )
    ...
    for wt in local.get("worktrees", []):
        ...
        if dirty:
            ...
            elif unlanded > 0 and wt.get("unlanded_vs_origin", -1) == 0:
                # LANDED ON ORIGIN, and the only thing that said otherwise was a stale
                # local default (BACKLOG #365). Still a finding -- an unowned worktree
                # is still residue -- but it must not claim the work was never landed,
                # because it was. The removal command STAYS WITHHELD: the two bases
                # disagree, and `unlanded_commits` is the conservative one by design.
                title = (
                    f"Orphan worktree: {wt['path']}"
                    f" — work has landed on {origin_base}, but the local"
                    f" {base_branch} is {default_ref_lag} commit(s) behind it, so"
                    " landedness could not be confirmed locally and no removal"
                    " command is offered. Refresh the primary and re-run."
                )
            elif unlanded > 0:
                title = (
                ...
                mark_keep(
                    Finding(
                        ...
                        evidence={
                            ...
                            "unlanded_vs_origin": wt.get("unlanded_vs_origin", -1),
                            "default_branch": base_branch,
                            "default_ref_lag": default_ref_lag,
                        },
    ...
    known = {alias for f in findings for alias in keep_aliases(f)}
    for ref in keeps:
        if ref not in known:
            findings.append(
                Finding(
