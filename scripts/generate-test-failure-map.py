...
def format_map(
    ...
    lines.append("")
    lines.append(
        "**Freshness IS pre-commit-enforced** via the "
        "`regen-test-failure-mode-map` hook in `.pre-commit-config.yaml` "
        "(session-123 Commit F, BACKLOG #123 closed). The hook regenerates "
        "this map on any change to the registry, test files, or the "
        "generator itself, and fails the commit if the regenerated output "
        "differs from the staged version. If this map disagrees with the "
        "registry, trust the registry and re-stage after running the "
        "generator."
    )
    lines.append("")
