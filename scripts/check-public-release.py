...
# the exact ROOT-RELATIVE path, NOT the basename — a file merely named like the
# guard but placed elsewhere under an allowlisted glob must still be scanned
# (otherwise it is a smuggle vector).
CONTENT_SCAN_EXCLUDED_PATHS = frozenset(
    {
        "documents/.release-forbidden",
        "documents/.public-allowlist",
        "scripts/check-public-release.py",
    }
)

# Path prefixes that are ALWAYS violations, even if (mistakenly) allowlisted. The
# index embeds domain plaintext + embeddings and must never ship — it is a
# gitignored build artifact rebuilt in CI/Docker. Default-deny already catches a
# non-allowlisted index; this closes the residual where an allowlist authoring
# mistake re-permits it.
DENY_PATH_PREFIXES = ("index/",)
...
def load_allowlist(root: Path) -> set[str]:
    ...
    Returns ROOT-RELATIVE POSIX paths (never resolved real paths): comparing
    resolved paths is what lets a symlink collide with — and impersonate — its
    target. Symlinks are skipped here so one can never enter the allowed set.
    """
    allowed: set[str] = set()
    for glob in _parse_pipe_file(root / ALLOWLIST_RELATIVE_PATH):
        for p in root.glob(glob):
            if p.is_symlink():
                continue  # a symlink can never be allowlisted (see check layer)
            if p.is_file():
                allowed.add(p.relative_to(root).as_posix())
    return allowed
...
def walk_files(root: Path) -> list[Path]:
    """All files AND symlinks under root, skipping VCS/build dirs, sorted.

    Uses os.walk(followlinks=False): symlinks (file- or dir-valued) are returned
    as entries so the path layer can flag them — the public tree must contain no
    symlinks. Symlinked directories are recorded but NOT descended into (fail-
    closed: the link is the violation; following it risks cycles and out-of-tree
    reads).
    """
    out: list[Path] = []
    for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        for name in dirnames:
            child = d / name
            if child.is_symlink():
                # Flag BEFORE the skip-dir prune so a symlink named like a skip
                # dir (.git, __pycache__) is still caught; do not descend.
                out.append(child)
                continue
...
def check_path_allowlist(root: Path, allowed: set[str]) -> list[Violation]:
        ...
        if p.is_symlink():
            violations.append(
                Violation(
                    "path-allowlist",
                    rel,
                    "symlink not permitted (public tree must contain no symlinks)",
                )
            )
            continue
        if any(rel == d.rstrip("/") or rel.startswith(d) for d in DENY_PATH_PREFIXES):
            violations.append(
                Violation(
                    "path-allowlist",
                    rel,
                    "always denied (build artifact embedding domain content — "
                    "must never be public, even if allowlisted)",
                )
            )
            continue
...
def check_forbidden_content(
    root: Path, allowed: set[str], forbidden: list[tuple[re.Pattern[str], str]]
) -> list[Violation]:
    violations: list[Violation] = []
    for rel in sorted(allowed):
        if rel in CONTENT_SCAN_EXCLUDED_PATHS:
            continue
        p = root / rel
        if p.suffix.lower() in BINARY_SUFFIXES:
            continue
        # Fail-closed decode: a marker hidden in a non-UTF-8 encoding must NOT be
        # silently mangled past the regex by errors='replace'. An undecodable text
        # file is itself a violation (it cannot be content-scanned).
        try:
            ...
            violations.append(
                Violation(
                    "forbidden-content",
                    rel,
                    "not valid UTF-8 — cannot be content-scanned (fail-closed)",
                )
            )
            continue
        except OSError as exc:  # pragma: no cover - unreadable file
            violations.append(
                Violation("forbidden-content", rel, f"unreadable: {exc} (fail-closed)")
            )
            continue
