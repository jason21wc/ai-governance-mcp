...
- **S-Series safety detection** -- Deterministic keyword detection; S-Series violations force the returned verdict to ESCALATE. The verdict is advisory: no code path consumes it, so "force" describes what the server returns, not what the client is prevented from doing.
...
- Protocol-level governance gate via proxy -- `ai-governance-proxy` wraps any MCP server (Phase 1: own tools, Phase 2: cross-MCP via `--govern-all` or `--config`). It gates the model's *call order* — the model can satisfy it itself — except for the act-intrinsic content gate (`--enforce-acts`), which reads the call's own arguments and is the one tier the model cannot satisfy by re-describing its plan.
