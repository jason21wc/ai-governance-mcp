...
**Multi-file cascade rule:** If a fix causes NEW failures in previously-passing, unrelated files → STOP. Revert the fix immediately. The diagnosis was wrong — the fix is treating a symptom, not the root cause.
...
| **Symptom Treatment** | Fix the visible error without finding root cause | Five Whys: ask "why?" at each level until you reach the originating cause |
