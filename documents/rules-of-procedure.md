...
**This list is the single source of truth for the floor's delivery layers.** Other surfaces (compliance-review Check 3, `EXECUTION-FRAMEWORK.md`) reference it rather than restating it — three documents previously enumerated "the delivery layers" and no two agreed, each omitting a different real one. **Five layers carry the floor, and they carry different amounts of it.** (It read "four" until 2026-08-31, omitting `SERVER_INSTRUCTIONS` — the same defect this paragraph was written to stop, committed in the paragraph that stops it. The checklist's step 4a knew about the layer; this list, which claims to be the single source of truth for layers, did not.)

- **The disposition kernel** in `AGENTS.md` — the shared two-part kernel (reasoning posture + communication style), loaded directly by Codex and imported by `CLAUDE.md` for Claude Code. Edit this shared source; do not duplicate it in the Claude overlay. The former copies diverged: one omitted `intent-over-literal`, the other BLUF and `freeform-dialogue`. A single imported source removes that synchronization requirement; actual host import resolution still needs live verification.
- **The FRAME inject** (`user-prompt-governance-inject.sh`, themes in `src/ai_governance_mcp/frame.py`) — re-anchors a subset of themes on every turn, for both Claude Code and Codex CLI. It adds no directive of its own; it repeats — **in compressed form, so it must never state a rule the floor does not.** Its first answer-shape draft stated the word ceiling *without* the floor's exemptions, making the FRAME stricter than the source it re-anchors; caught in review before landing. When compressing here, drop detail, never add constraint. It carried **reasoning themes only** until 2026-08-31, which is why communication directives decayed as a session grew: they were delivered once at session start and re-anchored never, while `evaluate_governance` (below) fires only before Bash/Edit/Write and so never fires on a purely conversational answer — exactly where communication style applies. It now also re-anchors answer shape (lead-with-the-answer, close-with-the-ask, ask-in-prose).
- **`SERVER_INSTRUCTIONS`** (`src/ai_governance_mcp/server/_constants.py`) — the sole **advisory** channel reaching Claude Desktop, which loads neither the kernel nor the FRAME. Any directive governing conversation style, language, register, or output format needs a paired section here, or on Desktop it surfaces only when the model happens to call `evaluate_governance` — the enforcing layer still reaches Desktop, so this is under-reach, not absence. (Stated precisely because step 4a states it precisely and an earlier copy of this line dropped the qualifier.) Procedure: `/completion-sequence-aigov` checklist step 4a.
...
- **Check:** WHEN you need a decision or information from the user, ask it as a question in prose — not a menu, numbered options, or a table of alternatives. Structured options only when converging on a bounded selection ("which of these 3 configs?").
- **Wrong:** "Option A: Add hooks. Option B: Use advisory. Option C: Build a proxy."
- **Right:** "I'd use hooks — highest reliability, already proven here. Advisory gets ~85%. Go with hooks?"
...
**6. `bluf-pyramid-briefing`** — implements `meta-quality-effective-efficient-outputs`

- **Check:** Open with the answer in 1-3 sentences — what is true, and what it means for the reader — end the update there unless more was asked for. Default ceiling ~200 words; exceed it only when the user asked for depth, or the output is a document or code. A structured report-out is NOT exempt: SA-002's four sections are a shape, not a length licence. See rules-of-procedure §7.13. This limits the message, not execution: continue available authorized work after a progress update or checkpoint.
- **Wrong:** 600 words of context before the two sentences that answer the question; or a dense work-dump where the outcome and the ask appear in the middle of section 4
- **Right:** The answer and the one fact that makes it credible; end the update there. Continue the authorized task. Depth on request, not by default.
...
- **Check:** No throat-clearing openers, no unbacked hedges, no restating a point already made. Stand behind the claim; earn emphasis with content. Advisory; function test, not a banlist. See rules-of-procedure §7.14.
