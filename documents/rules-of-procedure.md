...
3. **Line-only citations are drift-vulnerable.** When citing a line outside a numbered section (e.g., `constitution.md` Article headers at lines 177/424/629/816, Bill of Rights at line 998, Operative Hierarchy table at lines 84-92, F-P2-04 Q7 PASS block at lines 1002-1006), treat the citation as drift-vulnerable. Verify against the SOT file on each major file edit per LEARNING-LOG 2026-04-25 "Verify Source-of-Truth Files Before Anchoring on Review Notes" + 2026-04-28 "Apply Newly-Shipped Specs to Host Files in Same Arc."
4. **CI check (deferred D2 follow-up).** A CI script that parses `\.md:\d+` citations across `documents/` and verifies each cited line matches a header pattern (or named block pattern) would catch the entire drift class structurally on every PR. Filed as BACKLOG #144 — implementation-level, ~30 lines of Python or bash.
