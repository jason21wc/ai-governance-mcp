...
| Completing Specify phase | Gate Artifacts | GATE-SPECIFY.md template |
| Choosing technologies | §3.1 | Architecture Definition |
| Completing Plan phase | Gate Artifacts | GATE-PLAN.md template |
| Breaking down work | §4.1 | Decomposition Requirements |
| Completing Tasks phase | Gate Artifacts | GATE-TASKS.md template |
| Writing code | §5.1 | Implementation Loop |
| Completing Implementation | Gate Artifacts | GATE-IMPLEMENT.md template |
...
|-------------|------|---------|
| **Session State** | `SESSION-STATE.md` | Where we are now (transient) |
| **Project Memory** | `PROJECT-MEMORY.md` | Decisions, specs, architecture |
| **Learning Log** | `LEARNING-LOG.md` | Lessons learned, patterns discovered |
...
### 🔴 CRITICAL: Gate Artifacts

**Importance: CRITICAL — Gates must produce artifacts, not just pass checklists**

Each phase transition requires a completed Gate Artifact. These are structured documents that prove the gate criteria are met. AI must create these artifacts; Product Owner must approve them.

**On Gate Failure:** If any checklist item cannot be checked, the gate fails. Return to the current phase, address the deficiency, update relevant artifacts, then attempt the gate again. Do NOT proceed to the next phase with an incomplete gate. Document the failure and remediation in the Learning Log.

#### Gate Artifact: Specify → Plan

Create `GATE-SPECIFY.md` before transitioning to Plan phase:
...
- **Location:** [path to spec document]
...
- [ ] PO has reviewed specification
- [ ] PO approves proceeding to Plan phase

**PO Notes:** [Any comments from PO]
```

#### Gate Artifact: Plan → Tasks

Create `GATE-PLAN.md` before transitioning to Tasks phase:
...
## Product Owner Approval
- [ ] PO has reviewed architecture
- [ ] PO approves proceeding to Tasks phase

**PO Notes:** [Any comments from PO]
```

#### Gate Artifact: Tasks → Implement

Create `GATE-TASKS.md` before transitioning to Implement phase:
...
- **Parallel opportunities:** [Y tasks can run parallel]

## Task List Reference
- **Location:** [path to task list]
...
## Product Owner Approval
- [ ] PO has reviewed task breakdown
- [ ] PO approves proceeding to Implement phase

**PO Notes:** [Any comments from PO]
```

#### Gate Artifact: Implement → Complete

Create `GATE-IMPLEMENT.md` at project completion:

```markdown
# Gate Artifact: Implementation Complete
**Project:** [Name]
**Date:** [Date]
**Mode:** [Mode]
...
- [x] All tests passing
- [x] Test coverage ≥80%
- [x] Security scan: Zero HIGH/CRITICAL
...
| Issue | Severity | Plan |
|-------|----------|------|
| [Issue] | [Low/Med] | [Deferred to...] |

## Product Owner Approval
- [ ] PO has reviewed deliverables
- [ ] PO accepts project as complete
...
#### Solo Developer Mode: Combined Gate

For Solo Developer Mode, combine gates at natural breakpoints:

```markdown
# Combined Gate: Specify + Plan Complete
[Include both Specify and Plan gate content]

# Combined Gate: Tasks + Implement Complete  
[Include both Tasks and Implement gate content]
```
...
### 7.1.4 Session State is Transient
...
### 7.3.2 Learning Log File Structure
...
### 7.4.3 Handoff Summary (for complex transitions)
