...
**Note on the operational threshold's time component:** The checklist threshold uses both ≥5 file changes (observable indicator per `meta-method-effort-not-time-estimation` rules-of-procedure §7.12) AND >30-minute runtime as alternative triggers. The runtime component is a research-anchored operational gate (Agent Drift drift-onset = 35 minutes), not an effort estimate. By analogy with rules-of-procedure §7.12.1's existing scope-exclusion category for non-estimate operational values (e.g., timeout values in code), this runtime threshold is an operational gate value, not an effort estimate the AI is producing — it falls outside §7.12 estimation discipline. File-count is the primary trigger; runtime is secondary safety-net. (BACKLOG follow-up: §7.12.1 may be amended to enumerate "research-anchored operational thresholds" as a 5th explicit exception, canonicalizing this carve-out.)
