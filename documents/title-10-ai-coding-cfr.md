...
| Long plan execution (≥5 files OR runtime >30 min OR multi-phase) — drift mitigation | §5.1.8 | Mid-Execution Checkpoint Protocol |
...
**Applies To:** Project start when EITHER (a) §1.3.3 calibrated to ENHANCED mode (novel OR LOW-certainty requirements OR HIGH stakes), OR (b) `query_project("similar implementation in this project")` returns no strong matches (no semantically-related precedent in the codebase regardless of calibrated mode). Whichever fires first. Run before plan-mode, before SPECIFICATION drafting. Skip only when STANDARD/EXPEDITED calibration AND `query_project` surfaces strong-match precedent.

**Why the OR-clause exists:** AI under-calibrates ENHANCED mode — the LEARNING-LOG-documented failure pattern is treating novel work as STANDARD because the AI anchored on "this looks like a known pattern." Brainstorming gated only on ENHANCED therefore fires near-zero in exactly the cases most needing it. The `query_project` precedent check is the structural override: if the project has no semantic match for what's being built, brainstorming runs regardless of calibrated mode. Per `meta-core-discovery-before-commitment` "Question the frame" + the Anchor Bias Mitigation re-evaluation triggers.
...
**Implements:** Informational Readiness (Constitution)
...
**Protocol (run once at midpoint when threshold met):**

1. **Pause** — finish the current atomic task; do NOT pause mid-task.
2. **Re-read the plan file end-to-end** — every section, not just Recommended Approach. Forward-continuation bias makes the AI skip Discovery + Contrarian sections after they've been "read once" earlier in execution; re-read to refresh the frame.
3. **Spot-check delivered-vs-planned for each completed task — produce a written table.** **REQUIRED ARTIFACT:** the spot-check MUST produce a delivered-vs-planned table (markdown table with one row per completed task: planned scope vs. shipped scope vs. drift classification) committed to SESSION-STATE.md, the plan file, or a checkpoint-artifact file. **Bare assertions like "no drift, PROCEED" without a written table = checkpoint did not happen** (this is the "checkpoint theater" anti-pattern named below). The written artifact is the structural gate that prevents step 3 from collapsing to vibes; without it, future sessions cannot verify the checkpoint actually ran.
4. **External-evaluator pass (RECOMMENDED for HIGH-stakes plans, REQUIRED when step 3's table flags ANY drift)** — invoke a fresh-context subagent (validator or code-reviewer) with the original plan + the post-Phase-N delivered artifacts, asking "Does the delivered work match the planned work? Surface any drift." This combats the AI's own anchor on its in-progress narrative. Per [Multi-Agent Reflexion](https://arxiv.org/html/2512.20845v1). When step 3's table shows zero drift, the external-evaluator pass is the structural confirmation that "no drift" is genuine, not anchored.
5. **Decide:** continue per plan / re-plan / abort. Document the decision inline in the plan file or SESSION-STATE alongside the step 3 table.

**When to skip:** Plans below the operational threshold (small fixes, single-file edits, trivial PATCHes) skip checkpointing — the overhead exceeds the drift risk. The threshold is calibrated for cost-vs-benefit; tighter thresholds produce checkpoint fatigue (per [Hick's Law](https://lawsofux.com/hicks-law/)), looser thresholds let drift compound.

**Anti-pattern (the "checkpoint theater"):** Re-reading the plan and declaring "no drift detected" without actually comparing delivered-vs-planned per task. Per `meta-core-discovery-before-commitment` "Discovery Theater" trap. The external-evaluator pass exists specifically to break this anti-pattern; if you're doing your own checkpoint, the spot-check (step 3) MUST produce a delivered-vs-planned table, not a vibes-based "looks fine" assessment.

**Note on the operational threshold's time component:** The checklist threshold uses both ≥5 file changes (observable indicator per `meta-method-effort-not-time-estimation` rules-of-procedure §7.12) AND >30-minute runtime as alternative triggers. The runtime component is a research-anchored operational gate (Agent Drift drift-onset = 35 minutes), not an effort estimate. Per rules-of-procedure §7.12.1 5th exception (canonicalized v3.28.1, 2026-04-25): "Research-anchored operational thresholds — runtime/turn/iteration values derived from empirical research (e.g., Agent Drift onset 35 min per arxiv 2601.04170, circuit-breaker windows, debounce intervals, drift-detection windows) used as process gates rather than effort estimates the AI is producing." This runtime threshold is an externally-anchored operational tunable, not an AI-generated calibration claim — falls outside §7.12 estimation discipline. File-count is the primary trigger; runtime is secondary safety-net.

*Source: Superpowers v5.0.7 `executing-plans` skill (phase-transition checkpointing concept) + Agent Drift research (drift-onset thresholds) + Multi-Agent Reflexion (external-evaluator pattern) + this plan's own MIDPOINT CHECKPOINT (session-126, dogfood: the rule itself was applied to the plan that codifies it).*
...
**Implements:** Part 3.3 (Tool Integration) and Part 7.0 (Cognitive Taxonomy / memory files) for Claude Code CLI

**Applies To:** Claude Code CLI v2.0+; macOS / Linux / Windows

**Information Currency:** Verified 2026-04-26 against Claude Code CLI v2.0+

**Source:** https://docs.claude.com/en/docs/claude-code

**Framework Integration:** Yes — MCP servers, hooks (PreToolUse / PostToolUse / UserPromptSubmit / SessionEnd / Stop), and memory files (CLAUDE.md, AGENTS.md) all supported.
...
**Implements:** Part 3.3 (Tool Integration) and Part 7.0 (memory files) for Gemini CLI
...
**Framework Integration:** Yes — MCP servers supported; hierarchical context loading via `/memory` commands; `GEMINI.md` overlays AGENTS.md.
...
**Implements:** Part 3.3 (Tool Integration) and Part 7.0 (memory / project context) for Claude web interfaces

**Applies To:** claude.ai (Web), Claude Chrome extension, Claude Code (Web at claude.com/code); current Claude model family

**Information Currency:** Verified 2026-04-26 against current Claude web/extension feature set

**Source:** https://claude.ai (web), https://claude.com/code (web coding), Chrome Web Store (extension)

**Framework Integration:** Limited — Project Instructions + Project Knowledge replace `CLAUDE.md` per project; no subagent architecture; no MCP server support in web/extension surfaces. Best for chat-based and browser-automation workflows; deeper governance enforcement requires CLI.
...
**Implements:** Part 5.12 (Stateful System Interaction) and Part 5.8.6 (AI-Generated Database Code) for Postgres / Supabase

**Applies To:** Postgres 13+ (tested through 16); Supabase (current); macOS / Linux / Windows via standard client libraries

**Information Currency:** Verified 2026-04-26 against Postgres 16 and Supabase current

**Source:** https://www.postgresql.org/docs/ + https://supabase.com/docs + https://github.com/supabase/agent-skills

**Framework Integration:** Indirect — patterns surface in AI-generated code via the parameterized-query and migration-safety guidance in §5.8.6. Supabase Agent Skills repository is the comprehensive reference; this appendix is the 80/20.
...
**Implements:** Part 7.0 (Cognitive Taxonomy / project context) for cross-tool instruction files

**Applies To:** AGENTS.md standard (AAIF / Linux Foundation); native support in Cursor, GitHub Copilot, Gemini CLI, OpenAI Codex CLI, Windsurf, Cline, Aider, and 14+ other tools; partial Claude Code support (manual reference from CLAUDE.md)

**Information Currency:** Verified 2026-04-26 against AAIF standard (March 2026 status)

**Source:** https://aaif.dev (standard) and https://github.com/AAIF-Community

**Framework Integration:** Cross-tool — supported natively by 14+ AI coding tools; Claude Code does NOT auto-load AGENTS.md (community workaround: reference from CLAUDE.md, see §A.1).
...
**Constitutional basis:** Informational Readiness (curated reference documents for cross-session continuity; load relevant context before acting).
...
| 2.42.0 | 2026-04-25 | MINOR: New §5.1.8 Mid-Execution Checkpoint Protocol — bridges the gap between pre-action governance gates and post-action coherence audits for long-running plans. Protocol concept (phase-transition checkpoint + external-evaluator pattern) lives normatively in title-10 §5.1.8; tunable threshold values (≥5 file changes OR >30-min runtime) live in `workflows/COMPLETION-CHECKLIST.md` Code-Changes BEST-EFFORT tier per SSOT pattern (principle in rulebook, tunable values in checklist). 5-step protocol (Pause / Re-read plan / Spot-check delivered-vs-planned / External-evaluator pass / Decide). Anti-pattern "checkpoint theater" explicitly named (Discovery Theater per `meta-core-discovery-before-commitment`). Runtime threshold scope-tagged under §7.12.1 as research-anchored operational gate (Agent Drift onset 35 min) rather than effort estimate — file-count is primary trigger, runtime is secondary safety-net. **Root cause:** Pre-action evaluate_governance() and post-action coherence-audit are the two structural gates the framework enforces. Between them, agentic execution drifts (autoregressive forward-continuation bias + scope creep + anchor on original frame after evidence shifts). Pre-this-MINOR, the framework had no normative gate at midpoint — drift compounded silently until post-action review caught it (sometimes too late to roll back atomically). **Constitutional Basis:** `meta-core-discovery-before-commitment` "Periodic re-evaluation (Anchor Bias Mitigation)" — §C.5 already prescribes milestone re-evaluation; this MINOR operationalizes it for plan-execution context with a research-anchored threshold; `meta-core-systemic-thinking` (drift mitigation = structural gate at the natural midpoint, not per-incident cleanup); `meta-method-single-source-of-truth` (concept in rulebook, threshold in checklist — no duplication); `meta-quality-effective-efficient-communication` (rule follows BLUF — Importance / Why-this-exists / Protocol / When-to-skip / Anti-pattern); `meta-method-effort-not-time-estimation` (§7.12 — runtime threshold component scope-tagged under §7.12.1 operational-gate exception, not violated). Source: Superpowers v5.0.7 `executing-plans` skill (phase-transition concept) + [Agent Drift arxiv 2601.04170](https://arxiv.org/abs/2601.04170) (drift onset 73 ± 40 turns / 35 min) + [Multi-Agent Reflexion arxiv 2512.20845](https://arxiv.org/html/2512.20845v1) (external-evaluator pattern) + this plan's own session-126 MIDPOINT CHECKPOINT (dogfood: the rule's first invocation was on the plan that codifies it). ai-instructions PATCH bump v2.8.3 → v2.8.4 (pin update). Body-header + effective-date synced. Plan: `~/.claude/plans/federated-plotting-karp.md` Commit 7 of 8. Governance: `gov-b95156c01466`. Coherence-auditor invocation REQUIRED before push per §5.1.7 trigger table. |
