...
3. **Spot-check delivered-vs-planned for each completed task** — list what shipped, compare against the plan's Files Modified Summary + per-task Verification line. Identify any drift (scope expansion, deferral, fold-in beyond plan) and document the rationale (or recognize it as drift to roll back).
4. **External-evaluator pass (optional but recommended for HIGH-stakes plans)** — invoke a fresh-context subagent (validator or code-reviewer) with the original plan + the post-Phase-N delivered artifacts, asking "Does the delivered work match the planned work? Surface any drift." This combats the AI's own anchor on its in-progress narrative. Per [Multi-Agent Reflexion](https://arxiv.org/html/2512.20845v1).
5. **Decide:** continue per plan / re-plan / abort. Document the decision inline in the plan file or session state.
