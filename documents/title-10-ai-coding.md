...
This table maps each domain principle (and supporting methods) to its Constitutional basis and evidence foundation:

| Domain Principle / Method | Failure Mode Cluster | Constitutional Basis | Primary Truth Sources |
|------------------|---------------------|---------------------|----------------------|
...
| **Comprehension Scaffold** (method, not principle) | Rate inversion: AI generates faster than humans audit → comprehension debt | Effective & Efficient Outputs (comprehension scaffold obligation, rules-of-procedure §16.8), Human-AI Authority & Accountability, Visible Reasoning & Traceability | Task completion scaffolds, generation chunk size thresholds, Understood:Acknowledged ratio tracking |
...
- **Task Sizing Assessment (Before Starting Implementation):**
  ...
  2. If >15 files OR plan exceeds D2 (new tool/hook/section, moderate research, plan mode required) per `meta-method-effort-not-time-estimation`: STOP and decompose further
...
|---------|------|---------|
| v2.9.0 | 2026-05-17 | **MINOR: Comprehension Scaffold crosswalk entry.** Added crosswalk row for Comprehension Scaffold (method, not principle) mapping to E&E Outputs + Human-AI Authority & Accountability + Visible Reasoning & Traceability. Table header broadened from "Domain Principle" to "Domain Principle / Method" to accommodate method-level entries. Governance: `gov-aa596dedcd00`. |
...
**Database & Stateful Systems Research:**
- Supabase "Postgres Best Practices for AI Agents" (2025): 30 rules covering data types, indexing, query patterns, migrations, and RLS — derived from observing AI-generated database code failures at scale
- OFFSET pagination degradation is O(n) — cursor/keyset pagination maintains constant performance regardless of dataset depth (Postgres documentation, Markus Winand "Use The Index, Luke")
- N+1 query patterns are the most common ORM-related performance issue; AI assistants reproduce them because training data contains loop-based data access patterns
- Connection exhaustion from per-request connection creation is a leading cause of production database outages; pool sizing is critical (PgBouncer documentation, Supabase connection pooling guidance)
