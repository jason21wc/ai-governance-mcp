...
- `tests/test_config_launch_integration.py` → `test_content_lookup_does_not_mask_degraded_startup`
...
### `FM-HYGIENE-KEEP-REF-IDENTITY`

> Worktree keep markers must match collected branch identity, including namespaced and origin-prefixed spellings, in both alarm suppression and stale-marker detection. Legacy short names remain supported; an exact namespaced marker must not suppress a different branch sharing its leaf name.

- `tests/test_repo_hygiene.py` → `test_worktree_keep_for_unknown_namespace_remains_stale`
- `tests/test_repo_hygiene.py` → `test_worktree_keep_matches_real_branch_without_stale_marker`
- `tests/test_repo_hygiene.py` → `test_worktree_keep_preserves_branch_namespace_identity`

### `FM-HYGIENE-LANDEDNESS-STALE-BASE`

> Worktree landedness must not be asserted from a local default branch that is behind its remote. When refs/heads/<default> and refs/remotes/origin/<default> disagree, the report must say the work landed on origin and name the lag rather than claiming it never landed, must still withhold the removal command because the two bases disagree, and must report the stale default as its own advisory finding. Absent or -1 origin facts degrade to the pre-existing conservative behaviour.

- `tests/test_repo_hygiene.py` → `test_absent_origin_facts_behave_exactly_as_before`
- `tests/test_repo_hygiene.py` → `test_default_ref_lag_is_measured_against_the_real_remote`
- `tests/test_repo_hygiene.py` → `test_landed_on_origin_is_not_reported_as_NEVER_LANDED`
- `tests/test_repo_hygiene.py` → `test_stale_default_ref_is_reported_and_offers_ff_only_from_the_primary`
- `tests/test_repo_hygiene.py` → `test_unlanded_against_BOTH_refs_still_reads_as_never_landed`
...
### `FM-DOC-POINTER-OWNER-SCOPE`

> An absent pointer-source document exempts only that owner's expected pointers. Every surviving owner must still declare its expected pointers, including pointers shared with another document. Ownership must be independent of current discovery so deletion cannot erase its own obligation.

- `tests/test_doc_pointer_resolution.py` → `test_stale_check_catches_pointer_removed_from_one_shared_owner`
- `tests/test_doc_pointer_resolution.py` → `test_stale_check_catches_removal_in_complete_tree`
- `tests/test_doc_pointer_resolution.py` → `test_stale_check_catches_surviving_owner_after_source_omission`
