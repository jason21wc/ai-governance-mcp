...
Security is the highest-value cross-vendor surface — a different model lineage flags different
attack surfaces (independent reviewers give uncorrelated errors,
`multi-architecture-context-isolation-architecture`; validated session-238, where a Codex peer
found a real env-exfil path a Claude review down-weighted). For a pre-release or high-stakes audit, the INVOKING agent may run an
independent Codex (gpt) peer in parallel — at the ORCHESTRATOR level, not inside this subagent:
`python scripts/codex_review.py --role security …` (ai-governance repo) or `codex exec --sandbox
read-only < /dev/null` with this audit's instructions + the code. Fold its output as advisory
candidate findings through the same evidence gate + severity calibration; two lineages agreeing
is higher-confidence, a Codex-only finding is a candidate to weigh. It self-skips when `codex`
is absent.
...
| 10 | **Database/RLS security** | Supabase, Postgres | §5.3.6, §5.8.6, Appendix I.6 (RLS policies incl. tenant-context transport, JWT claim verification, connection pooling) |
| 11 | **Container security** | Docker, K8s | §5.8.6 (running as root, exposed secrets in images, privileged mode) |
| 12 | **MCP/LLM tool security** | MCP servers, AI tools | §5.6.5, §5.6.9, §9.3 (tool poisoning, tool-call injection, over-permissive tool definitions, prompt injection via tool results, publisher integrity for projects that ship MCP servers) |
