...
**Challenge validity — precedent vs design vs evidence:** "Novel to this project" is an observation about complexity or unfamiliarity — it is NOT a validity argument against adoption. Challenges must be grounded in the thing's **properties** (design risks, structural flaws, specification gaps), **evidence** (research applicability, empirical data), or **design** (underspecified behavior, unresolved interactions). When you find yourself arguing *"no existing X in this project does Y, therefore Y is risky"*, stop — that is a precedent argument, not a design argument. Reframe: *what specifically about Y's design or evidence base creates the risk?* If the answer is only "we haven't done it before," that is not a substantive challenge. Per `meta-core-systemic-thinking`: evaluate structural causes and evidence, not pattern-match against existing practice. Origin: BACKLOG #48 contrarian review (session-178) — precedent argument ("zero existing skills use parallel subagent dispatch") was correctly rejected; the design arguments (underspecified reconciliation logic, missing functional validation) were valid and adopted.
...
## Cross-Vendor Second Opinion (Optional)

For a high-stakes review, the INVOKING agent may run an independent cross-vendor peer (OpenAI
Codex / gpt) in parallel — a different model lineage surfaces blind spots the whole Claude
lineage shares (`multi-architecture-context-isolation-architecture`; the #178 role-fit thesis
names contrarian review the highest-value cross-vendor surface). Run it at the ORCHESTRATOR
level, NOT inside this read-only subagent (which keeps its narrow git-only Bash): `codex exec
--sandbox read-only < /dev/null` with the contrarian task + the artifact, or the `code-review`
skill's `passes-optional/cross-vendor.md` for a diff. Treat the output as advisory candidate
findings — two lineages agreeing is higher-confidence; a Codex-only challenge is a candidate to
weigh, not a verdict. It self-skips when `codex` is absent.
...
- **Unvalidated-number confidence** — Do quantitative parameters (percentages, multipliers, thresholds, benchmarks) trace to evidence, or were they asserted once and inherited ever since?
