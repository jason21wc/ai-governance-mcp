...
|---|---|---|
| Model selection | Choosing which model to use for a task | Which tier for which stakes? Per-task right-sizing? |
...
**CoALA provenance:** Semantic, Episodic, Prospective (core CoALA + extensions), and Procedural (core CoALA — reclassified from Reference Storage per contrarian review; skills are actively authored and session-modified, sharing the mutable/maintained pattern of OPERATIONS.md rather than the read-mostly pattern of reference-library). Subsystem boundary: durable, accumulates via append/update, analogous to HDD/SSD. Swap question: should these be structured data or a database instead of markdown files?
...
| Prospective memory (recurring) | Recurring commitments | `OPERATIONS.md` | Storing projects here → confuses discrete work with ongoing monitoring |
...
|---|---|---|
| Triggered handlers | Event-driven actions | 14 hook scripts in `.claude/hooks/` — 12 registered in `.claude/settings.json`, 2 (dream, journal) global-registered (PreToolUse, PostToolUse, UserPromptSubmit, SessionStart — count SSOT: compliance Check 1, which **derives** it; state the denominator whenever you quote a number here, because "hooks" has meant three different populations in this file). *`PrePush` is not a Claude Code event; `pre-push-quality-gate.sh` is a `PreToolUse`/`Bash` hook that inspects the command.* |
...
|---|---|---|
| Pre-action gates | Block before damage | 5 blocking PreToolUse gates (governance-check, push-quality, OOM, content-security, exit-plan) of the 14 scripts; the other nine (boot, cadence, dream, genesis, hygiene, journal, governance-inject, post-push-ci-check, memory-size-guard) are non-blocking; `pre-commit-memory-size-guard.sh` reports all three size checks for quality review, including LEARNING-LOG's 200-total-line target, without blocking commits |
...
The hooks in `.claude/hooks/` fire on specific events (PreToolUse, PostToolUse, UserPromptSubmit, SessionStart) and can halt execution. That's an **interrupt controller (PIC/APIC)** — hardware interrupts that preempt normal execution. Some are maskable (e.g., `CONTENT_SECURITY_SKIP=1`), some are effectively non-maskable (governance check has no practical bypass in normal operation).

Current hook inventory (14 scripts in `.claude/hooks/` — 12 project-registered + 2 repo-canonical/global-registered — + Rampart tripwired; count SSOT: compliance Check 1, derived. Compliance Review #18 derived 14 and found the expected list itself stale at 13):
- `pre-tool-governance-check.sh` — governance consultation enforcement
- `session-start-genesis.sh` — new-project founding-question genesis (session-217, BACKLOG #69; registered project + user level, self-clearing)
...
- `session-start-cadence.sh` — SessionStart cadence surfacer (C-078/C-155/C-109 due-date nudges, session-195, §7.2)
- `scan_transcript.py` — shared transcript scanning utility (not a registered hook; called by other hooks)
- `journal-reminder.sh` (**repo-canonical** `.claude/hooks/`, **symlinked** into `~/.claude/hooks/` and `~/.codex/hooks/`, registered user-level-only; session-241) — memory maintenance journaling trigger + fire log. Fires for all projects with memory files. See CFR §7.11.
- `session-start-dream.sh` (**repo-canonical** `.claude/hooks/`, **symlinked** into `~/.claude/hooks/` and `~/.codex/hooks/`, registered user-level-only) — SessionStart dream cadence: auto-run directive at the activity threshold (`DREAM_AUTORUN=0` → nudge) + fire log; fires for all projects with memory files. See §7.2.
...
- **Skills** = application programs (Word, Photoshop). Invoked by user, run in main context, full memory access. Use for repeatable workflows and on-demand reference material. Loading a skill executes nothing; when current evidence is needed, its instructions direct ordinary read-only tool calls after load.
...
**Current status:** **22 unique skills — 4 project-local + 18 global, with no name shared between them.** *(This sentence previously read "15 unique skills (4 project-local + 12 global; `/content-enhancer` exists in both)"; all three figures were stale and the overlap claim was false — `/content-enhancer` is global-only. The generated count in `STATUS.md` is the SSOT and already read 22; prefer it over any list here, which rots.)* Project-local (`.claude/skills/`): `/compliance-review` (13-check governance compliance review), `/completion-sequence-aigov` (post-change verification gate), `/test-authoring` (9-step test creation protocol), `/verify-handoff` (handoff verification). Global (cross-project, `global-skills/` → `~/.claude/skills/`): `/all-clear`, `/architecture-review`, `/code-review`, `/completion-sequence`, `/content-enhancer`, `/doc-gen`, `/doe-calibration`, `/dream` (cross-session transcript mining with HITL review, §7.11), `/eval-suite` (eval-driven development for probabilistic/LLM outputs, CFR §5.2.8), `/journal` (memory maintenance — session transcript analysis for unpersisted items, §7.11), `/model-routing`, `/pmo`, `/refactor-audit`, `/security-scan`, `/source-review` (intent-first review of an external source against governance, rules-of-procedure §9.8.5), `/start-project`, `/start-worktree`, `/test-suite`. Each follows the self-contained folder pattern: thin SKILL.md orchestration shell (~80-100 lines) + reference files for procedure content and mutable data. Authoring standards: CFR Part 9.5.

**Decision matrix — when to use which mechanism:**

| Need | Mechanism | Why |
|------|-----------|-----|
| Repeatable on-demand workflow with current context | **Skill** | Loads instructions into main context, then gathers live data through ordinary tool calls; user controls timing |
...
|---|---|---|---|
| **CPU** | LLM (the processor) | Claude frontier tier via Anthropic API — version resolved from the session/settings, not pinned here (rules-of-procedure §G.1) | Host harness API. Swappable if drivers (appendices) updated. |
| **OS Kernel** | Core governance framework | `documents/constitution.md` + `documents/rules-of-procedure.md` | 7-layer hierarchy, supremacy clause, derivation chain. **Coupled by design** — tight integration is architectural coherence, not a defect. |
...
| **Workload modules** | Domain principles | `documents/title-NN-*.md` (domains discovered from frontmatter) | Derives from kernel. Modular — add/remove without kernel changes. |
| **Drivers** | Domain regulations (CFR) | `documents/title-NN-*-cfr.md` (one per domain) | Translates OS rules to domain-specific ops. Paired with module. |
...
| **Runbook / crontab** | Prospective memory, recurring subtype (CFR §7.0.2) | `OPERATIONS.md` | **Implicit contract.** Interface contracts documented in §6.2. |
...
| **Application programs** | Skills | `.claude/skills/` (4: compliance-review, completion-sequence-aigov, test-authoring, verify-handoff) | User-invocable instruction sequences. Self-contained folders. Decision matrix in §3.7. |
...
| Computer Component | Project Equivalent | Current Implementation | Interface Type |
|---|---|---|---|
| **Interrupt controller (PIC/APIC)** | Hook system | `.claude/hooks/` (14 scripts + Rampart tripwired) | Event → exit code. Modular — each standalone. |
...
|---|---|---|---|---|
| **Working** | `SESSION-STATE.md` | read, write (overwrite), prune (§7.0.4 distillation) | Markdown. "Current Position" + "Immediate Context" + "Next Actions" sections; snapshot only, no per-session log. <300 lines target. | Session-start protocol, session-end update |
...
| **Prospective** (recurring) | `OPERATIONS.md` | read, append (new items), update (audit logs, status) | Markdown. 7 sections: Cadences, Tripwires, Verification Experiments, Metrics, Standing Authorizations, Rollback Watches, Scheduled Ops. C-/T-/V-/M-/SA-/RW-/SO- prefixes. | Compliance review, cadence triggers, tripwire evaluation |
...
#### 6.4 Definition — "memory files" (term scope)

**"Memory files"** names this framework's **governance-managed, git-tracked, user-editable state files** — the working / persistent / operational set above (§6.1–6.2): `SESSION-STATE`, `PROJECT-MEMORY`, `LEARNING-LOG`, `BACKLOG`, `OPERATIONS` (plus the procedural `.claude/skills/*`). On-disk location: `_ai-context/` at the project top level for all project types (unified layout per CFR §7.8.2, v2.62.0; pre-v2.62.0 root layouts grandfathered). The user-facing `CLAUDE.md` is the **loader / instructions** file that points at them — governance-managed and user-editable (in scope as "ours"), but instructions, not cognitive state. `ARCHITECTURE.md`, `README.md`, and `reference-library/` (§6.3) are framework **reference / charter** docs — also "ours," but classified as reference, not memory.

**Out of scope — hands off:** the host LLM's own **native memory** (Claude Code's `~/.claude/projects/*/memory/`, ChatGPT memory, Cursor/Windsurf equivalents) is tool-managed, not user-editable, and not governed by this framework. Don't read, write, or manage it. The binding rule is `rules-of-procedure §G.5` (Platform-Native Memory — Hands Off); the decision record is `PROJECT-MEMORY` ADR-10. This boundary is what keeps the framework **host-portable** — it runs identically on Claude Code / Cursor / ChatGPT / Cowork because it never reaches into any host's native memory.

The operative distinction is **ours vs. the host's**. Within "ours," the cognitive-type labels (Working / Persistent / Reference / Procedural) sub-classify the set; they do not gate the host boundary.

---

## 7. Context Retention Priority Policy

When the host (Claude Code) compresses prior messages to stay within context limits, not all memory content has equal priority. This policy defines which information should be preserved longest and which can be evicted earliest, guiding both automatic host compaction and manual SESSION-STATE pruning (§7.0.4 per AGENTS.md "On Session Start" protocol).
...
| P3 | Summarize when space-constrained | Completed task details, historical audit IDs | Distill to one-line pointers during §7.0.4 pruning. Full content recoverable via `git log`. |
| P4 | Archive (evict first) | LEARNING-LOG entries >60 days, closed BACKLOG item details, EXECUTION-FRAMEWORK design history sections | Remove from active memory files. Recoverable from git history. |
...
- **SESSION-STATE pruning (§7.0.4):** Manual distillation when file exceeds 300 lines. Route decisions to PROJECT-MEMORY (P1), lessons to LEARNING-LOG (P1), route content to its owning file (P3→P4).
- **Memory-file routing:** AGENTS.md "On Session Start" protocol directs which files to load at session start (P0-P1) vs. consult on demand (P2-P3).

**What this policy does NOT do:** It does not control Claude Code's internal compaction algorithm (that's host-level, not user-configurable). It provides a shared vocabulary for prioritization decisions when humans or AI agents prune memory files, and a reference for the SessionStart cadence surfacer (§7.2).

### 7.2 Session-boundary automation (updated session-195, 2026-05-29)

**Corrected premise.** Earlier versions of this section asserted *"Claude Code has no `on-session-end` hook event; session-end detection cannot be automated."* **That is no longer true** (verified against live docs 2026-05-29): Claude Code exposes `SessionStart`, `SessionEnd`, and `Stop` hook events. The stale claim had foreclosed session-boundary automation entirely — a point-in-time platform fact baked into doctrine behind a "when this changes" note that nobody re-checked (see LEARNING-LOG 2026-05-29; tripwire T-164).

**What each hook can and cannot do:**
- **`SessionStart`** — fires on startup/resume/clear/compact (a `source` field distinguishes them) and **can inject `additionalContext`**. The **reliable workhorse**: project cadences are date-checked (`Next due:`) and injected as a one-line nudge when overdue (`.claude/hooks/session-start-cadence.sh` — C-078/C-155/C-109); the **dream** cadence is activity-counted and injects an **AUTO-RUN directive** at its threshold (`.claude/hooks/session-start-dream.sh`, repo-canonical, symlinked into `~/.claude/hooks/`; `DREAM_AUTORUN=0` → nudge; session-241).
- **`SessionEnd`** — graceful-exit-only (does NOT fire on Ctrl-C-twice, window-close, or SIGKILL), runs **async after** termination, and **cannot inject context or invoke a skill**. **Unsuitable** for triggering memory work: by the time it fires, no agent remains to run anything. Deliberately **not used**.

**Honest scope — trigger solved; execution split by cadence class (session-241).** These hooks make the *check structural* (it happens deterministically every session start, closing the "did I remember to read OPERATIONS.md?" gap). Acting now splits: for the **dream cadence** the hook injects an **auto-run directive** (execute the dream analysis now, background, HITL on apply/commit; `DREAM_AUTORUN=0` reverts) — but be precise about what that is: the directive's *execution remains advisory* (injected text the model can ignore — the same structural class as a nudge, stronger wording), which is why both memory-maintenance hooks now write **fire logs** and the compliance review runs a **fired-vs-ran sub-check** (fires vs subsequent `/dream pass` commits / memory writes; V-010 converted to this live instrument; a blocking-gate escalation is pre-named at <~80% fired→ran over ≥10 fires). **Judgment-bearing cadences** (C-078 compliance, C-155 feedback-loop, C-109) stay surfaced-advisory — a hard push there would block unrelated work for reviews that need human framing. Autonomous *execution* of judgment-bearing reviews is still host/local-MCP-bound: cloud routines cannot call this project's local MCP servers. The local-MCP blockers in OPERATIONS SO-001/SO-002 remain true; only SO-003's "session-end detection is impossible" blocker is resolved.

**Compensating controls (still valid):** session-start pruning (AGENTS.md "On Session Start" + boot hook); Session Closer Protocol (`multi-method-session-closer-protocol`, manually invoked); context retention priorities (§7.1). The SessionStart surfacer now augments these by auto-surfacing due cadences. See OPERATIONS.md SO-003/SO-004 for operational tracking and §8.4 for the enforcement-layer rows.
...
2. AGENTS.md (Appendix K of title-10) is a cross-tool standard read natively by 14+ tools (Codex, Cursor, …) — POSIX-equivalent for AI tooling; Claude Code and Gemini reach it by importing it from their own loader.
...
| Sub-component | Method level — *how to do it generically* (multiple valid approaches) | "Use 6 cognitive memory types" |
...
- **Subsystem 1 (Inference Engine) caveat withdrawn.** It IS a peer subsystem. Principle: "Match model capability to task stakes." Method: "Right-size by task type." Appendix: "frontier tier for D2+ in Claude Code" — named by tier, because an authoring exemplar that pins a version teaches the defect `rules-of-procedure` §10.1.4 exists to prevent.
...
The governance system enforces through multiple layers that vary by client environment and reliability. Understanding which layers are available in each context is prerequisite to closing enforcement gaps. This is the comprehensive canonical reference; the simplified enforcement-layers table in title-10 CFR §9.3.10 (Layered Enforcement Stack) should reference this section.
...
| Enforcement proxy — act-intrinsic gate (`--enforce-acts`) | Hard-denies a call whose own arguments carry a credential path or secret value | Yes | Yes | Yes | No | No |

**The enforcement proxy has a second tier that does NOT belong in this table.** Only the act-intrinsic row above meets this section's standard, because it reads the call's real arguments. The proxy's *recency* gate — "was `evaluate_governance` called lately" — is **model-satisfiable**: the model can satisfy it itself by calling governance, and nothing consumes the resulting verdict. It gates call order, not authorization, so it is advisory (next section) no matter which mode it runs in. It sat in this table under the heading "the model cannot bypass them through prompting" until 2026-09-02, next to five hooks that genuinely hard-block, and two sibling projects inherited the stronger reading. See ADR-38; the naming rule that should have caught it is `rules-of-procedure` §9.8.1 Q7.

> **Cross-CLI note (2026-07-03, T-161):** the PreToolUse hook mechanism is not Claude-Code-exclusive. OpenAI Codex CLI 0.142.5 ships an equivalent `PreToolUse` deny system, and this framework runs an **act-intrinsic content-security hook** there (`python -m ai_governance_mcp.codex_hooks`, reusing the `safety_scan` core) — LIVE-FIRE VERIFIED blocking a secret value in **both** interactive and `codex exec` (headless) modes; hooks fire in `codex exec` where Codex's MCP tools do not. Scope is act-intrinsic *security* only (a secret/credential in the tool call's own arguments); the **behavioral** governance-recency gate (the *first* row above — "blocks Bash/Edit/Write until governance+CE called"; NOT row 2, the content-security hook, which is exactly what is ported to Codex) remains Claude-Code-only pending BACKLOG #176b (feasible — the Codex payload carries a populated `transcript_path`). Codex also runs the per-turn FRAME `UserPromptSubmit` **steering** sibling (`ai_governance_mcp.codex_frame`, FRAME SSOT shared with the Claude Code hook) — live-fire verified 2026-07-05 (940/940 injections); steering only, never blocks. Detail: title-10 Appendix N.5.
...
| Layer | Mechanism | Claude Code CLI | Claude App | Other MCP Clients | RAG-only | Degrades Over Conversation? |
|-------|-----------|:-:|:-:|:-:|:-:|:-:|
| tiers.json behavioral_floor | The behavioral floor; delivery layers are enumerated in `rules-of-procedure` §7.15, which owns that list (do not restate it here — three surfaces once disagreed). Directive count is derived, not pinned: `len(behavioral_floor.directives)` | Yes | No | No | No | Yes |
...
| Skills (4 project-local skills) | Structured procedures invoked by slash command | Yes | No | No | No | No |
...
| Layer | Mechanism | Claude Code CLI | Claude App | Other MCP Clients | RAG-only | Degrades Over Conversation? |
|-------|-----------|:-:|:-:|:-:|:-:|:-:|
| UserPromptSubmit hook | Injects reasoning FRAME every prompt (turn-start re-anchor) + governance reminder if tools not called. Codex CLI runs the FRAME-only sibling `ai_governance_mcp.codex_frame` (live-fire verified 2026-07-05; title-10 N.5) | Yes | No | No | No | No |
| SessionStart cadence surfacer | Date-checks cadences at session start; injects due-cadence nudges + the dream AUTO-RUN directive at its activity threshold (§7.2) | Yes | No | No | No | No |
...
|--------|:-:|:-:|:-:|:-:|
| **Claude Code CLI** | All 6 layers | All 4 layers | All 4 layers | All 3 layers |
| **Claude App** | Path-dependent; local-proxy reachability pending T-164/#309 | SERVER_INSTRUCTIONS only | All 4 layers | None |
| **Other MCP clients** (Cursor, Gemini CLI, etc.) | Proxy act-intrinsic gate only (the recency gate there is advisory) | SERVER_INSTRUCTIONS only | All 4 layers | None |
| **RAG-only** (no MCP) | None | Document content only | None | None |

> **Reachability — "Claude App" is transport-specific.** A remote custom connector (hosted URL) is cloud-origin, works across supported clients, and bypasses the local proxy. Classic Desktop can use local stdio `mcpServers`, but current Anthropic documentation separates Desktop-configured local servers from Cowork while Cowork also has plugin/local-session MCP paths. That conflicts with the 2026-06-28 user observation recorded in PROJECT-MEMORY. Until T-164/#309 reproduces each route independently, do **not** claim that Cowork or Dispatch traverses `ai-governance-proxy` or receives its act-intrinsic gate. Browser claude.ai has no Desktop local-stdio path. Structural behavioral enforcement (hooks) remains Claude Code only; every Claude App path is advisory for behavioral compliance.
...
| Enforcement proxy | Guide | Protocol-level governance wrapper — intercepts before tool execution. Two tiers: the act-intrinsic content gate is structural; the recency gate is model-satisfiable (ADR-38) |
| CLAUDE.md disposition kernel + FRAME inject | Guide | Thin reasoning-posture kernel loaded at session start; per-response FRAME inject reinforces behavioral floor |
...
#### Observation-locus / action-locus coincidence (orthogonal to Guide/Sensor)

Guide/Sensor classifies enforcement by *timing* (before vs. after the action). A second, orthogonal property classifies it by **where the deterministic layer's observation locus sits relative to the action it governs** — which determines what can structurally go wrong with it.

- **Trace-verification (decoupled locus):** the layer checks for *evidence that the actor performed a required behavior*, read from a source separate from the action itself. Our PreToolUse governance hook is the canonical case — it scans the session transcript (the observation locus) for `evaluate_governance` + `query_project` calls, then gates the tool call (the action locus). The two loci are decoupled.
- **Enactment (coincident locus):** the layer *performs the governed computation itself* on the action it is gating — observation locus and action locus coincide. The content-security hook is already this for credential paths (it inspects the command it blocks, not a trace of the model having considered it). Claude Code's `mcp_tool` hook handler newly makes this available for the *governance evaluation* specifically: a PreToolUse hook could call a governance tool directly on the pending `tool_input` rather than verifying the model called it.

These cross with Guide/Sensor independently (a 2×2): the governance hook is Guide×verify; `verify_governance_compliance` is Sensor×verify; the content-security hook is Guide×enact; a PostToolUse auto-remediation would be Sensor×enact. All four cells populate with distinct semantics, so the property is genuinely orthogonal — not a rename of Guide.

**Work-product analogue (pointer, not an extension of this axis).** The locus distinction has an analogue beyond *enforcement*: an agent verifying its *own* output can observe the real medium (the rendered DOM, a screenshot, the real CI result) or a proxy of it (its reading of its own code). The documented practices — `title-10-ai-coding-cfr` App. E.3 (*Claude in Chrome*), `title-15-ui-ux-cfr` §8.3 (Screenshot-Based Visual QA), the push→CI→fix loop — are real-medium verification, the work-product form of `meta-quality-verification-validation`; it *extends* the retrieval-scoped LEARNING-LOG 2026-05-30 lesson ("verify *retrieval* behavior, not corpus *artifact* integrity") to general work product. Two cautions keep this a pointer rather than a merged axis: (1) here "locus" is an *epistemic stance* (did you look at the real thing?), not the *mechanical observation-source* of a deterministic layer above — same word, different referent; (2) it does **not** inherit the "enactment is conditional" trade below, which is specific to *reasoned governance* forfeiting the model-reads-principles steering by-product (verbose-on-success) — work-product correctness has no such by-product, so real-medium verification is preferred without qualification. The actionable directive lives in those coding/UI titles, not here. (Origin: 15-Claude-Code-rules gap analysis, session-206 — the rules were already covered by E.3/§8.3/CI; this axis is the structural *why*.)

**Why the locus matters — and why this is not a value ranking.** Decoupled-locus layers are *susceptible to locus mismatch*: when the observation source does not contain the behavior, the layer mis-fires regardless of how correct its logic is. **T-152 is exactly this** — subagent governance calls land in a transcript the parent hook cannot read, so compliant subagents are false-denied. This is the root cause already named in the 2026-05-07 LEARNING-LOG ("a correct enforcement algorithm applied to the wrong data produces false negatives that look like algorithm bugs but are data-routing bugs"), promoted here to a structural variable — *not* a defect of verification as a modality. It has two distinct fixes: **(a) route the right locus** to the verifier (upstream `agentId`/subagent-transcript exposure — preserves verification), or **(b) force locus coincidence** via enactment. Either dissolves T-152; the modality is incidental. (Diagnostic: if Claude Code shipped `agentId` tomorrow, T-152 would die with verification fully intact — which is why "verification is the root cause" is the wrong frame and "locus mismatch" is the right one.)

**The trade — enactment is conditional, not superior.** An enactment gate does not have "nothing to observe": it trusts the hook payload (`tool_input` — the *mechanical* act, stripped of the model's articulated intent) and harness fidelity. It buys compliance-independence at the cost of intent-visibility. So enactment is sound only for **act-intrinsic governance** — properties decidable from the action alone (credential paths, dangerous commands, path allowlists). For **reasoned governance** (proportionality, root-cause-vs-symptom, alternative-generation) the verification path is *preferred*: its by-product — the model reads the surfaced principles and adjusts subsequent generation (the deliberate "verbose-on-success" design below) — is a steering effect enactment structurally cannot provide. Enactment blocks the act; it does not improve the next token. Per the guide-over-sensor rationale above, that by-product is the higher value where the governance is reasoned rather than act-intrinsic.

Application status: an enactment gate for the act-intrinsic subset is a candidate (BACKLOG), gated behind a derisking spike and the maturity of `mcp_tool`/`agent` hooks (experimental, Claude-Code-only). See OPERATIONS T-152.
...
#### Delivery-cadence axis (which always-on principle reaches the model how)

A third classification, orthogonal to Guide/Sensor (timing) and observation-locus (where the deterministic layer observes): **by what *moment* an always-on principle reaches the model.** Five delivery methods, each fitting a different principle nature:

| Method | Fires | Best for |
|--------|-------|----------|
| **Boot-load** (CLAUDE.md/AGENTS.md) | Session start; decays over the conversation | Foundational reference + *conditional* principles (consulted when their trigger arises) |
| **Per-prompt inject** (UserPromptSubmit hook) | Every prompt, turn-start; highest recency | The small *always-applicable reasoning frame* whose absence is the recurring failure |
| **Per-action inject** (`evaluate_governance` response) | At each governance evaluation — the action locus | *Action-gating* checks + the reasoned-governance scaffolds (the verbose-on-success steering above) |
| **Structural gate** (PreToolUse hooks) | On the gated tool call; deterministic | *Hard constraints* that must never be skipped |
| **On-demand retrieve** (domain floors, `query_governance`) | When relevance pulls it | *Situational / domain* principles |

**Assignment rule:** map each always-on principle to its single best-fit method by nature — **single-home default**. Multi-*moment* delivery is allowed only with a stated argument: justified when a principle decays at one locus and must be re-anchored at another **and the encodings differ in form** (so it is spaced reinforcement, not an identical repeated banner that invites blindness). **Worked example** — the reasoning frame (root-cause, verify, recommend-not-ask, proportional-rigor, transparent-limits) is deliberately delivered at three moments in three forms: boot-load *prose* (set-once, decays), the gov-call critical_5 *interrogative scaffolds* (steer at the action locus — the verbose-on-success by-product), and the UserPromptSubmit *terse-pointer* frame (turn-start re-anchor, filling the gap before the next gov-call). The single-home default still governs everything else; this is the documented exception, not a license to triplicate. Cost is watched (T-163 + §9 token observability); efficacy by the manual-frame-append rate (OPERATIONS V-011).

The per-principle assignment is *enacted* in the delivery configs (`documents/tiers.json`, the hooks, CLAUDE.md) — the single source of truth — not duplicated as a table here.

#### Gap analysis

The structural cause of the Claude App enforcement gap: these 5 PreToolUse hooks (governance gate, content security, pre-push quality, plan-mode gate, OOM gate) are a Claude Code CLI feature and do not run in the Claude *App*; the enforcement proxy's act-intrinsic gate is the only structural mechanism available to it — and only that tier, since the proxy's recency gate is model-satisfiable (ADR-38). (Note — hooks are **not** Claude-Code-exclusive across *platforms*: Codex CLI 0.142.5 ships an equivalent `PreToolUse` system, and this framework now runs an act-intrinsic content-security hook there, LIVE-FIRE VERIFIED 2026-07-03 in both interactive and `codex exec`; title-10 N.5 / #176a. Only the *behavioral* governance-recency hooks remain Claude-Code-only.) Without it, governance is purely advisory — SERVER_INSTRUCTIONS degrade over conversation length, and per-response reinforcement only fires when the model calls governance tools (which nothing structural compels it to do).
...
| **Resource monitor / power management** | Tracks resource consumption, provides telemetry | No token/API/context utilization tracking. OOM gate prevents catastrophe but no normal-operation observability. | **Genuine gap.** BACKLOG #58 adjacent but scoped to re-injection, not telemetry. **First concrete instance + method** (15-rules gap analysis #13/#14, session-206): the verbose-on-success per-PROCEED cost (§8.4, ~200-500 tok) + skill-load and universal-floor size are objectively measurable per session — token observability finds waste and catches *silent* behavioral regressions (e.g. a plugin routing work to the main thread instead of subagents → token spike). Bridges §8.4 verbose-on-success + T-163 (instruction density). This is governance *cost* telemetry — distinct from governance *effectiveness* (#48 *Objective Measurement*) and corpus integrity (#48-C). Still unbuilt. |
| **Scheduler / clock** | Automated time-based operations | Cadences tracked in OPERATIONS.md. 4 scheduled operations defined (SO-001–SO-004), incl. a SessionStart cadence surfacer (session-195). CronCreate session-only (7-day expiry); cloud routines lack local MCP access. | **Trigger closed (session-195):** SessionStart date-checks cadences and surfaces due ones (§7.2). Autonomous *execution* of 10+ day cadences still blocked by mechanism constraints. |
| **Virtual memory manager** | Page replacement policy, eviction priority | Host handles compaction; no framework say in eviction. Context retention policy defined (§7.1). Session-boundary automation resolved (§7.2): SessionStart surfacer injects cadence nudges + the dream auto-run directive; SessionEnd unsuitable (post-termination). | **Addressed** by §7 + SO-004. Autonomous execution still host-bound. |
...
| **Scheduling mechanism assessment:** CronCreate is session-local (7-day expiry); cloud routines lack local MCP access. Neither fully automates 10+ day cadences. | Assessed (2026-05-03, session-145). Documented in §7.2 and OPERATIONS.md SO-001–SO-004. Triggering since closed via the SessionStart surfacer (session-195). |
| **Session-end automation deferred** — no on-session-end hook exists. Session-start pruning is the compensating control. | **Resolved (session-195):** premise was stale — SessionStart/SessionEnd/Stop hooks exist. SessionEnd is post-termination (unsuitable); the SessionStart surfacer closes the trigger. See §7.2, OPERATIONS SO-004, tripwire T-164. |
| **Session-boundary automation premise corrected** — the "no on-session-end hook" claim baked into §7.2/SO-003 was found stale; SessionStart cadence surfacer added (project + global), SessionEnd rejected (post-termination), stale-premise doc surfaces corrected, T-164 tripwire created. | Adopted (2026-05-29, session-195) |
...
| 2026-05-29 | v2.1.0 | Session-boundary automation: corrected the stale "no on-session-end hook" premise (§7.2 rewrite — SessionStart/SessionEnd/Stop verified to exist). Documented the SessionStart cadence surfacer; §8.4 Enforcement Layer Matrix +1 row + client-summary update (T-161); §9 Gap Matrix (Scheduler + VM-manager rows) and §10 Decision Log updated (session-end resolved + correction row). Pairs with CFR 2.49.0 + OPERATIONS SO-004/T-164. | 195 |
| 2026-06-01 | v2.2.0 | §8.4: added "Observation-locus / action-locus coincidence" subsection — a second enforcement axis orthogonal to Böckeler Guide/Sensor (timing). Distinguishes trace-verification (decoupled locus, susceptible to locus mismatch — the structural cause of T-152) from enactment (coincident locus, sound only for act-intrinsic governance). Reframes T-152 (two fixes, not "wait for upstream") and feeds BACKLOG #57. Origin: RTK reference-brief re-review; contrarian-reviewed (locus reframe replaced an initial "verification-is-root-cause" framing). Pairs with title-20 §4.6.3 v2.19.0. | 205 |
| 2026-06-02 | v2.2.1 | §8.4: added a **work-product-verification pointer** to the locus axis (real-medium vs proxy self-verification — browser/screenshot/CI; the work-product form of `meta-quality-verification-validation`), kept a pointer (not a merged axis) per contrarian — "locus" is an epistemic stance here vs a mechanical observation-source for enforcement layers, and it does not inherit the "enactment is conditional" trade; directive home stays `title-10` E.3 / `title-15` §8.3. §9: annotated the Resource-monitor gap with a concrete token-observability method (governance *cost* telemetry — distinct from #48 *effectiveness*). Origin: 15-Claude-Code-rules gap analysis. Contrarian + coherence reviewed. | 206 |
| 2026-06-03 | v2.3.0 | §8.4: added the **delivery-cadence axis** (5th classification — which always-on principle reaches the model at which moment: boot-load / per-prompt / per-action / structural-gate / on-demand) + a single-home-default assignment rule; documented the reasoning frame's deliberate three-moment/three-form delivery (boot prose / gov-call interrogative scaffolds / per-prompt terse pointer). Companion: the UserPromptSubmit hook now injects the reasoning FRAME every prompt (turn-start re-anchor); OPERATIONS V-011 measures efficacy (manual-append-rate). Neighbor-drift fix: behavioral-floor count 10→12; the CFR §9.3.10 cross-ref clarified to title-10 (Layered Enforcement Stack) — it is valid, NOT dangling (an in-arc "drop as dangling" was reverted post-7S after the ref was found in title-10, not title-20). Plan: joyful-dancing-pebble.md; 3 contrarian rounds + coherence audit + code-reviewer. | 206 |
| 2026-06-27 | v2.4.0 | §6.4: added **Definition — "memory files" (term scope)** — names the framework's governance-managed, user-editable state files (SESSION-STATE/PROJECT-MEMORY/LEARNING-LOG/BACKLOG/OPERATIONS + procedural skills) vs. the host LLM's native memory (out of scope, hands-off; cross-ref `rules-of-procedure §G.5` + `PROJECT-MEMORY` ADR-10). Resolves prior membership drift (BACKLOG #72). Companion prevention edits: AGENTS.md scope line + scaffold CLAUDE.md note + a guard test. Contrarian `a0bc0f3f` + coherence audit. | 229 |
| 2026-07-11 | v2.5.1 | §6.4: one location clause — memory files live in `_ai-context/` for all project types (unified layout, CFR v2.62.0 reversal of the v2.36.0 split; root grandfathered). Definition scope unchanged. | 243 |
| 2026-06-28 | v2.4.1 | §8.4: added a **Reachability** note under the Client capability summary — the deciding line is the **Claude Desktop app vs the browser** (Cowork/Dispatch run only inside Desktop; browser claude.ai unreachable). A Desktop-app surface reaches MCP two ways: a remote custom connector (cloud-origin, bypasses the local proxy) OR the **Desktop local-agent connection to the local stdio `mcpServers`** (routes through the `ai-governance-proxy`). **User-verified 2026-06-28: ai-governance is reachable inside Cowork via the Desktop local-stdio connection → the proxy IS in Cowork's path** (beyond Anthropic's "connectors are remote" doc; corroborated by the on-disk Desktop↔Cowork bridge). Net: classic chat + Cowork (+ Dispatch) reach the local proxy (act-intrinsic security gate covers them); behavioral enforcement (hooks) stays Claude Code only. Companion: title-10 v2.58.1 (Dispatch cloud→local). coherence-auditor 0-dangerous. | 228 |
| 2026-07-03 | v2.4.2 | §8.4: added a **Cross-CLI note** to the structural-enforcement table + corrected the Gap-analysis "hooks unavailable outside Claude Code CLI" overreach — **T-161 obligation** (new enforcement layer). Codex CLI 0.142.5 ships a PreToolUse hook system; this framework now runs an act-intrinsic content-security hook there (`ai_governance_mcp.codex_hooks`), LIVE-FIRE VERIFIED blocking a secret value in **both** interactive and `codex exec` (hooks fire in exec where MCP tools don't). Scope: act-intrinsic security only; the behavioral governance-recency gate stays Claude-Code-only (#176b, feasible via the payload's `transcript_path`). Companion: title-10 v2.59.1, title-20 v2.21.1. `gov-8309001613f7`; #176a. | 236 |
| 2026-07-05 | v2.4.3 | §8.4: **#183 Codex FRAME `UserPromptSubmit` steering hook → LIVE-FIRE VERIFIED** (capture recon 940/940 `inject`, zero errors) — Cross-CLI note gains the steering sibling; the Session-lifecycle UserPromptSubmit row notes the Codex FRAME-only sibling (`ai_governance_mcp.codex_frame`, FRAME SSOT shared with the Claude Code hook). T-161 firing recorded in OPERATIONS. Companion: title-10 v2.59.2, title-20 v2.21.2. BACKLOG #183 closed. | 239 |
| 2026-07-05 | v2.5.0 | §7.2 **Honest scope rewritten — execution split by cadence class (dream auto-run).** At the activity threshold the dream hook now injects an AUTO-RUN directive (background analysis, HITL apply/commit, `DREAM_AUTORUN=0` reverts, cold-start stays a nudge); labeled truthfully: trigger structural, the directive's execution ADVISORY and now **measured** (both memory-maintenance hooks write fire logs; compliance-review fired-vs-ran sub-check; V-010 → live instrument; blocking-gate escalation pre-named at <~80% over ≥10 fires). Judgment-bearing cadences (C-078/C-155/C-109) stay surfaced-advisory. §3.3 hook list: `journal-reminder.sh` promoted to repo-canonical (was user-level-only — untracked/untested) + stale "(user-level)" label on `session-start-dream.sh` fixed. User-directed (session-241); contrarian `a57b74b89d14286d7`; companion title-10 v2.60.0, ai-instructions v4.20.0. | 241 |
| 2026-08-20 | v2.6.2 | §8.4: corrected the Claude App reachability matrix from blanket Cowork/Dispatch local-proxy coverage to a transport-specific, pending-revalidation boundary. Remote connectors bypass the local proxy; classic Desktop local stdio is distinct; current Anthropic documentation conflicts with the 2026-06-28 Cowork observation. T-164/#309 owns the live reproduction. | 309 |
| 2026-08-17 | v2.6.1 | §5 + §6.2: removed the last two surviving descriptions of the reversed seventh memory type. The CoALA-provenance line still named **Operational** a core-CoALA-plus-extensions type — the provenance claim CFR v2.69.1 refuted — and the OPERATIONS interface row advertised "5 sections … C/T/M/V prefixes" for a file with 7 sections and C-/T-/V-/M-/SA-/RW-/SO- forms. Found by a Codex cross-vendor review after a coherence-auditor pass had already run: two independent reviewers, two different misses, which is the argument for both. |
| 2026-08-17 | v2.6.0 | §6.1/§6.2 + §7: aligned with CFR Part 7's six-type taxonomy and BACKLOG #344. The §6.2 memory-interface table still labelled `OPERATIONS.md` **Operational** — the seventh cognitive type that CFR v2.69.1 derived and reversed the same day — directly beside the **Prospective** row for `BACKLOG.md`, splitting one cognitive type across two labels inside a single table. That is the surface most likely to re-derive the reversed type, and it is why §7.0.2 carries a paragraph recording the supersession. Now **Prospective (recurring)**. §6.1's contract for `SESSION-STATE.md` no longer specifies a "Session Summaries" section (the region deleted 2026-08-15 as the one two concurrent close-outs collide on), and §7's P3/P4 context-priority rows no longer describe evicting it. *Correction, same-day:* this entry originally claimed alignment with the six-type taxonomy while §5's CoALA-provenance line still listed **Operational** as a type and §6.2's OPERATIONS row still described 5 sections and 4 prefixes against a file with 7 and 6. Both corrected in v2.6.1; the overclaim is left visible here because a history entry asserting completeness it did not have is the thing that stops the next reader from re-checking. **Found by a dispatched coherence-auditor, not by the edits themselves** — this file was changed twice on 2026-08-16 with no version bump and no history entry, and a file nobody stamps is a file nobody re-audits. |
| 2026-07-26 | v2.5.2 | §3.3 + §7.2: corrected the global-hook install description — the `~/.claude` / `~/.codex` installs are **symlinks** into the checkout (BACKLOG #226), not byte-mirrored copies. §3.3 was fixed first and §7.2 missed, so the document briefly said two different things about one install; caught by coherence audit. Factual correction to descriptive prose, no normative change → PATCH, same shape as v2.4.2. Companion: title-10 v2.63.2. BACKLOG #237. | 266 |
