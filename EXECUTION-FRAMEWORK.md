...
last_updated: 2026-06-03 (session-206)
...
|---|---|---|
| Triggered handlers | Event-driven actions | 8 project hooks (PreToolUse, PrePush, PostToolUse, UserPromptSubmit, SessionStart) |
...
|---|---|---|
| Pre-action gates | Block before damage | 8 hard-mode hooks (PreToolUse blocking, etc.); the SessionStart cadence surfacer is non-blocking and excluded |
...
Current hook inventory (8 project-level + 2 user-level + Rampart tripwired):
...
- `journal-reminder.sh` (**user-level**, `~/.claude/hooks/`) — memory maintenance journaling trigger. Fires for all projects with memory files. See CFR §7.11.
- `session-start-dream.sh` (**user-level**, `~/.claude/hooks/`) — SessionStart dream cadence surfacer; fires for all projects with memory files. See §7.2.
...
**Current status:** 13 unique skills (4 project-local + 10 global; `/content-enhancer` exists in both). Project-local: `/compliance-review` (13-check governance compliance review), `/completion-sequence-aigov` (post-change verification gate), `/test-authoring` (9-step test creation protocol), `/content-enhancer` (content improvement via evidence-based analysis). Global (cross-project, `~/.claude/skills/`): `/code-review`, `/security-scan`, `/test-suite`, `/doc-gen`, `/refactor-audit`, `/architecture-review`, `/completion-sequence`, `/content-enhancer`, `/journal` (memory maintenance — session transcript analysis for unpersisted items, §7.11), `/dream` (cross-session transcript mining with HITL review, §7.11). Each follows the self-contained folder pattern: thin SKILL.md orchestration shell (~80-100 lines) + reference files for procedure content and mutable data. Authoring standards: CFR Part 9.5.
...
| **Application programs** | Skills | `.claude/skills/` (4: compliance-review, completion-sequence-aigov, test-authoring, content-enhancer) | User-invocable instruction sequences. Self-contained folders. Decision matrix in §3.7. |
...
| Computer Component | Project Equivalent | Current Implementation | Interface Type |
|---|---|---|---|
| **Interrupt controller (PIC/APIC)** | Hook system | `.claude/hooks/` (9 scripts + Rampart tripwired) | Event → exit code. Modular — each standalone. |
...
- **`SessionStart`** — fires on startup/resume/clear/compact (a `source` field distinguishes them) and **can inject `additionalContext`**. The **reliable workhorse**: used for date-checked **cadence surfacing** — at session start each cadence reads its `Next due:` date and, if overdue, injects a one-line nudge. See `.claude/hooks/session-start-cadence.sh` (project cadences: C-078/C-155/C-109) and `~/.claude/hooks/session-start-dream.sh` (global dream cadence).
...
**Honest scope — trigger solved, execution not.** These hooks make the *date-check structural* (it happens deterministically every session start, closing the "did I remember to read OPERATIONS.md?" gap). **Acting** on a surfaced cadence stays advisory — the proportionate level for periodic *maintenance* (a hard gate would block unrelated work). Autonomous *execution* of judgment-bearing reviews is still host/local-MCP-bound: cloud routines cannot call this project's local MCP servers, and `SessionStart` only nudges. The local-MCP blockers in OPERATIONS SO-001/SO-002 remain true; only SO-003's "session-end detection is impossible" blocker is resolved.

**Compensating controls (still valid):** session-start pruning (CLAUDE.md § Session Lifecycle); Session Closer Protocol (`multi-method-session-closer-protocol`, manually invoked); context retention priorities (§7.1). The SessionStart surfacer now augments these by auto-surfacing due cadences. See OPERATIONS.md SO-003/SO-004 for operational tracking and §8.4 for the enforcement-layer rows.
...
| Layer | Mechanism | Claude Code CLI | Claude App | Other MCP Clients | RAG-only | Degrades Over Conversation? |
|-------|-----------|:-:|:-:|:-:|:-:|:-:|
| CLAUDE.md behavioral floor | 12 directives loaded at session start | Yes | No | No | No | Yes |
...
| Layer | Mechanism | Claude Code CLI | Claude App | Other MCP Clients | RAG-only | Degrades Over Conversation? |
|-------|-----------|:-:|:-:|:-:|:-:|:-:|
| UserPromptSubmit hook | Injects reasoning FRAME every prompt (turn-start re-anchor) + governance reminder if tools not called | Yes | No | No | No | No |
| SessionStart cadence surfacer | Date-checks cadences at session start; injects due-cadence/dream nudges (§7.2) | Yes | No | No | No | No |
...
| **Virtual memory manager** | Page replacement policy, eviction priority | Host handles compaction; no framework say in eviction. Context retention policy defined (§7.1). Session-boundary automation resolved (§7.2): SessionStart surfacer triggers cadence/dream nudges; SessionEnd unsuitable (post-termination). | **Addressed** by §7 + SO-004. Autonomous execution still host-bound. |
