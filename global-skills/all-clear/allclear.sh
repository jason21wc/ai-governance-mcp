...
# `start-worktree`'s `publish_branch` publishes every branch at creation, which is why DURABLE
...
#   * It never proposes deleting a branch whose landing is inferred from CONTENT.
#     Ancestry LIES after a rebase or squash — `--no-merged` once called three
#     branches unmerged when every file they touched was byte-identical to main.
#     Unlanded branches get evidence and a merge command, never a delete command,
#     and neither do content-equivalent ones: their commits are unreachable from
#     the default, so the branch is the only place that history exists.
#     A delete command is offered in exactly one case: a remote-only branch whose
#     every commit is reachable from the REMOTE default ref. Nothing is lost by
#     deleting that, because everything on it is already on the integration
#     branch and off this disk. (This bullet said "never ... on ancestry alone"
#     until 2026-09-01, when the residue arm was added and the contract was not
#     updated with it — the defect class this file exists to catch.)
...
usage() {
    ...
    say "  --quiet          Suppress the repository header line (findings and the"
    say "                   verdict always print; this flag is narrower than its"
    say "                   old description of \"only findings and the verdict\")"
    say ""
...
_add_remote_ref() {
    git rev-parse --verify --quiet "$1" >/dev/null 2>&1 \
        && REMOTE_DEFAULT_REFS="${REMOTE_DEFAULT_REFS}${REMOTE_DEFAULT_REFS:+ }$1"
}
if [ -n "$DEFAULT_NAME" ]; then
    _add_ref "origin/$DEFAULT_NAME"
    _add_ref "$DEFAULT_NAME"
    # REMOTE-ONLY, deliberately. DEFAULT_REFS carries the local default too,
    # which is right for "has the work arrived?" and WRONG for "may this ref be
    # deleted?" -- a branch merged into local `main` and not yet pushed is landed
    # by that set while its commits still live on exactly one disk. cleanup.sh
    # already keeps the two sets apart for this reason and says so at its
    # durability check: a matching local default is completeness evidence, not
    # off-disk durability. Conflating them is the founding defect in this file's
    # own header, one level down.
    _add_remote_ref "origin/$DEFAULT_NAME"
fi
...
# landed_by_ancestry <branch> -> 0 only when every commit on <branch> is REACHABLE
# from a REMOTE default ref. Deliberately narrower than landed(): that one also accepts
# content equivalence, which is what makes a squash- or rebase-merge count as
# landed even though the branch's granular commits are unreachable from the
# default. Both answers are correct for their own question. "Has the work
# arrived?" is landed(). "Is this ref now redundant?" is this one — because
# deleting a squash-merged branch discards history the default never received.
landed_by_ancestry() {
    local br="$1" ref n
    [ -z "$REMOTE_DEFAULT_REFS" ] && return 1
    for ref in $REMOTE_DEFAULT_REFS; do
        n=$(git rev-list --count "$ref..$br" 2>/dev/null) || continue
        [ "${n:-1}" = "0" ] && return 0
    done
    return 1
...
check_branch() {
    ...
    # LANDED. Everything past here is about what the lifecycle LEFT BEHIND.
    #
    # `prepare.sh` publishes every topic branch to origin at creation, on both the
    # framework arm and the Codex Desktop arm, so the remote branch is a durability
    # artifact that exists from birth. Teardown destroys the worktree, the local
    # branch, the Git lock and the journal -- and never the remote branch. Nothing
    # in the lifecycle retires it, on ANY exit path: framework finalize, Desktop
    # release, abandonment, or a deliberate --allow-unmerged discard. This is where
    # that shows up. Eight had accumulated on the real repo before anyone looked,
    # two of them at an identical SHA.
    #
    # PRESENCE, NOT A FINDING -- the same call made for live worktrees and foreign
    # branches above. "All clear" has to keep meaning "your sessions are in", not
    # "the repo has zero leftover refs", or it can never go green and stops being
    # read. Residue is not outstanding work, so it is named, given its command, and
    # left out of the verdict.
    #
    # Only remote-only branches qualify, and that is a STRUCTURAL guarantee rather
    # than a liveness guess: REMOTE_ONLY is remote refs minus local refs, and a live
    # worktree always holds a local branch. An in-flight session cannot appear here
    # even at the instant its work becomes an ancestor of the default -- no registry
    # sample, no timestamp, nothing to go stale between the check and the command.
    if [ "$origin_label" = "remote-only" ]; then
        if landed_by_ancestry "$br"; then
            # "in this checkout" is the honest scope. The set arithmetic proves
            # no LOCAL ref here holds it; a session working from a different
            # clone of the same origin keeps its local ref over there.
            item "left  $br — landed in ${REMOTE_DEFAULT_REFS%% *}, no local ref in this checkout"
            fix "git push origin --delete ${br#origin/}"
            RESIDUE=$((RESIDUE + 1))
        else
            # Content-identical to the default but not an ancestor: squash or
            # rebase. The commits are unreachable from the default, so the branch
            # is the only place that history exists. Named, never offered for
            # deletion -- the same refusal the not-landed arm makes above.
            item "left  $br — content is in $PRIMARY_REF but its commits are not"
            item "      (squash/rebase merge, or an independently identical tree —"
            item "       either way this branch holds commits $PRIMARY_REF never got)"
        fi
    fi
}
...
# Residue is not outstanding work, so it must not gate the verdict -- but the
# verdict must not deny what the body just named either. A summary that
# contradicts its own output is the failure this tool exists to prevent.
[ "$RESIDUE" -gt 0 ] && say "$RESIDUE landed remote branch(es) left on origin — retire them with the → commands; not outstanding work."
if [ "$UNDETERMINED" -eq 1 ]; then
    ...
    if [ "$RESIDUE" -gt 0 ]; then
        say "ALL CLEAR — every worktree clean, every branch landed, nothing stashed."
    else
