...
**Never delete a branch because this report listed it as OPEN or as a `note`.**
Ancestry lies after a rebase or squash: a branch can be fully landed and still look
unmerged, and it can look merged when substantive work differs. No delete command is
offered for those, and none should appear in your response for them either.

**The one exception, added 2026-09-01 (ADR-37): a `left` line.** That class is a
remote-only session branch whose every commit is reachable from the *remote* default
ref — nothing on it exists anywhere else, so retiring it loses nothing. It carries a
`git push origin --delete` command and you should relay that command. It is reported
as presence, not a finding: it does not gate the verdict, because residue is not
outstanding work. A branch that is content-identical to the default but whose commits
are NOT reachable from it (squash or rebase) gets a `left` line with **no** command,
and you must not supply one — the branch is the only place that history exists.
