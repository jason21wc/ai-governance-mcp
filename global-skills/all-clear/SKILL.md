...
- **Durability is nearly free here, so it is the weakest signal.** `start-worktree`
  `publish_branch` publishes every branch at creation to reserve the name. Every worktree branch
...
- **Ancestry lies — so report evidence, never a verdict.** After a rebase or squash,
  ...
  command and a `git log` handle for unlanded work, never a delete command. It offers
  deletion in exactly one case (ADR-37): a remote-only session branch whose every
  commit is reachable from the REMOTE default ref, where deletion loses nothing
  because everything on it is already on the integration branch and off this disk.
  Landing inferred from CONTENT never qualifies — those commits are unreachable from
  the default, so that branch is the only place the history exists.
