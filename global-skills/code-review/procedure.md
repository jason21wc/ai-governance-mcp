...
- `passes-optional/cross-vendor.md` — a different-lineage (Codex/gpt) second opinion. **Default
  (security role) where `scripts/codex_review.py` exists in the repo and `codex` is on PATH**
  (promoted session-240 on measurement); elsewhere triggered by "cross-vendor", "codex review",
  "second opinion", or "full review". This one is orchestrator-run via **Bash**, not a subagent;
  it self-skips when `codex` is unavailable. Read it for the run + fold protocol.
...
**Cross-vendor pass (if activated) is NOT a subagent dispatch.** In the SAME turn as the parallel
Agent dispatch, also run the Codex peer via Bash (`python scripts/codex_review.py …` in this repo,
or `codex exec` per `passes-optional/cross-vendor.md`). It runs concurrently with the Claude
subagents, so it adds no serial latency. If `codex` is absent it self-skips — proceed with the
Claude passes alone.

## Step 5: Reconciliation

After all subagents return (and the cross-vendor Codex peer, if it ran), reconcile their
findings into a single report. Codex findings are advisory candidate input — they pass the
same evidence gate, dedup, and severity calibration as Claude-pass findings, never a bypass.
...
- **Same `file:line` raised by BOTH a Claude pass and the cross-vendor Codex peer** → merge and mark `(claude+codex — confirmed cross-vendor)`; two lineages agreeing is a higher-confidence signal. A **Codex-only** finding keeps a `(codex)` attribution so the reader can weigh it.
