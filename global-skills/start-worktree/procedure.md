...
`READY` also means any tracked `commit-msg` and `pre-push` hook contract was
installed and verified. If installation fails, the checkout remains non-ready;
fix the tool or declaration and resume the same recorded transition.
...
A `.git` write rule under `:workspace_roots` covers Git administration when the
primary checkout is a workspace root. It does not follow a linked checkout's
`.git` pointer to the shared directory. For an authorized writer that launches
or resumes directly in a linked worktree, resolve the physical directory with
`git rev-parse --git-common-dir` and configure that exact directory in the writing
profile, preserving all existing entries:

```toml
[permissions.governed.filesystem]
"/absolute/path/to/repository/.git" = "write"
```

This is a merge into the existing table, not a replacement of its deny rules.
An absolute entry in a user-level profile is available to every session using
that profile, including other projects. It authorizes this repository's shared
objects, refs, config, hooks, and worktree metadata; it is not branch-only scope.
Configure only the repository whose Git authority was authorized. Do not grant
the entire parent checkout to solve a Git-directory denial. Verify the resulting
profile in a fresh intended-host invocation; a saved setting alone does not prove
that an already-running session has adopted it.

If Codex was launched directly in a prepared worktree, validate before editing:

```bash
bash "$SW/prepare.sh" codex-cli-validate --owner-pid "$PPID" --verify-refresh
```
...
bash "$SW/prepare.sh" codex-desktop-adopt \
  --slug <work-slug> \
  --default-ref <default> --verify-refresh
```
...
### Verify refresh in the actual startup/resume host

`READY` without `--verify-refresh` proves lifecycle state only. The flag runs the
real sibling `completion-sequence/integrate.sh refresh` after ownership and state
checks, before reporting success. Hook requirements are verified again after
refresh because newly merged files can change that contract. It prints both the per-worktree Git directory
and the common Git directory: the checkout's `.git` pointer is not the physical
write boundary. Install completion-sequence beside start-worktree; a missing
helper fails explicitly rather than choosing a different installed version.

**This is an active fetch-and-merge operation.** It requires a clean topic and may
integrate newer commits from the recorded live default. A dirty checkout or merge
conflict leaves refresh capability unverified; preserve the work and follow the
helper's output. Never auto-stash or commit just to make a diagnostic pass. An
exit code alone cannot identify a network, authentication, or filesystem cause.

Run the flagged CLI validation above after creation or relaunch. For an already
adopted Desktop worktree, run:

```bash
bash "$SW/prepare.sh" continue --path <native-path> --verify-refresh
```

Run it inside that chat. The flag also works on other lifecycle actions
when active refresh is intended; ordinary preflight and status remain read-only.
A successful bootstrap in an ordinary shell does not certify the Codex host that
later targets the tree. Repeat verification in the intended host after resume or
a permission change; the script stores no portable capability-pass flag.

Successful refresh proves the fetch and metadata writes that actually ran. It
does not prove push/publication authority, package/model access, or all future Git
operations. The check never changes saved permissions or pushes a branch.
...
## 7. Clean up only after integration
...
### The published branch outlives every exit path

Creation publishes the topic branch to `origin` immediately, on both the framework
and the Codex Desktop arm, so the recovery handle exists before the first edit. (The
one exception is `--allow-local-only`, for a repository with no origin at all — there
is no remote branch to retire, and one-disk durability is the acknowledged cost.)
Four of the five things creation makes have an inverse in `cleanup.sh` — the worktree
directory, the local branch, the Git lock, the journal. **The remote branch has none,
on any exit path**: framework finalize, Desktop release, an abandoned tree, or
a deliberate `--allow-unmerged` discard. Cleanup deletes local refs only and says
so; it does not touch a remote.

That asymmetry is the whole reason landed `wt/*` branches accumulate. Eight had
collected on this repository's origin, two at an identical SHA — three days after a
manual sweep had deleted seven of the same class. The recurrence is the point: the
sweep was the wrong instrument, not an insufficient one. The retirement step is deliberately **not** automated inside
cleanup: `git push --delete` is a push, per-push authorization is a standing rule
here, and a cleanup-side deletion could not reach a Desktop tree or an abandoned
one anyway — cleanup exits early for the first and never runs for the second.

So retirement is reported, not performed. `/all-clear` names every landed
remote-only session branch and prints its exact `git push origin --delete`
command; cleanup prints the same command for the branch it just finished with. One
exception, and it is the one that matters: **after `--allow-unmerged` no deletion
is ever suggested**, because that path means the work never landed and the remote
branch is the only surviving copy. A squash- or rebase-merged branch is named but
also never offered for deletion — its content reached the default while its commits
did not, so the branch holds history the default never received.
