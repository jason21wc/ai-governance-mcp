...
usage() {
    ...
    say ""
    say "Removes the worktree and its LOCAL branch. It never touches a remote:"
    say "the published topic branch survives, and is reported by /all-clear."
    say ""
...
# Neither implies the other. `start-worktree`'s `publish_branch` publishes every branch AT
...
if [ "$DRY_RUN" -eq 1 ]; then
    ...
    # The preview must describe the same blast radius as the run. It does not
    # delete the remote branch, so say that here rather than letting the reader
    # infer symmetry between local and remote from silence.
    if [ "$ALLOW_UNMERGED" -eq 1 ]; then
        note "Would NOT touch origin/$WT_BRANCH — and would not suggest retiring it"
    else
        note "Would NOT touch origin/$WT_BRANCH — the real run leaves it and says so"
    fi
    exit 0
fi
...
# The remote branch OUTLIVES this script, and that is deliberate, not an
# oversight -- but it was previously silent, and silence is how eight of them
# accumulated on one origin before anyone counted.
#
# Why this prints a command instead of running it. `git push --delete` is a
# push, and session-250 deliberately removed a standing push grant to restore
# per-push authorization; widening an already-authorized command to reach the
# remote would change its blast radius without changing its authorization.
# `repo_hygiene.py` makes the same call for the same reason and says so.
#
# It also could not be a complete fix even if it ran: a codex-desktop tree exits
# this script long before any branch deletion, and an abandoned tree never runs
# it at all. Retirement therefore belongs to a surface that sees every exit path
# from the published state -- /all-clear -- and this line is only the pointer.
#
# NOT printed after --allow-unmerged: that is the deliberate-discard path, where
# the work never landed and the remote branch is the ONLY surviving copy.
# THREE conditions, and each excludes a case where the hint would point at the
# only surviving copy of something.
#
#   1. not --allow-unmerged  -- the deliberate-discard path, where the work never
#      landed and the remote branch is all that is left of it.
#   2. reachable from a REMOTE default ref -- `landed_in` above also accepts
#      CONTENT equivalence, which is what lets a squash- or rebase-merge count as
#      landed. Those commits are unreachable from the default, so that branch is
#      the only place the granular history exists. Completeness and redundancy
#      are different questions and this is the second one.
#   3. the ref is in the live set fetched at the durability check -- reusing that
#      instead of a fresh `git ls-remote`, which would be a second network round
#      trip after the worktree is already gone, with no connect timeout over
#      HTTPS and a transport failure indistinguishable from "no such ref".
#
# `origin` is hardcoded because publication is: prepare.sh pushes to origin only.
# Ask about the REMOTE-TRACKING ref, not $WT_BRANCH. Two reasons, and the first
# is load-bearing: the local branch was deleted a few lines up, so $WT_BRANCH no
# longer resolves here and every rev-list against it fails -- which would silently
# suppress the hint on exactly the runs that should print it. The second is that
# the remote ref IS the subject of the question: "may origin/<branch> be retired?"
HINT_REDUNDANT=0
HINT_REMOTE_REF="refs/remotes/origin/$WT_BRANCH"
for _dref in $REMOTE_DEFAULT_REFS; do
    _n=$(git rev-list --count "$_dref..$HINT_REMOTE_REF" 2>/dev/null) || continue
    if [ "${_n:-1}" = "0" ]; then HINT_REDUNDANT=1; break; fi
done
case " $LIVE_REMOTE_REFS " in
    *" refs/remotes/origin/$WT_BRANCH "*) HINT_ON_ORIGIN=1 ;;
    *)                                    HINT_ON_ORIGIN=0 ;;
esac
if [ "$ALLOW_UNMERGED" -eq 0 ] && [ "$HINT_REDUNDANT" -eq 1 ] && [ "$HINT_ON_ORIGIN" -eq 1 ]; then
    note "origin/$WT_BRANCH still exists; every commit on it is already in the default"
    note "Retire it when you are ready: git push origin --delete $WT_BRANCH"
fi
exit 0
