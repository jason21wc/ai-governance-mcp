...
Common options:
  ...
  --verify-refresh       After lifecycle validation, actively fetch/merge the recorded
                         default into the clean topic using completion-sequence.
                         Run inside the intended host at startup/resume; no push.
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --verify-refresh) VERIFY_REFRESH=1; shift ;;
    --help|-h) usage; exit 0 ;;
    *) say "Unknown argument: $1"; usage; exit 2 ;;
  esac
done

if [ "$VERIFY_REFRESH" -eq 1 ]; then
  case "$MODE" in
    claude-create|codex-cli-create|continue|claude-resume|codex-cli-validate|codex-desktop-adopt) : ;;
    *) say "ERROR: --verify-refresh requires a lifecycle action, not $MODE"; exit 2 ;;
  esac
fi
...
ensure_publication_gate() {
  local path="$1" checker="$1/scripts/check_git_hook_contract.py"
  git -C "$path" config --local ai-governance.defaultRef "$DEFAULT_REF" || {
    undet "could not record the default branch for the local publication gate"
    return 2
  }
  if [ -f "$checker" ]; then
    python3 "$checker" --ensure --repo "$path" || {
      block "declared Git hooks are not installed; checkout is NOT ready"
      return 2
    }
  fi
}
...
verify_refresh() {
  [ "$VERIFY_REFRESH" -eq 1 ] || {
    note "Refresh capability UNVERIFIED in this invocation; READY describes lifecycle state only"
    return 0
  }
  local helper="$SCRIPT_DIR/../completion-sequence/integrate.sh" gd common rc
  gd="$(gitdir_for "$TARGET")" || { undet "cannot resolve worktree Git directory; refresh UNVERIFIED"; return 2; }
  common="$(git -C "$TARGET" rev-parse --git-common-dir)" || return 2
  case "$common" in /*) : ;; *) common="$TARGET/$common" ;; esac
  common="$(cd "$common" && pwd -P)" || return 2
  note "Refresh checkout: $TARGET"
  note "Worktree Git directory: $gd"
  note "Common Git directory: $common"
  [ -f "$helper" ] || {
    undet "refresh helper unavailable: $helper; install completion-sequence beside start-worktree"
    return 2
  }
  note "Active verification: refresh fetches and may merge origin/$DEFAULT_REF; requires a clean topic"
  # Keep the real helper's stdout/stderr and exit status. A failed refresh can
  # mean dirty state, conflict, filesystem, network, auth, or another Git error;
  # its status alone does not distinguish those causes.
  (cd "$TARGET" && bash "$helper" refresh --default-ref "$DEFAULT_REF")
  rc=$?
  if [ "$rc" -ne 0 ]; then
    undet "Refresh capability UNVERIFIED (helper exit $rc); inspect the Git/helper output above"
    return "$rc"
  fi
  # Refresh can introduce new hook declarations/checker code from the default.
  # The pre-refresh prerequisite covered the old tree, not the merged contract.
  ensure_publication_gate "$TARGET"
  rc=$?
  if [ "$rc" -ne 0 ]; then
    undet "Refresh succeeded, but the resulting hook contract failed; checkout is NOT ready"
    return "$rc"
  fi
  ok "Refresh VERIFIED in this invocation: remote fetch and required Git metadata writes succeeded"
  note "This does not certify push, dependency access, or a later host/profile; no permissions changed"
}

ready_report() {
  ensure_publication_gate "$TARGET" || return $?
  verify_refresh || return $?
  ...
  if [ "$HOST" = "claude" ]; then
    ...
    note "Codex CLI active-session next: bash '$SCRIPT_DIR/prepare.sh' codex-cli-validate --path '$TARGET' --owner-pid '$OWNER_PID' --verify-refresh; then anchor every subsequent tool to '$TARGET'"
...
advance_existing() {
  ...
  if [ "$LIFECYCLE_OWNER" = "framework" ]; then
    ...
    if ! ensure_publication_gate "$TARGET"; then
      write_state "$TARGET" setup-failed >/dev/null 2>&1 || true
      return 2
    fi
    ...
    if [ "$state" = "ready" ]; then ready_report; return $?; fi
    if ! run_hook setup "$TARGET"; then
      write_state "$TARGET" setup-failed >/dev/null 2>&1 || true
