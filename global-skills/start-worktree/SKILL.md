...
adapter. **The slug is the user's, not yours:** it comes from the slash-command
argument (`/start-worktree backlog4`); when none was given, ask for it in one line
before running `prepare.sh`. Never guess it — creation publishes `wt/<slug>-<nonce>`
to `origin` immediately, and `cleanup.sh` deletes local refs only, so a guessed name
becomes a remote branch nothing automated can retire. The load-bearing behavior
lives in the scripts beside this file:
...
- `prepare.sh` records the default branch and installs/verifies the tracked `commit-msg` and `pre-push` hook contracts before it reports `READY`.
- `prepare.sh`'s `--verify-refresh` option actively runs the sibling completion helper after lifecycle checks; success applies only to the current host invocation. Unflagged `READY` does not certify Git/network capability.
