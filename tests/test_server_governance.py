        ...
        with patch(
            ...
            with patch("sentence_transformers.SentenceTransformer", mock_st):
                with patch("sentence_transformers.CrossEncoder", mock_ce):
                    ...
                    assert parsed["verification_scope"] == "consultation_evidence"
                    assert "Does not verify behavior" in parsed["limitation"]
...
class TestIntentOverLiteralInBehavioralFloor:
    ...
    def test_intent_over_literal_in_claude_md(self):
        """CLAUDE.md should import the shared disposition containing intent-over-literal."""
        claude_path = Path(__file__).parent.parent / "CLAUDE.md"
        # Structural import contract; actual host resolution is a separate live gate.
        assert "@AGENTS.md" in claude_path.read_text().splitlines()
        agents_path = claude_path.with_name("AGENTS.md")
        assert "intent over literal" in agents_path.read_text().lower()
...
class TestDefaultRegisterInBehavioralFloor:
    ...
    def test_default_register_in_claude_md(self):
        """CLAUDE.md should import the shared disposition containing the default-register directive."""
        claude_path = Path(__file__).parent.parent / "CLAUDE.md"
        # Structural import contract; actual host resolution is a separate live gate.
        assert "@AGENTS.md" in claude_path.read_text().splitlines()
        agents_path = claude_path.with_name("AGENTS.md")
        assert "default register" in agents_path.read_text().lower()
...
class TestPlainLanguageInBehavioralFloor:
    ...
    def test_plain_language_in_claude_md(self):
        """CLAUDE.md should import the shared disposition containing plain language / audience calibration."""
        claude_path = Path(__file__).parent.parent / "CLAUDE.md"
        # Structural import contract; actual host resolution is a separate live gate.
        assert "@AGENTS.md" in claude_path.read_text().splitlines()
        agents_path = claude_path.with_name("AGENTS.md")
        assert "plain language" in agents_path.read_text().lower()
...
class TestProactivePartnershipInBehavioralFloor:
    ...
    def test_proactive_partnership_in_claude_md(self):
        """CLAUDE.md should import the shared disposition containing proactive partnership."""
        claude_path = Path(__file__).parent.parent / "CLAUDE.md"
        # Structural import contract; actual host resolution is a separate live gate.
        assert "@AGENTS.md" in claude_path.read_text().splitlines()
        agents_path = claude_path.with_name("AGENTS.md")
        assert "proactive partnership" in agents_path.read_text().lower()
...
class TestRetrievalCoverageAssessment:
    """Pin retrieval health at the serialized handler boundary using real models."""

    @pytest.mark.asyncio
    async def test_missing_index_requires_judgment_without_revoking_authorization(
        self, reset_server_state, test_settings
    ):
        ...
        # Actual unavailable engine, not an empty permissive mock result.
        engine = RetrievalEngine(test_settings)
        response = await _handle_evaluate_governance(
            engine, {"planned_action": "Rename an approved fixture"}
        )
        parsed = json.loads(response[0].text)
        assert parsed["index_loaded"] is False
        assert parsed["semantic_available"] is False
        assert parsed["assessment"] == "REVIEW"
        assert parsed["confidence"] == "low"
        assert parsed["requires_ai_judgment"] is True
        assert "index unavailable" in parsed["retrieval_warning"]
        assert "Action may proceed" not in parsed["rationale"]
        assert (
            "Existing user authorization remains in effect"
            in parsed["ai_judgment_guidance"]
        )
        assert (
            "does not require another permission request"
            in parsed["ai_judgment_guidance"]
        )

    @pytest.mark.asyncio
    @pytest.mark.parametrize("semantic_available", [True, False])
    @pytest.mark.parametrize("score", [None, 0.1, 0.9])
    async def test_coverage_independent_of_match_strength(
        self,
        reset_server_state,
        test_settings,
        sample_principle,
        ...
        from ai_governance_mcp.models import RetrievalResult, ScoredPrinciple
        from ai_governance_mcp.retrieval import RetrievalEngine
        from ai_governance_mcp.server.handlers.governance import (
            _handle_evaluate_governance,
        )

        principle = sample_principle.model_copy(update={"series_code": "Q"})
        result = RetrievalResult(
            query="Rename fixture",
            domains_detected=[],
            index_loaded=True,
            semantic_available=semantic_available,
            constitution_principles=[]
            if score is None
            else [ScoredPrinciple(principle=principle, combined_score=score)],
        )
        engine = RetrievalEngine(test_settings)
        with patch.object(engine, "retrieve", return_value=result):
            response = await _handle_evaluate_governance(
                engine, {"planned_action": result.query}
            )
        parsed = json.loads(response[0].text)
        assert parsed["index_loaded"] is True
        assert parsed["semantic_available"] is semantic_available
        assert parsed["compliance_evaluation"] == []
        assert len(parsed["relevant_principles"]) == (score is not None)
        if semantic_available:
            assert parsed["retrieval_warning"] is None
            assert parsed["assessment"] == ("REVIEW" if score == 0.9 else "PROCEED")
        else:
            assert parsed["assessment"] == "REVIEW"
            assert parsed["confidence"] == "low"
            assert parsed["requires_ai_judgment"] is True
            assert "keyword-only" in parsed["retrieval_warning"]
            assert "Action may proceed" not in parsed["rationale"]

    @pytest.mark.asyncio
    @pytest.mark.parametrize("safety_path", ["principle", "floor", "genuine"])
    async def test_incomplete_coverage_preserves_safety_precedence(
        self, reset_server_state, test_settings, sample_principle, safety_path
    ):
        from ai_governance_mcp.models import RetrievalResult, ScoredPrinciple
        from ai_governance_mcp.retrieval import RetrievalEngine
        ...
        test_settings.keyword_judge_mode = "active"
        action = "Rename fixture" if safety_path == "principle" else "Use credentials"
        if safety_path == "floor":
            action = "Store credentials in plaintext"
        principle = sample_principle.model_copy(
            update={"series_code": "S", "id": "test-safety"}
        )
        result = RetrievalResult(
            query=action,
            domains_detected=[],
            index_loaded=safety_path == "principle",
            semantic_available=False,
            constitution_principles=[
                ScoredPrinciple(principle=principle, combined_score=0.9)
            ]
            if safety_path == "principle"
            else [],
        )
        engine = RetrievalEngine(test_settings)
        with (
            patch.object(engine, "retrieve", return_value=result),
            patch.object(
                governance,
                "adjudicate_keyword_trigger",
                return_value={"verdict": "genuine", "reason": "fixture"},
            ) as judge,
        ):
            response = await governance._handle_evaluate_governance(
                engine, {"planned_action": action}
            )
        parsed = json.loads(response[0].text)
        assert parsed["assessment"] == "ESCALATE"
        assert parsed["confidence"] == "high"
        assert parsed["requires_ai_judgment"] is False
        assert parsed["s_series_check"]["triggered"] is True
        assert parsed["retrieval_warning"]
        assert parsed["retrieval_warning"] in parsed["rationale"]
        if safety_path == "principle":
            assert parsed["s_series_check"]["principles"] == ["test-safety"]
            judge.assert_not_called()
        else:
            assert parsed["s_series_check"]["keyword_adjudication"] == safety_path

    @pytest.mark.asyncio
    @pytest.mark.parametrize("match", ["none", "different", "partial", "matching"])
    async def test_verification_always_discloses_consultation_scope(
        self, reset_server_state, match
    ):
        from ai_governance_mcp.models import GovernanceAuditLog, AssessmentStatus
        from ai_governance_mcp.server._logging import log_governance_audit_async
        from ai_governance_mcp.server.handlers.governance import (
            _handle_verify_governance,
        )

        if match != "none":
            await log_governance_audit_async(
                GovernanceAuditLog(
                    audit_id="gov-fixture",
                    timestamp="2026-09-13T00:00:00Z",
                    action="Change fixture title"
                    if match != "different"
                    else "Review alternate layout",
                    assessment=AssessmentStatus.REVIEW,
                    confidence="low",
                )
            )
        args = {"action_description": "Change fixture title"}
        if match == "partial":
            args["expected_principles"] = ["test-expected"]
        response = await _handle_verify_governance(args)
        parsed = json.loads(response[0].text)
        assert (
            parsed["status"]
            == {
                "none": "NON_COMPLIANT",
                "different": "NON_COMPLIANT",
                "partial": "PARTIAL",
                "matching": "COMPLIANT",
            }[match]
        )
        assert parsed["verification_scope"] == "consultation_evidence"
        assert "Does not verify behavior" in parsed["limitation"]
        assert "before this action" not in parsed["finding"]
        assert "not all checked" not in parsed["finding"]
        if match in ("matching", "partial"):
            assert "matching consultation record" in parsed["finding"]
