...
class TestLandedRemoteOnlyBranchResidue:
    """The `published` lifecycle transition had no inverse.

    `prepare.sh` publishes every topic branch to origin at creation, on BOTH the
    framework arm and the Codex Desktop arm. Four of the five things creation makes
    (worktree dir, local branch, Git lock, journal) are destroyed on the way out;
    the remote branch never was. Eight of them accumulated on this repo's origin
    before anyone looked, two at an identical SHA.

    `repo_hygiene.py`'s stale-branch check keys on committerdate >= 14 days, so
    fresh residue is invisible for two weeks and then arrives as N findings needing
    per-branch adjudication. This surface is the right home instead: `check_branch`
    already computes everything needed and reported only the NOT-landed case, so a
    landed remote-only session branch produced no output at all.

    Why remote-only is the correct signal and needs no liveness probe: REMOTE_ONLY
    is remote refs MINUS local refs, and a live worktree always holds a local
    branch. An in-flight session is therefore excluded structurally, not by an
    instant-in-time registry sample that is stale the moment it is read.
    """

    def _land_and_orphan(self, repo: Path, name: str) -> None:
        """Leave `name` on origin only, fully landed, with no worktree or local ref."""
        wt = _worktree(repo, name)
        _commit_in(wt, "real work")
        _git(wt, "push", "-q", "origin", name)
        _git(repo, "push", "-q", "origin", f"{name}:main")
        _git(repo, "worktree", "remove", "--force", str(wt))
        _git(repo, "branch", "-D", name)
        _git(repo, "fetch", "-q", "origin")

    def test_landed_remote_only_session_branch_is_reported(self, repo):
        """RED before the else-arm existed: the branch produced no output at all."""
        self._land_and_orphan(repo, "wt/finished-a1b2c3d4")
        r = _run(repo)
        assert "wt/finished-a1b2c3d4" in r.stdout, (
            "a landed remote-only session branch is residue and must be named; "
            f"got:\n{r.stdout}"
        )
        assert "push origin --delete wt/finished-a1b2c3d4" in r.stdout

    def test_squash_merged_branch_is_named_but_gets_no_delete_command(self, repo):
        """`landed()` accepts CONTENT equivalence, not just ancestry.

        A squash- or rebase-merged branch is content-identical to the default while
        its granular commits are unreachable from it. Deleting it discards history
        `main` never received, so the delete command is withheld and only that arm
        gets it. This mirrors the refusal already written into the not-landed arm:
        ancestry lies, so the human adjudicates.
        """
        wt = _worktree(repo, "wt/squashed-b2c3d4e5")
        _commit_in(wt, "first")
        _commit_in(wt, "second")
        _git(wt, "push", "-q", "origin", "wt/squashed-b2c3d4e5")
        # Squash: same tree, different history.
        _git(repo, "merge", "-q", "--squash", "wt/squashed-b2c3d4e5")
        _git(repo, "commit", "-q", "-m", "squash: all of it")
        _git(repo, "push", "-q", "origin", "main")
        _git(repo, "worktree", "remove", "--force", str(wt))
        _git(repo, "branch", "-D", "wt/squashed-b2c3d4e5")
        _git(repo, "fetch", "-q", "origin")

        r = _run(repo)
        assert "wt/squashed-b2c3d4e5" in r.stdout
        assert "push origin --delete wt/squashed-b2c3d4e5" not in r.stdout, (
            "a squash-merged branch holds history main never got; "
            "naming it is right, offering deletion is not"
        )

    def test_live_session_branch_is_never_called_residue(self, repo):
        """The false positive that would make this check destructive.

        A live worktree's branch is an ancestor of main the moment its work lands,
        which satisfies the landed test while the session is still writing. It must
        not be reported, and it is not — because it still has a local ref and so is
        never in REMOTE_ONLY.
        """
        wt = _worktree(repo, "wt/inflight-c3d4e5f6")
        _commit_in(wt, "landed so far")
        _git(wt, "push", "-q", "origin", "wt/inflight-c3d4e5f6")
        _git(repo, "push", "-q", "origin", "wt/inflight-c3d4e5f6:main")
        _git(repo, "fetch", "-q", "origin")
        reason = f"claude session test (pid {os.getpid()} start 2026-01-01T00:00:00Z)"
        _git(repo, "worktree", "lock", "--reason", reason, str(wt))

        r = _run(repo)
        assert "push origin --delete wt/inflight-c3d4e5f6" not in r.stdout, (
            "an in-flight session's branch was offered for deletion"
        )

    def test_locally_merged_but_unpushed_branch_is_never_offered_for_deletion(
        self, repo
    ):
        """The delete command must rest on REMOTE landing, not local.

        DEFAULT_REFS is `origin/main main` — both refs. A predicate that iterates
        it accepts a branch merged into LOCAL main whose commits were never
        pushed. Deleting the remote branch there takes the work from two disks to
        one, which is the durability-vs-completeness conflation this tool's own
        header names as the defect it exists to prevent.
        """
        wt = _worktree(repo, "wt/local-only-d4e5f6a7")
        _commit_in(wt, "unpushed work")
        _git(wt, "push", "-q", "origin", "wt/local-only-d4e5f6a7")
        # Merge into LOCAL main and deliberately do NOT push main.
        _git(repo, "merge", "-q", "--ff-only", "wt/local-only-d4e5f6a7")
        _git(repo, "worktree", "remove", "--force", str(wt))
        _git(repo, "branch", "-D", "wt/local-only-d4e5f6a7")
        _git(repo, "fetch", "-q", "origin")

        r = _run(repo)
        assert "push origin --delete wt/local-only-d4e5f6a7" not in r.stdout, (
            "offered to delete a remote branch whose commits are not on any "
            "remote default — that is the only off-disk copy"
        )

    def test_residue_does_not_gate_the_verdict_but_is_reconciled(self, repo):
        """Presence, not a finding — and the verdict must not contradict it.

        Residue is not outstanding work, so it must not flip the exit code. But a
        run that names a leftover ref while asserting 'nothing stranded' is a
        control whose summary contradicts its own body.
        """
        self._land_and_orphan(repo, "wt/verdict-e5f6a7b8")
        r = _run(repo)
        assert r.returncode == 0, (
            f"residue must not gate the verdict; got rc={r.returncode}\n{r.stdout}"
        )
        assert "wt/verdict-e5f6a7b8" in r.stdout
        assert "stranded" not in r.stdout.split("ALL CLEAR")[-1], (
            "the verdict still claims 'nothing stranded' while naming residue"
        )

    def test_foreign_landed_branch_is_not_offered_for_deletion(self, repo):
        """Ownership boundary: non-session branches belong to their PR or owner."""
        _git(repo, "branch", "dependabot/pip/thing")
        _git(repo, "push", "-q", "origin", "dependabot/pip/thing")
        _git(repo, "branch", "-D", "dependabot/pip/thing")
        _git(repo, "fetch", "-q", "origin")
        r = _run(repo)
        assert "push origin --delete dependabot/pip/thing" not in r.stdout
