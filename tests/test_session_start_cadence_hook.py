...
class TestCadenceLib:
    ...
    def test_text_field_parses_exact_markdown_label(self, tmp_path):
        ops = tmp_path / "OPERATIONS.md"
        ops.write_text(
            "## Watches\n### RW-313. Watch\n"
            "This prose mentions Count basis: transcript-file and is not a field.\n"
            "**Count basis:** independent-context\n\n---\n"
        )
        assert (
            _lib_call(f'text_field_from_operations "{ops}" RW-313 "Count basis"')
            == "independent-context"
        )
...
class TestCountBasedWatchSurfacing:
    ...
    evidence for the current candidate row, never infer a tally from transcript
    files. A watch declares which property it means by "session"; the human applies
    its criteria and the rows remain the authoritative record.
    """
    ...
    def test_open_watch_surfaces_current_evidence_not_a_transcript_count(
        self, tmp_path
    ):
        ...
        assert "independent-context" in ctx
        assert "session_id=same-session" in ctx
        assert "source=resume" in ctx
        assert "compare" in ctx.lower() and "prior rows" in ctx.lower()
        assert "2026-08-14" in ctx
        assert "~2 session(s)" not in ctx
        assert "window may be full" not in ctx

    def test_transcript_file_count_cannot_change_watch_output(self, tmp_path):
        """BACKLOG #355: transcript files are not logical sessions."""
        proj = make_project(tmp_path, ops_body=self._ops_with_watch())
        one = self._transcripts(tmp_path, 1)
        first = context(
            run(
                source="startup",
                project_dir=proj,
                transcript_path=one / "sess0.jsonl",
        ...
        def scrub_timestamp(value):
            return re.sub(r"observed_at=\S+", "observed_at=<timestamp>", value or "")

        assert scrub_timestamp(first) == scrub_timestamp(fifth)

    def test_independent_context_message_records_repeated_id_rule(self, tmp_path):
        proj = make_project(tmp_path, ops_body=self._ops_with_watch())
        ctx = context(
            run(
                source="resume",
                project_dir=proj,
                session_id="already-in-a-row",
                env={"CADENCE_CANONICAL_REF": "0"},
            )
        )
        assert ctx is not None
        assert "repeated" in ctx.lower() and "does not qualify" in ctx.lower()
        assert "new session_id" in ctx and "candidate" in ctx.lower()

    def test_session_boundary_allows_repeated_id_without_claiming_a_tally(
        self, tmp_path
    ):
        proj = make_project(
            tmp_path, ops_body=self._ops_with_watch(basis="session-boundary")
        )
        ctx = context(
            run(
                source="resume",
        ...
        assert ctx is not None
        assert "session-boundary" in ctx
        assert "repeated session_id" in ctx.lower()
        assert "candidate" in ctx.lower()
        assert "window may be full" not in ctx

    @pytest.mark.parametrize("basis", [None, "transcript-file", "cold-ish"])
    def test_missing_or_unknown_count_basis_is_unassessable(self, tmp_path, basis):
        proj = make_project(tmp_path, ops_body=self._ops_with_watch(basis=basis))
        result = run(project_dir=proj, env={"CADENCE_CANONICAL_REF": "0"})
        assert result.returncode == 0
        ctx = context(result)
        assert ctx is not None
        assert "UNASSESSABLE" in ctx
        assert "Count basis" in ctx
    ...
    def test_missing_identity_evidence_is_unassessable(
        self, tmp_path, source, session_id
    ):
        proj = make_project(tmp_path, ops_body=self._ops_with_watch())
        result = run(
            source=source,
        ...
        assert ctx is not None
        assert "UNASSESSABLE" in ctx
        assert "session_id" in ctx and "source" in ctx
        assert "forged-reminder" not in ctx
        assert "forged-source" not in ctx
        assert "forged-id" not in ctx
    ...
    def test_compact_remains_silent_for_open_watch(self, tmp_path):
        proj = make_project(tmp_path, ops_body=self._ops_with_watch())
        ctx = context(
            run(
                source="compact",
