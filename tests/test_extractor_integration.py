...
class TestExtractAll:
    ...
    def test_extract_all_saves_content_embeddings(
        ...
        with patch("sentence_transformers.SentenceTransformer", mock_st):
            ...
            # _save_embeddings casts to float32 — the canonical storage/transport
            # dtype — even though this mock encoder returns float64
            # (np.random.rand). Guards against the stored index drifting to
            # float64 (2x size + silent upcast vs float32 query vectors).
    ...
    def test_extract_all_saves_domain_embeddings(
        ...
        with patch("sentence_transformers.SentenceTransformer", mock_st):
            ...
            # Same float32 storage contract as content embeddings (shared
            # _save_embeddings chokepoint; mock encoder returns float64).
            assert np.load(domain_embeddings_file).dtype == np.float32
...
class TestTitle10AgentOperations:
    ...
    def extract_real_index(self, request, tmp_path_factory):
        """Run the extractor against real documents once per test class.

        index_path MUST be redirected to a temp dir: extract_all() SAVES the
        index as part of the pipeline, and with the real load_settings()
        index_path this fixture silently overwrote the repo's committed
        index/ artifacts with this mock's np.random.rand vectors (norm
        ~sqrt(384/3)≈11.3, orthogonal to the canonical space) on every full
        suite run — the long-misattributed T-165 "index churn" writer, and
        the same divergence signature as the session-205→209 dead-semantic-
        search incident.
        ...
        with (
            ...
            settings.index_path = tmp_path_factory.mktemp("title10_index")
