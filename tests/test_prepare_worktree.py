...
def _advance_default(repo: Path) -> str:
    """Publish a real default-branch change after the topic has been prepared."""
    (repo / "remote-change.txt").write_text("arrived from origin/main\n")
    _git(repo, "add", "remote-change.txt")
    _git(repo, "commit", "-q", "-m", "advance default")
    _git(repo, "push", "-q", "origin", "main")
    return _git(repo, "rev-parse", "HEAD")


def _fetch_fault_env(tmp_path: Path) -> tuple[dict[str, str], Path]:
    """Fail and record any fetch while leaving other Git operations real."""
    bin_dir = tmp_path / "fetch-fault-bin"
    bin_dir.mkdir()
    marker = tmp_path / "fetch-attempts"
    wrapper = bin_dir / "git"
    wrapper.write_text(
        "#!/usr/bin/env bash\n"
        'case " $* " in *" fetch "*)\n'
        '  printf "fetch attempted\\n" >> "$AI_TEST_FETCH_LOG"\n'
        '  printf "simulated FETCH_HEAD: Operation not permitted\\n" >&2\n'
        "  exit 47 ;;\n"
        "esac\n"
    ...
    return {
        ...
        "AI_TEST_REAL_GIT": real_git,
        "AI_TEST_FETCH_LOG": str(marker),
    }, marker


def test_validate_without_verify_refresh_does_not_fetch(repo: Path, tmp_path: Path):
    target = repo.parent / "wt-no-refresh"
    assert _run(repo, *_create_args(target)).returncode == 0
    before = _git(target, "rev-parse", "HEAD")
    _advance_default(repo)
    env, marker = _fetch_fault_env(tmp_path)
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Refresh capability UNVERIFIED" in result.stdout
    assert "Refresh VERIFIED" not in result.stdout
    assert not marker.exists()
    assert _git(target, "rev-parse", "HEAD") == before


@pytest.mark.parametrize("mode", ["codex-cli-validate", "continue"])
def test_verify_refresh_cli_merges_live_default(repo: Path, mode: str):
    target = repo.parent / "wt-live-refresh"
    assert _run(repo, *_create_args(target)).returncode == 0
    expected = _advance_default(repo)
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Refresh VERIFIED in this invocation" in result.stdout
    assert _git(target, "rev-parse", "HEAD") == expected
    assert (target / "remote-change.txt").read_text() == "arrived from origin/main\n"
    assert _git(target, "status", "--porcelain") == ""


def test_verify_refresh_dirty_topic_preserves_edits_without_fetch(
    repo: Path, tmp_path: Path
):
    target = repo.parent / "wt-dirty-refresh"
    assert _run(repo, *_create_args(target)).returncode == 0
    before = _git(target, "rev-parse", "HEAD")
    (target / "README.md").write_text("unfinished user edit\n")
    (target / "untracked.txt").write_text("keep this too\n")
    env, marker = _fetch_fault_env(tmp_path)
    ...
    assert not marker.exists()
    assert _git(target, "rev-parse", "HEAD") == before
    assert (target / "README.md").read_text() == "unfinished user edit\n"
    assert (target / "untracked.txt").read_text() == "keep this too\n"


def test_verify_refresh_foreign_owner_blocks_before_helper(repo: Path, tmp_path: Path):
    owner = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(30)"])  # nosec B603
    try:
        target = repo.parent / "wt-foreign-refresh"
        ...
        assert result.returncode == 1, result.stdout + result.stderr
        assert "owner pid" in result.stdout
...
def test_verify_refresh_desktop_adopt_and_continue(repo: Path):
    target = repo.parent / "desktop-refresh"
    ...
    assert "Refresh VERIFIED in this invocation" in adopted.stdout
    expected = _advance_default(repo)

    resumed = _run(target, "continue", "--path", str(target), "--verify-refresh")

    assert resumed.returncode == 0, resumed.stdout + resumed.stderr
    assert "Refresh VERIFIED in this invocation" in resumed.stdout
    assert _git(target, "rev-parse", "HEAD") == expected
    assert (target / "remote-change.txt").read_text() == "arrived from origin/main\n"
    assert _state_values(target)["lifecycle_owner"] == "codex-desktop"


def test_verify_refresh_missing_sibling_helper_fails_without_fallback(
    repo: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    target = repo.parent / "wt-missing-helper"
    assert _run(repo, *_create_args(target)).returncode == 0
    isolated_script = tmp_path / "isolated-skills" / "start-worktree" / "prepare.sh"
    ...
    assert "refresh helper unavailable" in result.stdout
    assert str(isolated_script.parent) in result.stdout
    assert "Refresh VERIFIED" not in result.stdout
    assert "READY:" not in result.stdout
    assert not marker.exists()


def test_verify_refresh_real_helper_fetch_error_retains_diagnostic(
    repo: Path, tmp_path: Path
):
    target = repo.parent / "wt-denied-fetch"
    assert _run(repo, *_create_args(target)).returncode == 0
    env, marker = _fetch_fault_env(tmp_path)
    ...
    assert result.returncode == 2, result.stdout + result.stderr
    assert marker.read_text() == "fetch attempted\n"
    assert "simulated FETCH_HEAD: Operation not permitted" in result.stderr
    assert "helper exit 2" in result.stdout
    assert "Refresh VERIFIED" not in result.stdout
    assert "READY:" not in result.stdout


@pytest.mark.parametrize("exit_status", [1, 2, 47])
def test_verify_refresh_preserves_helper_exit_status_and_stderr(
    repo: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, exit_status: int
):
    target = repo.parent / "wt-helper-status"
    assert _run(repo, *_create_args(target)).returncode == 0
    isolated_script = tmp_path / "isolated-skills" / "start-worktree" / "prepare.sh"
    isolated_script.parent.mkdir(parents=True)
    shutil.copy2(SCRIPT, isolated_script)
    helper = isolated_script.parent.parent / "completion-sequence" / "integrate.sh"
    helper.parent.mkdir()
    helper.write_text(
        '#!/usr/bin/env bash\nprintf "specific helper failure\\n" >&2\n'
        f"exit {exit_status}\n"
    )
    monkeypatch.setattr(sys.modules[__name__], "SCRIPT", isolated_script)
    ...
    assert "specific helper failure" in result.stderr
    assert f"helper exit {exit_status}" in result.stdout
    assert "Refresh VERIFIED" not in result.stdout
    assert "READY:" not in result.stdout


@pytest.mark.parametrize("mode", ["codex-cli-validate", "continue"])
def test_verify_refresh_failed_hook_setup_prevents_helper_execution(
    repo: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mode: str
):
    target = repo.parent / "wt-broken-hooks"
    assert _run(repo, *_create_args(target)).returncode == 0
    checker = target / "scripts" / "check_git_hook_contract.py"
    checker.parent.mkdir()
    checker.write_text("raise SystemExit('hook installation failed')\n")
    _git(target, "add", "scripts/check_git_hook_contract.py")
    _git(target, "commit", "-q", "-m", "declare failing hook prerequisite")
    before = _git(target, "rev-parse", "HEAD")
    isolated_script = tmp_path / "isolated-skills" / "start-worktree" / "prepare.sh"
    isolated_script.parent.mkdir(parents=True)
    shutil.copy2(SCRIPT, isolated_script)
    ...
    marker = tmp_path / "helper-executions"
    monkeypatch.setattr(sys.modules[__name__], "SCRIPT", isolated_script)
    ...
    result = _run(
        ...
        str(os.getpid()),
        "--verify-refresh",
        env_overrides={"AI_TEST_HELPER_LOG": str(marker)},
    )

    assert result.returncode == 2, result.stdout + result.stderr
    assert "hook installation failed" in result.stderr
    assert "declared Git hooks are not installed" in result.stdout
    assert not marker.exists()
    assert _git(target, "rev-parse", "HEAD") == before
    assert "READY:" not in result.stdout
...
@pytest.mark.parametrize("mode", ["codex-cli-validate", "continue"])
def test_verify_refresh_new_hook_contract_failure_prevents_ready(repo: Path, mode: str):
    target = repo.parent / "wt-new-hook-contract"
    assert _run(repo, *_create_args(target)).returncode == 0
    checker = repo / "scripts" / "check_git_hook_contract.py"
    checker.parent.mkdir()
    checker.write_text("raise SystemExit('new hook contract cannot be installed')\n")
    _git(repo, "add", "scripts/check_git_hook_contract.py")
    _git(repo, "commit", "-q", "-m", "introduce new hook prerequisite")
    _git(repo, "push", "-q", "origin", "main")
    ...
    assert _git(target, "rev-parse", "HEAD") == expected
    assert (
        target / "scripts" / "check_git_hook_contract.py"
    ).read_text() == checker.read_text()
    assert result.returncode == 2, result.stdout + result.stderr
    assert "new hook contract cannot be installed" in result.stderr
    assert "resulting hook contract failed" in result.stdout
    assert "READY:" not in result.stdout
...
def test_create_installs_declared_pre_push_hook_before_ready(
    repo: Path, tmp_path: Path
):
    source = (
        Path(__file__).resolve().parent.parent
        / "scripts"
        / "check_git_hook_contract.py"
    )
    ...
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "declare hooks")
    _git(repo, "push", "-q", "origin", "main")

    bin_dir = tmp_path / "fake-pre-commit"
    bin_dir.mkdir()
    pre_commit = bin_dir / "pre-commit"
    pre_commit.write_text(
        "#!/usr/bin/env bash\n"
        "set -eu\n"
        'hook="$(git rev-parse --git-path "hooks/$3")"\n'
        f'printf \'#!/usr/bin/env bash\\n# File generated by pre-commit: https://pre-commit.com\\n# start templated\\nINSTALL_PYTHON={sys.executable}\\nARGS=(hook-impl --config=.pre-commit-config.yaml --hook-type=%s)\\n# end templated\\nHERE="$(cd "$(dirname "$0")" && pwd)"\\nARGS+=(--hook-dir "$HERE" -- "$@")\\nif [ -x "$INSTALL_PYTHON" ]; then\\n    exec "$INSTALL_PYTHON" -mpre_commit "${{ARGS[@]}}"\\nelif command -v pre-commit > /dev/null; then\\n    exec pre-commit "${{ARGS[@]}}"\\nelse\\n    exit 1\\nfi\\n\' "$3" > "$hook"\n'
        'chmod +x "$hook"\n'
    )
    pre_commit.chmod(0o755)

    target = repo.parent / "wt-hooks"
    result = _run(
        repo,
        *_create_args(target),
        env_overrides={"PATH": f"{bin_dir}:{os.environ['PATH']}"},
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    pre_push = Path(_git(target, "rev-parse", "--git-path", "hooks/pre-push"))
    assert pre_push.is_file()
    assert "--hook-type=pre-push" in pre_push.read_text()
