...
def test_guard_script_excluded_from_content_scan(tmp_path: Path) -> None:
    ...
    excludes them by exact relative path (CONTENT_SCAN_EXCLUDED_PATHS) so they
    don't self-flag."""
    root, fpath = make_tree(
        tmp_path,
        {
...
def test_symlink_under_allowlisted_glob_fails(tmp_path: Path) -> None:
    """A symlink under an allowlisted glob is a violation (C-1).

    Without symlink handling, the path layer resolve()s both the symlink and its
    target to the same real path, so the symlink matches the allowlist and slips
    through — and shutil.copy2 then dereferences it into the public tree. The guard
    must flag any symlink: the public tree must contain none.
    """
    root, _ = make_tree(
        tmp_path,
        {
    ...
    # Private target OUTSIDE the tree; the symlink sits at an allowlisted path.
    outside = tmp_path / "outside_secret.txt"
    outside.write_text("AcmeHotelCo private domain content\n", encoding="utf-8")
    link = root / "src" / "ai_governance_mcp" / "leak.py"
...
def test_symlink_named_like_skip_dir_still_flagged(tmp_path: Path) -> None:
    """A symlink whose name collides with a skip-dir (.git/__pycache__) must still
    be flagged — the symlink check must precede the skip-dir prune, or the stated
    'no symlinks' invariant is silently false for those names."""
    root, _ = make_tree(tmp_path, {"documents/constitution.md": "# C\n"})
    secret = tmp_path / "secret_dir"
    secret.mkdir()
    (secret / "domain.md").write_text("AcmeHotelCo\n", encoding="utf-8")
    (root / ".git").symlink_to(secret, target_is_directory=True)
    result = run_guard(root)
    assert result.returncode == 2, result.stderr
    assert "symlink" in result.stderr.lower()
...
def test_excluded_name_at_noncanonical_path_is_scanned(tmp_path: Path) -> None:
    """The content-scan exclusion must key on the exact relative PATH, not the
    basename (C-2). A file merely NAMED like the guard, placed elsewhere under an
    allowlisted glob, must still be content-scanned."""
    root, fpath = make_tree(
        tmp_path,
        {
            "documents/constitution.md": "# C\n",
            # Named like the guard, but NOT at the canonical scripts/ path:
            "src/check-public-release.py": 'x = "leak AcmeHotelCo here"\n',
        },
        forbidden=FORBIDDEN,
    )
    result = run_guard(root, forbidden=fpath)
    assert result.returncode == 2, result.stderr
    assert "forbidden-content" in result.stderr
    assert "src/check-public-release.py" in result.stderr


def test_non_utf8_content_fails_closed(tmp_path: Path) -> None:
    """A marker stored in a non-UTF-8 encoding must fail closed (H-1), not be
    silently mangled past the regex by errors='replace'."""
    root, fpath = make_tree(
        tmp_path,
        {"documents/constitution.md": "# C\n"},
        forbidden=FORBIDDEN,
    )
    # Overwrite an allowlisted text file with UTF-16 bytes containing a marker.
    leak = root / "tests" / "test_x.py"
    leak.parent.mkdir(parents=True, exist_ok=True)
    leak.write_bytes("note = 'AcmeHotelCo'\n".encode("utf-16"))
    result = run_guard(root, forbidden=fpath)
    assert result.returncode == 2, result.stderr
    assert "forbidden-content" in result.stderr
    assert "tests/test_x.py" in result.stderr
...
def test_mistakenly_allowlisted_index_still_denied(tmp_path: Path) -> None:
    """index/ embeds domain plaintext and must NEVER ship, even if an allowlist
    authoring mistake adds it (the default-deny layer would otherwise pass it)."""
    root, _ = make_tree(
        tmp_path,
        {
            "documents/constitution.md": "# C\n",
        ...
        allowlist=ALLOWLIST + "index/**/* | MISTAKE: index must never be public\n",
    )
    result = run_guard(root)
    assert result.returncode == 2, result.stderr
    assert "index/content_embeddings.npy" in result.stderr
