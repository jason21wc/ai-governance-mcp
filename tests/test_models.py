...
@pytest.mark.parametrize(
    "index_loaded,semantic_available", [(True, True), (True, False), (False, False)]
)
def test_assessment_serializes_retrieval_coverage(index_loaded, semantic_available):
    assessment = GovernanceAssessment(
        action_reviewed="Rename fixture",
        assessment=AssessmentStatus.REVIEW,
        confidence=ConfidenceLevel.LOW,
        rationale="Coverage fixture",
        index_loaded=index_loaded,
        semantic_available=semantic_available,
        retrieval_warning=None if semantic_available else "Incomplete coverage",
    )
    output = assessment.model_dump(mode="json")
    assert output["index_loaded"] is index_loaded
    assert output["semantic_available"] is semantic_available
    assert output["retrieval_warning"] == (
        None if semantic_available else "Incomplete coverage"
    )
    assert output["compliance_evaluation"] == []
