...
class TestGetEngine:
    ...
    setup. Kept as separate classes per CFR §5.2.9 "when NOT to
...
class TestValidateLogPath:
    ...
    def test_validate_log_path_accepts_home_directory(self, monkeypatch):
        """Exercises the HOME branch specifically.

        Under conftest.isolate_home, `$HOME` is inside the system temp root, so
        `_validate_log_path`'s is_in_temp branch would accept this path and the
        is_in_home branch would go untested. Point HOME outside temp.
        """
        from pathlib import Path

        from ai_governance_mcp.server import _validate_log_path

        outside = Path(__file__).resolve().parent.parent / ".pytest-home"
        outside.mkdir(exist_ok=True)
        monkeypatch.setenv("HOME", str(outside))

        # Should not raise
        _validate_log_path(outside / ".ai-governance" / "logs" / "test.jsonl")
...
class TestGovernanceReminderCENudge:
    ...
    def test_reminder_retains_governance_check(self):
        ...
        # Honest labeling: an S-Series *principle* is the veto; a bare keyword
        # match is advisory (not a principle veto). See governance.py rationale
        # split + _constants.py SERVER_INSTRUCTIONS qualification.
        assert "S-Series *principle* = veto" in GOVERNANCE_REMINDER
        assert "bare keyword match is advisory" in GOVERNANCE_REMINDER
...
class TestServerDiscoveryInstructions:
    """Protect the complete MCP initialization message against observed host clipping."""

    def test_initialization_delivers_complete_instructions_within_host_window(self):
        from ai_governance_mcp.server import SERVER_INSTRUCTIONS, server

        delivered = server.create_initialization_options().instructions
        assert delivered == SERVER_INSTRUCTIONS
        # Compatibility with the observed 2,048-character host cutoff, not an
        # assertion that MCP defines one universal tool/instruction token limit.
        assert len(delivered.encode("utf-8")) <= 2048
        assert delivered.endswith(
            "get_principle('meta-method-behavioral-floor-directives')."
        ), "The detail-retrieval path must survive the same delivery window"

    def test_assessment_and_retrieval_limits_stay_visible(self):
        ...
        for required in (
            ...
            "REVIEW:",
            "ESCALATE: stop",
            "S-Series principles are vetoes",
            "genuine/floor also require escalation",
            "benign means REVIEW",
            "unavailable adjudication are heuristic warnings",
            "Never bypass host safety or approval controls",
            "index_loaded",
            "semantic_available",
            "retrieval_warning",
            "does not revoke existing authorization",
        ):
            assert required in SERVER_INSTRUCTIONS

    @pytest.mark.asyncio
    async def test_discovery_references_registered_tools(self):
        from ai_governance_mcp.server import SERVER_INSTRUCTIONS, list_tools
        ...
        assert "query_project in the Context Engine MCP" in SERVER_INSTRUCTIONS

    def test_read_only_and_authorized_work_do_not_require_reconfirmation(self):
        from ai_governance_mcp.server import SERVER_INSTRUCTIONS

        assert (
            "except read-only exploration, non-sensitive questions"
            in SERVER_INSTRUCTIONS
        )
        assert "Continue already-authorized work" in SERVER_INSTRUCTIONS
        assert "not a manufactured permission question" in SERVER_INSTRUCTIONS
        # Desktop clients without project loaders still receive concise style guidance.
        assert "plain language for the reader" in SERVER_INSTRUCTIONS
        assert "around 200 words" in SERVER_INSTRUCTIONS
        assert "ask in prose only when" in SERVER_INSTRUCTIONS
