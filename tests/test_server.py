...
class TestRequiredActionsCEReference:
    ...
    def test_required_actions_include_reference_search_and_capture(self):
        # The Reference Library is cross-project: every connected project's session must
        # be told to BOTH reuse (search_references) and contribute (capture_reference).
        # Capture is the directive that stops the library's signal from being
        # single-project-biased. Guards against silent removal of either half.
        from ai_governance_mcp.server import SERVER_INSTRUCTIONS

        assert "search_references" in SERVER_INSTRUCTIONS
        assert "capture_reference" in SERVER_INSTRUCTIONS
        assert "6. **Search for precedent**" in SERVER_INSTRUCTIONS
        assert "7. **Capture reusable precedent**" in SERVER_INSTRUCTIONS

    def test_server_instructions_carry_communication_floor_for_desktop(self):
        # plain-language (#74) and default-register (§7.14) must ride SERVER_INSTRUCTIONS —
        # the only ai-governance advisory channel that reaches Claude Desktop at MCP session
        # start (Desktop has no hooks and does not load CLAUDE.md). freeform already had a
        # "Conversation Style" section; these close the matching Desktop gap so all three
        # communication-register directives reach the model on the Desktop surface too.
        from ai_governance_mcp.server import SERVER_INSTRUCTIONS

        assert (
            "Conversation Style" in SERVER_INSTRUCTIONS
        )  # freeform-dialogue (pre-existing)
        assert "Plain Language" in SERVER_INSTRUCTIONS
        assert "Audience Calibration" in SERVER_INSTRUCTIONS
        assert "Default Register" in SERVER_INSTRUCTIONS
