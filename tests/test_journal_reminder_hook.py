...
trigger: on each user prompt it checks new activity since observed memory-byte
freshness or a request-scoped, self-attested analysis checkpoint. An attempted
tool call is not a write, and completed no-change analysis needs no memory edit.
...
def make_git_project(tmp_path, marker="AGENTS.md", subdir="src"):
    ...
    # Freshness requires a known unchanged HEAD; an unborn branch is UNKNOWN,
    # deliberately conservative rather than proof no integration occurred.
    subprocess.run(
        [
            "git",
            "-C",
...
def run_hook(
    ...
    e["JOURNAL_STATE_DIR"] = str(
        (
            Path(transcript).parent
            if transcript is not None
            else Path(project_dir).parent
        )
        / "journal-state"
    )
    if env:
...
class TestJournalReminderHook:
    ...
    def test_attempted_edit_alone_does_not_suppress_initial_analysis(self, tmp_path):
        proj = make_project(tmp_path)
    ...
    def test_observed_memory_change_suppresses_new_request(self, tmp_path):
        proj = make_project(tmp_path)
        tx = make_transcript(tmp_path, lines=300)
        assert context(run_hook(proj, tx)) is not None
        make_transcript(tmp_path, lines=700)
        (proj / "SESSION-STATE.md").write_text("persisted during this interval\n")
...
class TestSessionRootResolution:
    ...
    def test_fires_from_subdirectory_of_repo(self, tmp_path):
        ...
        # Same logical checkout+transcript: subdirectory must reuse its request.
        assert context(run_hook(repo / "src", tx)) is None
        make_transcript(tmp_path, lines=700)
        assert "JOURNAL" in context(run_hook(repo / "src", tx))
    ...
    def test_subdirectory_still_respects_recent_memory_write(self, tmp_path):
        ...
        """
        repo = make_git_project(tmp_path)
        written = make_transcript(tmp_path, lines=300)
        assert context(run_hook(repo / "src", written)) is not None
        make_transcript(tmp_path, lines=700)
        (repo / "SESSION-STATE.md").write_text("persisted\n")
    ...
    def test_completion_command_from_real_directive_handles_no_change(self, tmp_path):
        repo = make_git_project(tmp_path)
        tx = make_transcript(tmp_path, lines=300)
        ctx = context(run_hook(repo, tx))
        assert "SELF-ATTESTATION" in ctx
        command = ctx.split("outcome with: ", 1)[1].split(" --outcome no_change", 1)[0]
        result = subprocess.run(
            shlex.split(command) + ["--outcome", "no_change"],
            capture_output=True,
            text=True,
            timeout=5,
        )
        assert result.returncode == 0 and json.loads(result.stdout)["accepted"]
        assert context(run_hook(repo, tx)) is None
        make_transcript(tmp_path, lines=700)
        assert context(run_hook(repo, tx)) is not None

    def test_degraded_state_is_explicit_and_does_not_block(self, tmp_path):
        repo = make_git_project(tmp_path)
        tx = make_transcript(tmp_path, lines=300)
        bad = tmp_path / "state-file"
        bad.write_text("not a directory")
        result = run_hook(repo, tx, env={"JOURNAL_STATE_DIR": str(bad)})
        assert result.returncode == 0
        assert "State tracking unavailable" in context(result)
        assert "--request-id" not in context(result)

    def test_busy_checker_does_not_emit_untracked_duplicate(self, tmp_path):
        import fcntl
        ...
        assert context(run_hook(repo, tx)) is not None
        make_transcript(tmp_path, lines=700)
        lock = next((tmp_path / "journal-state").glob("*.lock"))
        with lock.open("a") as f:
            fcntl.flock(f, fcntl.LOCK_EX)
            result = run_hook(repo, tx)
        assert result.returncode == 0 and context(result) is None
        assert "check is in progress" in result.stderr
        assert context(run_hook(repo, tx)) is not None
