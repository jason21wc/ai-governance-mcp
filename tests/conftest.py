...
def _resolve_launch_cache(environ):
    """Resolve the model source BEFORE isolate_home changes library defaults."""
    default = Path(environ.get("HOME", str(Path.home()))) / ".cache"
    hf_home = environ.get(
        "HF_HOME", str(Path(environ.get("XDG_CACHE_HOME", default)) / "huggingface")
    )
    cache = (
        environ.get("SENTENCE_TRANSFORMERS_HOME")
        or environ.get("HF_HUB_CACHE")
        or environ.get("HUGGINGFACE_HUB_CACHE")
        or str(Path(hf_home) / "hub")
    )
    return Path(os.path.expandvars(cache)).expanduser().resolve()


# Collection happens before any function-scoped HOME mutation. Capture paths,
# not huggingface_hub's import-cached constants (another test may import those
# while HOME is isolated). No model is copied unless the named fixture is used.
_LAUNCH_SOURCE_CACHE = _resolve_launch_cache(os.environ)
_LAUNCH_MODEL = Settings().embedding_model
_LAUNCH_LEGACY_ENV = {
...
def _validate_launch_cache_policy(environ, source):
    # ST's explicit cache_folder unifies metadata and transformer weights.
    if environ.get("SENTENCE_TRANSFORMERS_HOME"):
        return
    for key in (
        "TRANSFORMERS_CACHE",
        "PYTORCH_TRANSFORMERS_CACHE",
        ...
        if environ.get(key):
            legacy = Path(os.path.expandvars(environ[key])).expanduser().resolve()
            if legacy != source:
                raise RuntimeError(
                    f"Launch-test model cache is split: {key}={legacy}, Hub={source}. "
                    "Set SENTENCE_TRANSFORMERS_HOME to one unified model cache, "
                    "then provision it with the governance extractor before pytest."
                )
            return
...
def _seed_launch_cache(source, destination, model):
    ...
    The extractor preceding pytest in CI/public builds bootstraps the model.
    Standalone launch tests require that prerequisite too; no hidden downloads.
    """
    model_dir = "models--" + model.replace("/", "--")
    origin = source / model_dir
    try:
        ref = origin / "refs" / "main"
        if not origin.resolve().is_relative_to(
            source.resolve()
        ) or not ref.resolve().is_relative_to(origin.resolve()):
            raise ValueError("model cache/ref link escapes its cache")
        ...
        if not any(
            ...
            raise ValueError("incomplete model snapshot: missing safetensors weights")
        target = destination / model_dir / "snapshots" / revision
        shutil.copytree(snapshot, target, symlinks=False)
        refs = destination / model_dir / "refs"
        refs.mkdir(parents=True)
        (refs / "main").write_text(revision)
    except (OSError, ValueError) as exc:
        raise RuntimeError(
            f"Launch-test model prerequisite unavailable: {model} in {source}. "
            "Provision the model with the governance extractor before pytest; "
            "launch tests consume a disposable copy offline and never download. "
            f"Cause: {exc}"
        ) from exc
    return destination
...
def launch_model_cache(tmp_path_factory):
    """Opt-in, private model artifacts; unrelated tests keep their own policy."""
    _validate_launch_cache_policy(_LAUNCH_LEGACY_ENV, _LAUNCH_SOURCE_CACHE)
    return _seed_launch_cache(
        _LAUNCH_SOURCE_CACHE,
        tmp_path_factory.mktemp("launch-model-cache"),
        _LAUNCH_MODEL,
    )
