...
class TestContextEngineRetrievalQuality:
    ...
    def setup_engine(self, request, tmp_path_factory):
        """Build one isolated Context Engine index for this benchmark class.

        The global ``isolate_home`` fixture gives every test a fresh HOME. Using
        default storage here therefore rebuilt the entire project index once per
        test. Keep the index independent of both HOME and machine state, but share
        that explicit temporary storage across the five read-only benchmark cases.
        """
        ...
        storage = FilesystemStorage(
            base_path=tmp_path_factory.mktemp("context-engine-quality") / "indexes"
        )
    ...
    def test_individual_queries(self):
        ...
        for case in self.benchmark["test_cases"]:
    ...
    def test_mrr(self):
        ...
        target = self.benchmark["metrics"]["target_mrr"]
    ...
    def test_recall_at_5(self):
        ...
        for case in self.benchmark["test_cases"]:
            result = self.results[case["id"]]
            result_files = [r.chunk.source_path for r in result.results[:5]]
        ...
        target = self.benchmark["metrics"]["target_recall_at_5"]
    ...
    def test_recall_at_10(self):
        ...
        target = self.benchmark["metrics"]["target_recall_at_10"]
