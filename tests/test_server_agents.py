...
class TestInstallAgent:
    ...
    @pytest.mark.asyncio
    async def test_install_agent_invalid_scope(self, tmp_path, monkeypatch):
        """install_agent should reject invalid scope values."""
        from ai_governance_mcp.server import _handle_install_agent

        monkeypatch.chdir(tmp_path)

        result = await _handle_install_agent(
            {"agent_name": "orchestrator", "scope": "Project"}
        )

        assert len(result) == 1
        response = json.loads(extract_json_from_response(result[0].text))
        assert response["error_code"] == "INVALID_SCOPE"
        assert "Project" in response["message"]
...
class TestUninstallAgent:
    ...
    @pytest.mark.asyncio
    async def test_uninstall_agent_invalid_scope(self, tmp_path, monkeypatch):
        """uninstall_agent should reject invalid scope values."""
        from ai_governance_mcp.server import _handle_uninstall_agent

        monkeypatch.chdir(tmp_path)

        result = await _handle_uninstall_agent(
            {"agent_name": "orchestrator", "scope": "global"}
        )
...
class TestStandingStance:
    """Every agent definition carries the standing stance (session-266 D1).

    THE ORIGIN IS THE POINT. The human observed that pasting
    "dogfood ai-governance, use systemic thinking, avoid anchor bias" into each
    dispatch prompt puts the instruction in the wrong place: it makes the stance
    depend on the dispatcher remembering to type it, and the dispatcher is the
    party whose framing the subagent exists to check. The stance belongs in the
    DEFINITION. It was agreed twice in session-265, never done, and never filed —
    found by a fresh-context audit of the session transcripts in session-266.

    Derived from the directory, never enumerated: an agent added tomorrow is
    covered the day it lands. A hand-listed set here would reproduce the defect
    this session closed four times.
    """
    ...
    def _agent_files(self):
        root = Path(__file__).resolve().parent.parent / "documents" / "agents"
        return sorted(root.glob("*.md"))

    def test_discovery_finds_agents(self):
        """An empty glob would make every case below pass vacuously."""
        assert self._agent_files(), "no agent definitions discovered"

    def test_every_agent_carries_the_stance(self):
        missing = []
        for f in self._agent_files():
            text = f.read_text()
            absent = [m for m in self.REQUIRED if m not in text]
            if absent:
                missing.append(f"{f.name}: missing {absent}")
        assert missing == [], (
            "agent definitions without the standing stance — the stance must live "
            "in the definition, not in whatever the dispatcher remembers to "
            "type:\n" + "\n".join(missing)
        )

    def test_stance_omits_clauses_agents_cannot_act_on(self):
        """Scope discipline, asserted so it is not quietly widened later.

        Two clauses of the session-level frame are deliberately excluded: the
        dispatch clause (most agents cannot spawn anything) and the
        call-the-governance-tools clause (a dead pointer for read-only tool
        grants). An instruction the reader cannot act on trains readers to skim.
        """
        for f in self._agent_files():
            stance = f.read_text().split("## Standing Stance", 1)[1].split("\n## ")[0]
            # DIRECTIVES only — the trailing scope note QUOTES the excluded
            # clauses in order to explain the exclusion, so matching the whole
            # block flags its own explanation. Third time this session a matcher
            # could not tell a quotation from an instruction; here the fix is to
            # narrow what is scanned rather than to write a cleverer pattern.
            directives = stance.split("*Scope note:", 1)[0]
            assert "leverage skills" not in directives.lower(), f.name
            assert "call the governance tools" not in directives.lower(), f.name
            assert "*Scope note:" in stance, (
                f"{f.name}: the scope note is what records WHY two clauses are "
                "omitted; without it a later editor re-adds them as an oversight"
            )
