...
The hook is documented at:
  ...
  BACKLOG.md #49 (CLOSED — design rationale in git history)
...
BACKLOG #230(b) — substring false positive fix (5 new tests across 1 class):
  - pytest-as-argument allowed: grep/rg/find with 'pytest' as arg, not invocation (4 parametrized)
  - Regression lock: chained grep+pytest still denied when real pytest segment present
"""
...
class TestSafeSubsetPatternsAllow:
    ...
    @pytest.mark.parametrize(
        "command",
        [
            'pytest "tests/test_fake_module.py" -q',
            "pytest 'tests/test_fake_module.py' -q",
            'pytest -q "tests/test_fake_module.py"',
            'pytest "tests/test_fake_module.py::TestFakeClass" -v',
            'python3 -m pytest "tests/test_fake_module.py"',
        ],
        ids=["dquote", "squote", "dquote-last", "dquote-class", "python-m-dquote"],
    )
    def test_a_quoted_test_path_is_still_a_targeted_run(self, command, tmp_path):
        """A QUOTED path is targeted too — the terminator class forgot quotes.

        The single-file matcher required the path to be followed by whitespace
        or end-of-string. A closing quote is neither, so `pytest "tests/test_x.py"`
        was classified as a bare full-suite run and DENIED. Observed live in
        session-272 when the gate blocked a routine targeted run.

        This is a false positive in the direction that matters most: quoting a
        path is ordinary, the deny is confusing (the command plainly names one
        file), and the only escape is a bypass env var. A gate that fires on
        normal work gets bypassed within days — the failure mode T-143 already
        cost this repo once.

        Fixing it cannot under-block: the presence of a `tests/test_*.py` path is
        what makes a run targeted, and that fact does not change with quoting.
        """
        home = make_fake_daemon_home(tmp_path, heartbeat_age_seconds=30)
        exit_code, response = run_hook(command, base_dir_override=home)
        assert is_allow(response, exit_code), (
            f"a quoted single-file path was denied as a full-suite run: {command}"
        )
...
class TestPytestSubstringFalsePositiveAllow:
    """Commands where 'pytest' appears as an argument (not in executable position)
    but is followed by a space, causing the top-level substring detector to fire.

    Before the fix, these fell through to environment checks and were denied when
    daemon/torch processes were present. The segment-level first-token analysis
    correctly finds 0 pytest segments, but the missing early exit let the command
    continue to the environment risk checks.

    Distinct from TestNonPytestCommandsAllow: those commands dodge the top-level
    detector entirely (pytest at end-of-string has no trailing space, or is inside
    quotes that get stripped). These commands ENTER the gate and rely on the
    _PYTEST_SEGMENTS_SEEN=0 early exit to allow them.
    """
    ...
    def test_pytest_as_argument_allowed(self, command, tmp_path):
        """Commands that mention 'pytest' as an argument, not as an invocation,
        must be allowed even with a fresh daemon running.

        Covers: FM-HOOK-OOM-GATE-SUBSTRING-FP
        """
        home = make_fake_daemon_home(tmp_path, heartbeat_age_seconds=30)
        exit_code, response = run_hook(command, base_dir_override=home)
        assert is_allow(response, exit_code), (
            f"'{command}' has pytest as argument, not invocation — "
            f"should be allowed, got {response}"
        )

    def test_chained_grep_pytest_then_real_pytest_still_denied(self, tmp_path):
        """Regression lock: `grep pytest Makefile && pytest tests/` must still
        deny the pytest segment when daemon is alive.

        Covers: FM-HOOK-OOM-GATE-SUBSTRING-FP
        """
        home = make_fake_daemon_home(tmp_path, heartbeat_age_seconds=30)
        exit_code, response = run_hook(
            "grep pytest Makefile && pytest tests/", base_dir_override=home
        )
        assert is_deny(response), (
            f"Chained grep+pytest: the pytest segment should still be evaluated "
            f"and denied with fresh daemon, got {response}"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "timeout --signal=KILL 60 pytest tests/",
        ],
        ids=[
            "timeout-duration",
            "time-bare",
    ...
    def test_wrapped_pytest_still_denied(self, command, tmp_path):
        """Command wrappers (timeout, time, nice) preceding a real pytest
        invocation must still be recognized as pytest and denied when daemon
        is alive. Without wrapper-skipping in _segment_is_pytest, the early
        exit would allow these.
        ...
        exit_code, response = run_hook(command, base_dir_override=home)
        assert is_deny(response), (
            f"Wrapped pytest '{command}' should still be denied with fresh "
            f"daemon, got {response}"
        )
