...
class PointerExpectation:
    """Required result tokens and independently declared document owners."""

    id_substrings: tuple[str, ...]
    owners: tuple[str, ...]


# What each pointer MUST surface and where that contract is declared. Result tokens
# are substrings matched against returned ids (principles, methods, references).
# Ownership must not be inferred from discovery: a removed pointer would then erase
# its own obligation. Every surviving owner is checked, including shared pointers.
#
...
EXPECTATIONS: dict[str, PointerExpectation] = {
    ...
    # CLAUDE.md imports AGENTS.md; the shared loader owns this literal pointer.
    # Keep ownership explicit so deleting it from either surviving owner fails.
    "behavioral floor directives": PointerExpectation(
        id_substrings=("behavioral-floor", "floor-directive"),
        owners=(
            "AGENTS.md",
            ".claude/skills/compliance-review/procedure.md",
        ),
    ...
    "reference library curation §15.4": PointerExpectation(
        id_substrings=("intake-paths", "bloat-prevention", "reference-entry"),
        owners=("CLAUDE.md",),
    ),
    ...
    "framework recovery": PointerExpectation(
        id_substrings=("coding-method-recovery", "recovery-procedure"),
        owners=("AGENTS.md",),
    ),
    # compliance-review Check 6 canary.
    "which principle governs validation before action?": PointerExpectation(
        id_substrings=("verification-validation",),
        owners=(".claude/skills/compliance-review/procedure.md",),
    ),
}
...
def test_every_discovered_pointer_has_a_declared_expectation():
    ...
    invalid_owners = {
        query: expectation.owners
        for query, expectation in EXPECTATIONS.items()
        if not expectation.owners or set(expectation.owners) - set(POINTER_DOCS)
    }
    assert not invalid_owners, (
        f"EXPECTATIONS must declare nonempty owners from POINTER_DOCS: {invalid_owners}"
    )
    unowned = sorted(
        (query, doc)
        for query, docs in discovered.items()
        for doc in docs
        if doc not in EXPECTATIONS[query].owners
    )
    assert not unowned, f"these pointer sources lack declared ownership: {unowned}"
...
def test_no_stale_expectations():
    ...
    stale = sorted(
        (query, doc)
        for query, expectation in EXPECTATIONS.items()
        for doc in expectation.owners
        if (REPO_ROOT / doc).exists() and doc not in discovered.get(query, ())
    )
    assert not stale, (
        f"EXPECTATIONS names pointers missing from their surviving owners: {stale}"
    )


@pytest.fixture
def pointer_source_tree(tmp_path, monkeypatch):
    """Independent source fixtures: do not derive owners from EXPECTATIONS."""
    queries_by_doc = {
        "CLAUDE.md": ("reference library curation §15.4",),
        "AGENTS.md": ("behavioral floor directives", "framework recovery"),
        ".claude/skills/compliance-review/procedure.md": (
            "behavioral floor directives",
            "which principle governs validation before action?",
        ),
    }
    for doc, queries in queries_by_doc.items():
        path = tmp_path / doc
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("\n".join(f"query_governance({query!r})" for query in queries))
    monkeypatch.setattr(sys.modules[__name__], "REPO_ROOT", tmp_path)
    return tmp_path


def _check_stale_expectations_without_global_skip():
    try:
        test_no_stale_expectations()
    except pytest.skip.Exception as exc:
        pytest.fail(f"missing sources must not skip checks of surviving owners: {exc}")


def _remove_fixture_pointer(root, doc, query):
    path = root / doc
    path.write_text(path.read_text().replace(f"query_governance({query!r})", ""))


def test_stale_check_exempts_only_absent_compliance_owner(pointer_source_tree):
    (pointer_source_tree / ".claude/skills/compliance-review/procedure.md").unlink()
    _check_stale_expectations_without_global_skip()
...
@pytest.mark.parametrize(
    ...
    [
        ("CLAUDE.md", "reference library curation §15.4"),
        ("AGENTS.md", "framework recovery"),
    ],
)
def test_stale_check_catches_surviving_owner_after_source_omission(
    pointer_source_tree, doc, query
):
    """Covers: FM-DOC-POINTER-OWNER-SCOPE"""
    (pointer_source_tree / ".claude/skills/compliance-review/procedure.md").unlink()
    _remove_fixture_pointer(pointer_source_tree, doc, query)
    with pytest.raises(AssertionError, match=re.escape(query)):
        _check_stale_expectations_without_global_skip()


@pytest.mark.parametrize(
    "doc", ("AGENTS.md", ".claude/skills/compliance-review/procedure.md")
)
def test_stale_check_catches_pointer_removed_from_one_shared_owner(
    pointer_source_tree, doc
):
    """Covers: FM-DOC-POINTER-OWNER-SCOPE"""
    query = "behavioral floor directives"
    _remove_fixture_pointer(pointer_source_tree, doc, query)
    with pytest.raises(AssertionError, match=re.escape(doc)):
        _check_stale_expectations_without_global_skip()
...
def test_stale_check_catches_removal_in_complete_tree(pointer_source_tree, doc, query):
    """Covers: FM-DOC-POINTER-OWNER-SCOPE"""
    _remove_fixture_pointer(pointer_source_tree, doc, query)
    with pytest.raises(AssertionError, match=re.escape(query)):
        _check_stale_expectations_without_global_skip()
...
def test_stale_check_accepts_complete_tree(pointer_source_tree):
    _check_stale_expectations_without_global_skip()
...
def test_documented_pointer_resolves(query, real_settings, request):
    ...
    expected = EXPECTATIONS[query].id_substrings
