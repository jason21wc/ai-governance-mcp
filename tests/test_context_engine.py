...
class TestProjectIdValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "./a",  # explicit current-dir traversal prefix
            "a/..",  # embedded traversal
            "a/../b",  # middle-of-string traversal
        ],
    )
    def test_rejects_traversal_patterns(self, bad_input):
        """Every path-traversal variant must raise ValueError from `_validate_project_id`.

        Parametrized strict-match coverage of the traversal rejection taxonomy.
        ...
        regex change like `^[0-9a-f.]{1,64}$`. This test catches that class:
        each input contains a `..` or `/` sequence that the FM description
        names explicitly.
    ...
    @pytest.mark.parametrize(
        "bad_input",
        [
            ".",  # single dot — regex-loosening canary
            "~/foo",  # home-dir expansion (different attack class)
            "a b",  # whitespace
        ],
    )
    def test_rejects_nonhex_regex_breadth(self, bad_input):
        """Non-traversal invalid inputs that demonstrate regex breadth.

        These inputs are NOT path-traversal patterns per FM-PROJECT-ID-PATH-TRAVERSAL
        — they're general non-hex inputs included as regex-loosening canaries.
        If the regex were loosened to allow dots, tildes, or whitespace, these
        would slip through silently while the strict-traversal patterns above
        would still catch some (e.g., `..` would still fail any regex without `.`).

        Not annotated: this is general-input hardening, not a registered FM.
        """
        with pytest.raises(ValueError, match="hex characters only"):
            _validate_project_id(bad_input)
...
class TestChildHeadingSummary:
    """Tests for parent-chunk child heading summary enrichment."""
    ...
    def test_parent_chunk_includes_child_heading_summary(self, tmp_path):
        """Parent chunk with >=3 children includes [Contains N sections: ...] summary."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        f.write_text(
            "## Parent\n\nIntro text.\n\n"
        ...
        assert (
            "[Contains 4 sections: Child One | Child Two | Child Three | Child Four]"
            in parent_chunk.content
        )

    def test_no_summary_below_threshold(self, tmp_path):
        """Parent chunk with <3 children does NOT include child summary."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        f.write_text(
            "## Parent\n\nIntro text.\n\n"
        ...
        chunks = conn.parse(f, project_root=tmp_path)
        parent_chunk = chunks[0]
        assert "[Contains" not in parent_chunk.content

    def test_child_chunks_have_position_indicator(self, tmp_path):
        """Child chunks include (section N of M) in breadcrumb when parent has >=3 children."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        f.write_text(
            "## Parent\n\nIntro text.\n\n"
        ...
        chunks = conn.parse(f, project_root=tmp_path)
        # Child chunks are chunks[1], chunks[2], chunks[3]
        assert "(section 1 of 3)" in chunks[1].content
        assert "(section 2 of 3)" in chunks[2].content
        assert "(section 3 of 3)" in chunks[3].content

    def test_deeply_nested_children(self, tmp_path):
        """Works for ### parent with #### children (not just h2->h3)."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        f.write_text(
            "## Top\n\nTop intro.\n\n"
        ...
        parent_chunk = [c for c in chunks if c.heading == "Parent"][0]
        assert (
            "[Contains 3 sections: Deep One | Deep Two | Deep Three]"
            in parent_chunk.content
        )

    def test_heading_text_cleaned_and_capped(self, tmp_path):
        """Markdown formatting stripped; truncated at ' *(' delimiter; capped at 60 chars."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        long_name = "A" * 65
        f.write_text(
            "## Parent\n\nIntro.\n\n"
            "### **Bold Child** with extras\n\nBody.\n\n"
            "### `Code Child` stuff\n\nBody.\n\n"
            f"### {long_name}\n\nBody.\n\n"
        ...
        chunks = conn.parse(f, project_root=tmp_path)
        parent_chunk = chunks[0]
        summary = parent_chunk.content
        # Bold markers stripped
        assert "**" not in summary.split("[Contains")[1].split("]")[0]
        # Backticks stripped
        assert "`" not in summary.split("[Contains")[1].split("]")[0]
        # Long name capped at 60 with ...
        assert long_name not in summary
        assert "A" * 57 + "..." in summary
        # Truncated at ' *(' delimiter
        assert "canonical" not in summary
        assert "Legacy" not in summary
        assert "Name" in summary

    def test_child_summary_after_heading_not_prefix(self, tmp_path):
        """[Contains ...] line appears AFTER the heading line, not in the breadcrumb prefix."""
        conn = self._make_connector()
        f = tmp_path / "doc.md"
        f.write_text(
            "## Parent\n\nIntro text.\n\n"
        ...
        chunks = conn.parse(f, project_root=tmp_path)
        parent_content = chunks[0].content
        lines = parent_content.split("\n")
        # Find the breadcrumb line and the contains line
        breadcrumb_idx = next(
            idx for idx, ln in enumerate(lines) if ln.startswith("[") and ">" in ln
        )
        contains_idx = next(
            idx for idx, ln in enumerate(lines) if ln.startswith("[Contains")
        )
        # Contains line must be AFTER breadcrumb (not on the same line or before)
        assert contains_idx > breadcrumb_idx
        # The heading line (## Parent) should be between breadcrumb and contains
        heading_idx = next(
            idx for idx, ln in enumerate(lines) if ln.strip() == "## Parent"
        )
        assert breadcrumb_idx < heading_idx < contains_idx
...
class TestIndexer:
    ...
    def test_build_bm25_index_tokenization(self):
        ...
        Per CFR §5.2.9 (BACKLOG #122 Case 5).
        """
        from ai_governance_mcp.context_engine.indexer import Indexer
...
class TestProjectManager:
    ...
    def test_bm25_search_tokenization(self):
        ...
        sites, distinct failure modes — kept per CFR §5.2.9 "when NOT to
...
class TestFileWatcher:
    ...
    def test_flush_changes_handles_callback_error(self, tmp_path):
        ...
        try:
            ...
            # A failed callback deliberately schedules a retry. Stop it so this
            # unit test does not leak background work into later tests.
            watcher.stop()
...
class TestEnvIgnorePatterns:
    ...
    def test_env_wildcard_in_security_patterns(self):
        from ai_governance_mcp.context_engine.indexer import SECURITY_IGNORE_PATTERNS

        assert ".env*" in SECURITY_IGNORE_PATTERNS
    ...
    def test_contextignore_negation_cannot_override_security_patterns(self, tmp_path):
        """#319: a user `!.env` in .contextignore must NOT re-include secrets."""
        from ai_governance_mcp.context_engine.indexer import Indexer
        ...
        assert spec.match_file("id_rsa")
        assert spec.match_file("secret.pem")


class TestWorktreeIgnorePattern:
    """In-repo Claude Code worktrees must not be indexed (they duplicate the parent tree)."""

    def test_worktrees_in_defaults(self):
        from ai_governance_mcp.context_engine.indexer import DEFAULT_IGNORE_PATTERNS

        assert ".claude/worktrees/" in DEFAULT_IGNORE_PATTERNS
    ...
    def test_worktrees_ignored_even_with_contextignore(self, tmp_path):
        ...
        (tmp_path / ".contextignore").write_text("*.log\n")
        indexer = Indexer(storage=Mock())
        spec = indexer.load_ignore_patterns(tmp_path)

        assert spec.match_file(".claude/worktrees/session-229/BACKLOG.md")
        assert spec.match_file(".claude/worktrees/foo/src/x.py")
        assert not spec.match_file(".claude/skills/compliance-review/procedure.md")
        assert not spec.match_file(".claude/agents/code-reviewer.md")
        assert not spec.match_file(".claude/hooks/lib/cadence.sh")
...
class TestStorageDirectoryPermissions:
    ...
    def test_chmod_tightens_preexisting_base_dir(self, tmp_path):
        ...
        # mkdir's mode is filtered by the caller's umask; set the fixture explicitly.
    ...
    def test_chmod_tightens_preexisting_project_dir(self, tmp_path):
        ...
        assert project_dir.stat().st_mode & 0o077 != 0
...
class TestIncrementalUpdateFallback:
    ...
    def test_chunking_version_mismatch_triggers_full_reindex(self, tmp_path):
        ...
        # Stored version differs from current (line-based-v2)
        indexer.storage.load_metadata = Mock(
            return_value={
                "index_mode": "ondemand",
...
class TestGetChunkingVersion:
    ...
    def test_returns_valid_version_string(self):
        ...
        assert version in ("line-based-v2", "tree-sitter-v3")
    ...
    def test_line_based_when_tree_sitter_not_available(self):
        ...
        for c in indexer.connectors:
            if isinstance(c, CodeConnector):
                c._tree_sitter_available = False
        assert indexer._get_chunking_version() == "line-based-v2"
    ...
    def test_tree_sitter_when_available(self):
        ...
        assert indexer._get_chunking_version() == "tree-sitter-v3"
...
class TestRankingSignals:
    ...
    def test_reranking_improves_ordering(self, monkeypatch):
        """Cross-encoder reranking reorders results when IPC is available."""
        from unittest.mock import patch, MagicMock
        from ai_governance_mcp.context_engine.project_manager import ProjectManager

        monkeypatch.delenv("AI_CONTEXT_ENGINE_EMBED_SOCKET", raising=False)
    ...
    @pytest.mark.parametrize("setting", ["none", " NONE "])
    def test_reranking_explicit_disable_never_contacts_daemon(
        self, monkeypatch, setting
    ):
        from unittest.mock import patch
        from ai_governance_mcp.context_engine.project_manager import ProjectManager

        monkeypatch.setenv("AI_CONTEXT_ENGINE_EMBED_SOCKET", setting)
        pm = ProjectManager(reranking=True)
        results = [
            QueryResult(chunk=self._make_chunk("a.py", "first"), combined_score=0.8),
            QueryResult(chunk=self._make_chunk("b.py", "second"), combined_score=0.7),
        ]
        with patch(
            "ai_governance_mcp.context_engine.project_manager.EmbeddingClient"
        ) as client:
            assert pm._rerank_results("query", results) is results
        client.available.assert_not_called()
        client.assert_not_called()

    def test_reranking_skipped_when_unavailable(self, monkeypatch):
    ...
    def test_reranking_fallback_on_predict_exception(self, monkeypatch):
...
class TestResolveProjectPathCWDFallback:
    ...
    def test_cwd_with_project_markers_returns_cwd(self, tmp_path):
        ...
        Patches ``safe_cwd`` rather than the module's whole ``Path`` class: the
        working-directory read moved behind a single guarded accessor (see
        ``tests/test_no_unguarded_cwd.py``), and patching that seam expresses the
        same intent without stubbing an entire stdlib type.
        """
        from ai_governance_mcp.context_engine.server import _resolve_project_path

        (tmp_path / ".git").mkdir()
        with patch(
            "ai_governance_mcp.context_engine.server.safe_cwd", return_value=tmp_path
        ):
    ...
    def test_unavailable_cwd_returns_none(self):
        """A deleted working directory returns None instead of raising.

        ``query_project`` is hook-enforced like ``evaluate_governance``, so a
        raise here would take the CE half of the gate down the way a dead cwd
        already took the governance half down. Full incident:
        ``tests/test_deleted_cwd_resilience.py``.
        """
        from ai_governance_mcp.context_engine.server import _resolve_project_path

        with patch(
            "ai_governance_mcp.context_engine.server.safe_cwd", return_value=None
        ):
            assert _resolve_project_path({}) is None
...
# =============================================================================
# Code Edge Model Tests
# =============================================================================


class TestCodeEdge:
    """Test CodeEdge model validation and serialization."""

    def test_valid_edge_types(self):
        for et in ("imports", "calls", "extends", "implements", "type_of"):
            edge = CodeEdge(
                source_path="src/main.py",
                source_symbol="main",
    ...
    def test_required_fields(self):
        with pytest.raises(ValidationError):
            CodeEdge(source_path="src/main.py", edge_type="imports")
    ...
    def test_round_trip_serialization(self):
        edge = CodeEdge(
            ...
            target_symbol="BaseApp",
            edge_type="extends",
            confidence=0.9,
        )
        data = edge.model_dump()
        restored = CodeEdge(**data)
        assert restored == edge
        assert restored.source_path == "src/main.py"
        assert restored.edge_type == "extends"
        assert restored.confidence == 0.9


# =============================================================================
# Code Edge Storage Tests
# =============================================================================


class TestCodeEdgeStorage:
    """Test save/load code edges in filesystem storage."""

    @pytest.fixture
    def storage(self, tmp_path):
        return FilesystemStorage(base_path=tmp_path)

    @pytest.fixture
    def project_id(self):
        return "abc123def456"

    def test_round_trip(self, storage, project_id):
        edges = [
            {
                "source_path": "a.py",
                "source_symbol": "foo",
        ...
        storage.save_code_edges(project_id, edges)
        loaded = storage.load_code_edges(project_id)
        assert loaded == edges

    def test_load_nonexistent_returns_none(self, storage, project_id):
        assert storage.load_code_edges(project_id) is None

    def test_corrupt_file_recovery(self, storage, project_id):
        path = storage._ensure_dir(project_id)
        edge_file = path / "code_edges.json"
        edge_file.write_text("{invalid json")
        result = storage.load_code_edges(project_id)
        assert result is None
        assert not edge_file.exists()

    def test_readonly_save_raises(self, tmp_path):
        from ai_governance_mcp.context_engine.storage.filesystem import (
            ReadOnlyFilesystemStorage,
            ReadOnlyStorageError,
        )

        ro_storage = ReadOnlyFilesystemStorage(base_path=tmp_path)
        with pytest.raises(ReadOnlyStorageError):
            ro_storage.save_code_edges("abc123def456", [])

    def test_readonly_load_works(self, tmp_path):
        from ai_governance_mcp.context_engine.storage.filesystem import (
            ReadOnlyFilesystemStorage,
        )

        rw_storage = FilesystemStorage(base_path=tmp_path)
        pid = "abc123def456"
        edges = [
            {
                "source_path": "a.py",
        ...
        rw_storage.save_code_edges(pid, edges)

        ro_storage = ReadOnlyFilesystemStorage(base_path=tmp_path)
        loaded = ro_storage.load_code_edges(pid)
        assert loaded == edges

    def test_empty_edges_round_trip(self, storage, project_id):
        storage.save_code_edges(project_id, [])
        loaded = storage.load_code_edges(project_id)
        assert loaded == []
...
class TestReferenceResolverPython:
    ...
    def project(self, tmp_path):
        ...
        (pkg / "star_importer.py").write_text(
            "from .utils import *\n\ndef use_all():\n    pass\n"
        )

        (pkg / "absolute_user.py").write_text(
            "from pkg.utils import helper\n\ndef use_abs():\n    pass\n"
        )

        return tmp_path

    def _make_chunks(self, project_path):
        """Parse project files into chunks using CodeConnector."""
        from ai_governance_mcp.context_engine.connectors.code import CodeConnector
        ...
        for py_file in sorted(project_path.rglob("*.py")):
            ...
            parsed = cc.parse(py_file)
            for c in parsed:
                c.source_path = str(rel)
            chunks.extend(parsed)
        return chunks

    def test_relative_import_resolves(self, project):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )

        chunks = self._make_chunks(project)
        resolver = ReferenceResolver(project)
        edges = resolver.resolve_all(chunks)

        import_edges = [e for e in edges if e.edge_type == "imports"]
        main_imports = [e for e in import_edges if e.source_path == "pkg/main.py"]
        assert len(main_imports) >= 1
        assert any(e.target_path == "pkg/utils.py" for e in main_imports)

    def test_absolute_import_resolves(self, project):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        abs_imports = [
            e
            for e in edges
            if e.edge_type == "imports" and e.source_path == "pkg/absolute_user.py"
        ]
        assert len(abs_imports) >= 1

    def test_star_import_creates_edges(self, project):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        star_imports = [
            e
            for e in edges
            if e.edge_type == "imports" and e.source_path == "pkg/star_importer.py"
        ]
        assert len(star_imports) >= 1

    def test_stdlib_imports_skipped(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        chunks = self._make_chunks(tmp_path)
        resolver = ReferenceResolver(tmp_path)
        edges = resolver.resolve_all(chunks)
        assert len([e for e in edges if e.edge_type == "imports"]) == 0

    def test_unresolvable_import_no_crash(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
    ...
    def test_circular_import_handled(self, tmp_path):
        ...
        pkg.mkdir()
        (pkg / "__init__.py").write_text("")
        (pkg / "a.py").write_text("from .b import bfunc\ndef afunc():\n    pass\n")
        (pkg / "b.py").write_text("from .a import afunc\ndef bfunc():\n    pass\n")
...
class TestReferenceResolverJSTS:
    """Test JS/TS import resolution."""
    ...
    def test_named_import_resolves(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        import_edges = [
            e for e in edges if e.edge_type == "imports" and e.source_path == "main.ts"
        ]
        assert len(import_edges) >= 1
        assert any(e.target_path == "utils.ts" for e in import_edges)
    ...
    def test_default_import_resolves(self, tmp_path):
        ...
        import_edges = [
            e for e in edges if e.edge_type == "imports" and e.source_path == "App.tsx"
        ]
        assert len(import_edges) >= 1

    def test_barrel_export_chain(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
    ...
    def test_barrel_depth_limit(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        for i in range(7):
            ...
            (d / "index.ts").write_text(f"export {{ thing }} from '../level{i + 1}';\n")

        final = tmp_path / "level7"
        final.mkdir(exist_ok=True)
        (final / "index.ts").write_text("export function thing() { return 42; }\n")

        (tmp_path / "user.ts").write_text(
            "import { thing } from './level0';\n\nfunction use() { thing(); }\n"
        )
    ...
    def test_extension_probing(self, tmp_path):
        ...
        (tmp_path / "helper.jsx").write_text(
        ...
        import_edges = [
            e for e in edges if e.edge_type == "imports" and e.source_path == "main.js"
        ]
        assert len(import_edges) >= 1
        assert any(e.target_path == "helper.jsx" for e in import_edges)


class TestCallEdgeExtraction:
    """Test function/method call edge extraction."""
    ...
    def test_function_call_creates_edge(self, tmp_path):
        ...
        assert any(
            e.source_path == "pkg/main.py" and e.confidence == 0.8 for e in call_edges
        )

    def test_builtin_calls_no_edge(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
...
class TestInheritanceEdgeExtraction:
    ...
    def test_python_extends_edge(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        assert any(
            e.source_symbol == "Dog" and e.target_path == "pkg/base.py"
            for e in extends_edges
        )

    def test_python_multiple_inheritance(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        (pkg / "duck.py").write_text(
            "from .mixins import Flyable, Swimmable\n\n"
        ...
        extends_edges = [
            e for e in edges if e.edge_type == "extends" and e.source_symbol == "Duck"
        ]
        assert len(extends_edges) >= 2

    def test_ts_implements_edge(self, tmp_path):
        from ai_governance_mcp.context_engine.reference_resolver import (
            ReferenceResolver,
        )
        ...
        assert any(e.source_symbol == "User" for e in impl_edges)


# =============================================================================
# Indexer Edge Integration Tests
# =============================================================================
...
class TestIndexerEdgeIntegration:
    ...
    def test_index_project_produces_edges(self, tmp_path):
        """index_project on a multi-file Python project stores edges."""
        pkg = tmp_path / "project"
        pkg.mkdir()
        src = pkg / "src"
        src.mkdir()
        (src / "__init__.py").write_text("")
        (src / "base.py").write_text("class BaseWidget:\n    pass\n")
        ...
        indexer = Indexer(storage=storage)
        pid = FilesystemStorage.project_id_from_path(pkg)
        indexer.index_project(pkg, pid)

        edges = storage.load_code_edges(pid)
        assert edges is not None
        assert len(edges) >= 1
        assert any(e["edge_type"] == "imports" for e in edges)

    def test_parse_error_does_not_block_edges(self, tmp_path):
        """A parse error in one file doesn't block edges from other files."""
        pkg = tmp_path / "project"
        pkg.mkdir()
        (pkg / "good.py").write_text("def foo():\n    pass\n")
        (pkg / "bad.py").write_bytes(b"\x80\x81\x82")
        ...
        indexer = Indexer(storage=storage)
        pid = FilesystemStorage.project_id_from_path(pkg)
        index = indexer.index_project(pkg, pid)
...
class TestFindReferences:
    ...
    def test_find_references_returns_callers(self, tmp_path):
        pkg = tmp_path / "project"
        pkg.mkdir()
        src = pkg / "src"
        src.mkdir()
        ...
        pm = ProjectManager(storage=storage)
        pm.get_or_create_index(pkg)

        result = pm.find_references("helper", project_path=pkg, direction="callers")
        assert result["symbol"] == "helper"
        assert result["total"] >= 1
        assert len(result["callers"]) >= 1

    def test_find_references_unknown_symbol(self, tmp_path):
        pkg = tmp_path / "project"
        pkg.mkdir()
        ...
        result = pm.find_references("nonexistent_symbol", project_path=pkg)
        assert result["total"] == 0
        assert result["callers"] == []
        assert result["callees"] == []

    def test_find_references_direction_filter(self, tmp_path):
        pkg = tmp_path / "project"
        pkg.mkdir()
        src = pkg / "src"
        src.mkdir()
        ...
        all_refs = pm.find_references("Animal", project_path=pkg, direction="all")
        callers_only = pm.find_references(
            "Animal", project_path=pkg, direction="callers"
        )
        callees_only = pm.find_references(
            "Animal", project_path=pkg, direction="callees"
        )

        assert callers_only["total"] <= all_refs["total"]
        assert (
            callees_only["callees"] == [] or callees_only["total"] <= all_refs["total"]
        )


class TestFindReferencesHandler:
    """Test find_references server handler."""

    @pytest.mark.asyncio
    async def test_handler_returns_json(self):
        """find_references handler returns valid JSON."""
        import json

        from ai_governance_mcp.context_engine.server import _handle_find_references

        manager = MagicMock()
        manager.find_references.return_value = {
            "symbol": "test",
            "callers": [],
            "callees": [],
            "total": 0,
        ...
        result = await _handle_find_references(manager, {"symbol": "test"})
        text = result[0].text
        data = json.loads(text)
        assert data["symbol"] == "test"

    @pytest.mark.asyncio
    async def test_handler_rejects_empty_symbol(self):
        """find_references handler rejects empty symbol."""
        import json

        from ai_governance_mcp.context_engine.server import _handle_find_references

        manager = MagicMock()
        result = await _handle_find_references(manager, {"symbol": ""})
        text = result[0].text
        data = json.loads(text)
        assert "error" in data
...
# =============================================================================
# Knowledge Graph Integration Tests (Tasks 13-23)
# =============================================================================


class TestKnowledgeGraphPM:
    """ProjectManager.build_knowledge_graph and query_knowledge_graph."""

    def _make_pm(self, tmp_path, readonly=False):
        from ai_governance_mcp.context_engine.project_manager import ProjectManager
        from ai_governance_mcp.context_engine.storage.filesystem import (
            FilesystemStorage,
        )
    ...
    @pytest.mark.asyncio
    async def test_build_kg_raises_in_readonly(self, tmp_path):
        pm = self._make_pm(tmp_path, readonly=True)
        with pytest.raises(RuntimeError, match="read-only"):
            await pm.build_knowledge_graph(tmp_path)

    @pytest.mark.asyncio
    async def test_build_kg_raises_when_not_indexed(self, tmp_path):
        pm = self._make_pm(tmp_path)
        from ai_governance_mcp.context_engine.cognee_adapter import CogneeAdapter

        if not CogneeAdapter.is_available():
            pytest.skip("cognee not installed — 'not indexed' guard is unreachable")
        with pytest.raises(ValueError, match="not indexed"):
            await pm.build_knowledge_graph(tmp_path / "no-such-project")

    @pytest.mark.asyncio
    async def test_build_kg_succeeds_when_indexed(self, tmp_path):
        pm = self._make_pm(tmp_path)
        ...
        # Simulate an indexed project by writing minimal index data
        index_path = pm.storage.get_index_path(project_id)
        index_path.mkdir(parents=True, exist_ok=True)
        import json
        ...
        mock_adapter.add_chunks = AsyncMock(return_value=1)
        mock_adapter.cognify = AsyncMock(
            return_value={
        ...
        mock_adapter_cls.is_available.return_value = True

        with patch(
            "ai_governance_mcp.context_engine.project_manager.CogneeAdapter",
            mock_adapter_cls,
        ):
            result = await pm.build_knowledge_graph(project_path)

        assert result["status"] == "success"
        mock_adapter.configure.assert_called_once()
        mock_adapter.add_chunks.assert_called_once()
        mock_adapter.cognify.assert_called_once()

    @pytest.mark.asyncio
    async def test_query_kg_returns_results(self, tmp_path):
        pm = self._make_pm(tmp_path)
        project_path = tmp_path / "myproject"
        project_path.mkdir()
        ...
        mock_adapter = MagicMock()
        mock_adapter.configure = MagicMock()
        mock_adapter.has_knowledge_graph.return_value = True
        mock_adapter.search = AsyncMock(
            return_value=[{"content": "found it", "score": 0.9, "source": "main.py"}]
        )
        mock_adapter_cls.return_value = mock_adapter
        mock_adapter_cls.is_available.return_value = True
        ...
        with patch(
            ...
            result = await pm.query_knowledge_graph("test query", project_path)

        assert result["total"] == 1
        assert result["results"][0]["content"] == "found it"

    @pytest.mark.asyncio
    async def test_query_kg_raises_when_cognee_unavailable(self, tmp_path):
        pm = self._make_pm(tmp_path)
        project_path = tmp_path / "myproject"
        project_path.mkdir()
        ...
        mock_adapter_cls = MagicMock()
        mock_adapter_cls.is_available.return_value = False
        ...
        with (
            ...
            pytest.raises(ValueError, match="cognee.*not installed"),
        ):
            await pm.query_knowledge_graph("test", project_path)

    @pytest.mark.asyncio
    async def test_query_kg_empty_when_no_kg_built(self, tmp_path):
        pm = self._make_pm(tmp_path)
        project_path = tmp_path / "myproject"
        project_path.mkdir()
        ...
        mock_adapter_cls = MagicMock()
        mock_adapter = MagicMock()
        mock_adapter.has_knowledge_graph.return_value = False
        mock_adapter_cls.return_value = mock_adapter
        mock_adapter_cls.is_available.return_value = True
        ...
        with patch(
            ...
            result = await pm.query_knowledge_graph("test", project_path)
...
class TestKnowledgeGraphHandlers:
    """Test build_knowledge_graph and query_knowledge_graph MCP handlers."""

    @pytest.mark.asyncio
    async def test_build_kg_handler_returns_json(self):
        import json

        from ai_governance_mcp.context_engine.server import (
            _handle_build_knowledge_graph,
        )

        manager = MagicMock()
        manager.build_knowledge_graph = AsyncMock(
            return_value={
                "dataset_name": "test",
                "status": "success",
        ...
        with (
            ...
            patch("ai_governance_mcp.context_engine.server.CogneeAdapter") as mock_cls,
        ):
            mock_cls.is_available.return_value = True
            result = await _handle_build_knowledge_graph(manager, {})
    ...
    @pytest.mark.asyncio
    async def test_build_kg_handler_returns_install_when_unavailable(self):
        import json
        ...
        with patch("ai_governance_mcp.context_engine.server.CogneeAdapter") as mock_cls:
    ...
    @pytest.mark.asyncio
    async def test_build_kg_handler_readonly_error(self):
        import json
        ...
        manager = MagicMock()
        manager.build_knowledge_graph = AsyncMock(
            side_effect=RuntimeError("Cannot build knowledge graph in read-only mode.")
        )
    ...
    @pytest.mark.asyncio
    async def test_query_kg_handler_returns_results(self):
        import json

        from ai_governance_mcp.context_engine.server import (
            _handle_query_knowledge_graph,
        )

        manager = MagicMock()
        manager.query_knowledge_graph = AsyncMock(
            return_value={
                "query": "test",
                "results": [{"content": "found", "score": 0.9, "source": "a.py"}],
        ...
        with patch(
            ...
            result = await _handle_query_knowledge_graph(
    ...
    @pytest.mark.asyncio
    async def test_query_kg_handler_rejects_empty_query(self):
        import json
        ...
        manager = MagicMock()
        result = await _handle_query_knowledge_graph(manager, {"query": ""})
        data = json.loads(result[0].text)
        assert "error" in data

    @pytest.mark.asyncio
    async def test_query_kg_handler_cognee_unavailable(self):
        import json
        ...
        manager = MagicMock()
        manager.query_knowledge_graph = AsyncMock(
            side_effect=ValueError("cognee is not installed")
        )
        ...
        with patch(
            ...
            result = await _handle_query_knowledge_graph(manager, {"query": "test"})
...
class TestKnowledgeGraphStatus:
    """Test KG info in project_status and delete_project cleanup."""

    @pytest.mark.asyncio
    async def test_project_status_includes_kg_info(self):
        import json
        ...
        manager = MagicMock()
        manager.get_project_status.return_value = mock_status
        ...
        with (
            ...
            patch(
                "ai_governance_mcp.context_engine.server.CogneeAdapter",
                mock_adapter_cls,
            ),
            patch(
                "ai_governance_mcp.context_engine.server._read_daemon_heartbeat",
                return_value=None,
            ),
        ):
            result = await _handle_project_status(manager, {})
        ...
        assert data["knowledge_graph"]["available"] is False
        assert data["knowledge_graph"]["cognee_installed"] is True

    def test_delete_project_removes_cognee_data(self, tmp_path):
        from ai_governance_mcp.context_engine.storage.filesystem import (
            FilesystemStorage,
        )
        ...
        index_path = storage.get_index_path(project_id)
        index_path.mkdir(parents=True, exist_ok=True)
        cognee_path = index_path / "cognee"
        cognee_path.mkdir()
        (cognee_path / "databases").mkdir()
        (cognee_path / "databases" / "cognee_db").write_text("data")
        assert cognee_path.exists()

        storage.delete_project(project_id)
        assert not index_path.exists()
        assert not cognee_path.exists()


# =============================================================================
# BACKLOG #59 — Load-time embedding-space canary gate (port of #58)
# =============================================================================


def _deterministic_encoder():
    """Mock encoder producing a stable, content-derived normalized vector.

    Same text always yields the same vector; different texts yield different
    vectors — so re-encoding a canary text reproduces its stored vector
    exactly (the healthy-index case), while tampered stored vectors diverge.
    """
...
class TestEmbeddingCanaryGate:
    """BACKLOG #59: build-time canary write + load-time space verification."""

    @pytest.fixture
    def project_dir(self, tmp_path):
        proj = tmp_path / "canary_project"
        proj.mkdir()
        (proj / "alpha.py").write_text("def alpha():\n    return 'alpha feature'\n")
        (proj / "beta.md").write_text("# Beta\n\nBeta documentation content.\n")
        (proj / "gamma.py").write_text("def gamma():\n    return 'gamma feature'\n")
        return proj

    def _index(self, project_dir, storage, model=None):
        from ai_governance_mcp.context_engine.indexer import Indexer

        indexer = Indexer(storage=storage)
        indexer._embedding_model = model or _deterministic_encoder()
        pid = FilesystemStorage.project_id_from_path(project_dir)
        index = indexer.index_project(project_dir, pid)
        return indexer, pid, index

    def _manager(self, storage, indexer):
        from ai_governance_mcp.context_engine.project_manager import ProjectManager

        pm = ProjectManager(storage=storage, semantic_weight=0.6, reranking=False)
        pm._indexer = indexer
        return pm

    # ── Model field ──

    def test_project_index_canaries_default_empty(self):
        index = ProjectIndex(
            project_id="abc",
            project_path="/tmp/x",
            created_at="now",
        ...
        assert index.embedding_canaries == []

    # ── Build path ──

    def test_index_project_writes_canaries(self, project_dir, storage):
        from ai_governance_mcp.context_engine.indexer import Indexer

        _, pid, index = self._index(project_dir, storage)

        assert 1 <= len(index.embedding_canaries) <= 3
        # Persisted into metadata (the dump pops only chunks/files)
        metadata = storage.load_metadata(pid)
        assert metadata.get("embedding_canaries")
        # Each canary vector matches the stored embedding row of the chunk
        # whose transformed text it carries (text↔vector pairing is the core
        # correctness property the load gate relies on).
        embeddings = storage.load_embeddings(pid)
        assert embeddings is not None
        rows_by_text = {
            Indexer._embedding_input_text(chunk): embeddings[chunk.embedding_id]
            for chunk in index.chunks
        }
        for canary in index.embedding_canaries:
            assert canary.text in rows_by_text
            assert np.allclose(
                np.asarray(canary.vector, dtype=np.float32),
                rows_by_text[canary.text],
            )

    def test_canary_text_is_exact_encoded_string(self):
        """Canary text must be the EXACT string fed to encode() at build
        (import-context prefix + MAX_EMBEDDING_INPUT_CHARS truncation), not
        raw chunk.content — otherwise the load-gate re-encode produces a
        different vector on a healthy index (contrarian finding 4). Unit-level
        with a crafted chunk so the transform is actually exercised (an
        import-free short chunk has transform == content, proving nothing)."""
        from ai_governance_mcp.context_engine.indexer import (
            MAX_EMBEDDING_INPUT_CHARS,
            Indexer,
        )
        ...
        indexer = Indexer(storage=Mock())
        long_content = "x" * (MAX_EMBEDDING_INPUT_CHARS + 500)
        chunks = [
            ContentChunk(
                content=long_content,
                source_path="/tmp/big.py",
                start_line=1,
                end_line=1,
                content_type="code",
        ...
        canaries = indexer._make_canaries(chunks, embeddings)

        assert len(canaries) == 1
        expected = ("import os\nimport sys" + "\n" + long_content)[
            :MAX_EMBEDDING_INPUT_CHARS
        ]
        assert canaries[0].text == expected
        assert canaries[0].text != chunks[0].content  # transform exercised

    def test_make_canaries_edge_cases(self):
        """0/1/2 chunks, missing or misaligned embeddings."""
        from ai_governance_mcp.context_engine.indexer import Indexer
        ...
        def chunk(i):
            return ContentChunk(
                content=f"content {i}",
                source_path=f"/tmp/f{i}.py",
                start_line=1,
                end_line=1,
                content_type="code",
            )

        assert indexer._make_canaries([], np.zeros((0, 384))) == []
        assert indexer._make_canaries([chunk(0)], None) == []
        # Length mismatch → no canaries (never pair texts with wrong rows)
        assert indexer._make_canaries([chunk(0)], np.zeros((2, 384))) == []
        # 1 chunk → indices {0,0,0} dedup to [0]
        assert len(indexer._make_canaries([chunk(0)], np.zeros((1, 384)))) == 1
        # 2 chunks → indices {0,1,1} dedup to [0,1]
        assert (
            len(indexer._make_canaries([chunk(0), chunk(1)], np.zeros((2, 384)))) == 2
        )

    def test_build_load_roundtrip_passes_gate(self, project_dir, storage):
        """Healthy index + same encoder → gate passes, semantic search active."""
        model = _deterministic_encoder()
        indexer, pid, _ = self._index(project_dir, storage, model=model)
        pm = self._manager(storage, indexer)

        result = pm.query_project("alpha feature", project_dir)
        assert pid in pm._loaded_embeddings
        assert result.total_results > 0

    # ── Load gate ──

    def test_load_gate_discards_divergent_embeddings(self, project_dir, storage):
        """Stored canary vectors orthogonal to the query encoder's space →
        embeddings discarded (BM25-only), query still answers.
        ...
        # Tamper: replace canary vectors with vectors from a different space
        metadata = storage.load_metadata(pid)
        rng = np.random.default_rng(99)
        for canary in metadata["embedding_canaries"]:
            divergent = rng.standard_normal(384).astype(np.float32)
            divergent /= np.linalg.norm(divergent)
            canary["vector"] = divergent.tolist()
        storage.save_metadata(pid, metadata)

        pm = self._manager(storage, indexer)
        result = pm.query_project("alpha feature", project_dir)

        assert pid not in pm._loaded_embeddings  # semantic disabled
        assert result.total_results > 0  # BM25 fallback still works

    def test_load_gate_error_fails_safe(self, project_dir, storage):
        """Encoder failure during the canary check → embeddings discarded."""
        model = _deterministic_encoder()
        indexer, pid, _ = self._index(project_dir, storage, model=model)

        failing = MagicMock()
        failing.encode = MagicMock(side_effect=RuntimeError("daemon down"))
        indexer._embedding_model = failing

        pm = self._manager(storage, indexer)
        pm.query_project("alpha feature", project_dir)
        assert pid not in pm._loaded_embeddings

    def test_load_gate_skipped_for_legacy_metadata(self, project_dir, storage):
        """Metadata without canaries (pre-#59 index) → gate skipped,
        embeddings kept (mirrors #58 legacy behavior)."""
        model = _deterministic_encoder()
        indexer, pid, _ = self._index(project_dir, storage, model=model)

        metadata = storage.load_metadata(pid)
        metadata.pop("embedding_canaries", None)
        storage.save_metadata(pid, metadata)
    ...
    def test_gate_runs_at_load_not_per_query(self, project_dir, storage):
        """The gate encodes canaries ONCE at load; repeated queries must not
        re-run it (prevents a per-query encode 'fix' that would add daemon
        roundtrips to every query — contrarian finding 5)."""
        model = _deterministic_encoder()
        indexer, pid, index = self._index(project_dir, storage, model=model)
        canary_texts = [c.text for c in index.embedding_canaries]
        model.encode.reset_mock()  # discard build-time encode calls

        pm = self._manager(storage, indexer)
        pm.query_project("alpha feature", project_dir)
        pm.query_project("beta documentation", project_dir)
        pm.query_project("gamma feature", project_dir)

        canary_batches = [
            call.args[0]
            for call in model.encode.call_args_list
            if list(call.args[0]) == canary_texts
        ]
        assert len(canary_batches) == 1

    # ── Incremental path ──

    def test_incremental_update_refreshes_canaries(self, project_dir, storage):
        model = _deterministic_encoder()
        indexer, pid, index = self._index(project_dir, storage, model=model)
        original_canaries = [c.text for c in index.embedding_canaries]

        (project_dir / "delta.py").write_text(
            "def delta():\n    return 'delta feature'\n"
        )
        updated = indexer.incremental_update(
            project_dir, pid, [project_dir / "delta.py"]
        )

        assert 1 <= len(updated.embedding_canaries) <= 3
        metadata = storage.load_metadata(pid)
        assert metadata.get("embedding_canaries")
        # Refreshed canaries must be drawn from the UPDATED chunk set — a
        # stale carry-forward of the original canaries would pass the count
        # and gate checks while no longer witnessing the live matrix.
        from ai_governance_mcp.context_engine.indexer import Indexer

        updated_texts = {
            Indexer._embedding_input_text(chunk) for chunk in updated.chunks
        }
        for canary in updated.embedding_canaries:
            assert canary.text in updated_texts
        assert original_canaries  # original build had canaries too
        # Refreshed canaries must round-trip the gate against the live encoder
        from ai_governance_mcp.context_engine.project_manager import ProjectManager

        pm = ProjectManager(storage=storage, semantic_weight=0.6, reranking=False)
        pm._indexer = indexer
        pm.query_project("delta feature", project_dir)
        assert pid in pm._loaded_embeddings

    # ── Model-label mismatch (pre-existing no-op discard, fixed with #59) ──

    def test_model_mismatch_actually_discards_embeddings(self, project_dir, storage):
        """_load_project's model-mismatch warning claims 'Semantic search
        disabled — using BM25-only', but the pop-then-reload sequence made
        that a no-op: _load_search_indexes saw the popped key and reloaded
        the incompatible embeddings anyway (code-reviewer finding 2). For a
        legacy no-canary index, a mismatched model must mean BM25-only."""
        model = _deterministic_encoder()
        indexer, pid, _ = self._index(project_dir, storage, model=model)

        # Make it a legacy index built under a different model label
        metadata = storage.load_metadata(pid)
        metadata.pop("embedding_canaries", None)
        ...
        assert pid not in pm._loaded_embeddings  # actually discarded now
        assert result.total_results > 0  # BM25 fallback still answers
