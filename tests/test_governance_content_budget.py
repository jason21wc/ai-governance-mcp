...
async def test_degraded_coverage_warning_survives_body_budget(
    reset_server_state, test_settings, sample_principle, monkeypatch
):
    import json
    ...
    monkeypatch.setattr(engine, "retrieve", lambda *args, **kwargs: result)
    monkeypatch.setattr(governance, "PRINCIPLE_CONTENT_BUDGET_CHARS", 1000)
    response = await governance._handle_evaluate_governance(
        engine, {"planned_action": result.query}
    )
    parsed = json.loads(response[0].text)
    assert sum(len(p["content"] or "") for p in parsed["relevant_principles"]) <= 1000
    assert "principle_content_note" in parsed
