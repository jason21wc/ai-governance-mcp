...
class TestTransactionalReload:
    ...
    def test_reload_recovers_when_embeddings_become_loadable_on_retry(
        self, engine, index_dir
    ):
        """A refused generation is retried once instead of staying stale."""
        import time as time_mod
        ...
        # Refusal preserves the last accepted generation rather than claiming
        # the new one was loaded.
        assert engine._index_mtime == old_mtime

        # Repair only the matrix. The JSON commit signal is unchanged, so this
        # succeeds only if the refused generation gets its bounded retry.
        new_emb = np.random.rand(1, 384).astype(np.float32)
        np.save(index_dir / "content_embeddings.npy", new_emb)
        engine._check_index_freshness()

        assert engine._index_mtime > old_mtime
        assert engine.index.domains["constitution"].principles[0].title == "Newer"
        np.testing.assert_array_equal(engine.content_embeddings, new_emb)

    def test_reload_retry_exhaustion_prevents_storm_and_new_commit_rearms(
        self, engine, index_dir
    ):
        """Two failures suppress one generation; a new JSON commit tries again."""
        import time as time_mod

        old_mtime = engine._index_mtime
        time_mod.sleep(0.05)
        (index_dir / "global_index.json").write_text(
            self._make_index_json(title="Persistently Corrupt")
        )
        (index_dir / "content_embeddings.npy").write_bytes(b"corrupt")

        with patch.object(engine, "_load_index", wraps=engine._load_index) as load:
            engine._check_index_freshness()
            engine._check_index_freshness()
            engine._check_index_freshness()

            assert load.call_count == 2
            assert engine._reload_attempts == 2
            assert engine._reload_failures == 2
            assert engine._index_mtime == old_mtime
            ...
            np.save(index_dir / "content_embeddings.npy", new_emb)
            (index_dir / "global_index.json").write_text(
                self._make_index_json(title="Next Commit")
            )
            engine._check_index_freshness()

            assert load.call_count == 3
            assert engine.index.domains["constitution"].principles[0].title == (
                "Next Commit"
            )
            assert engine._reload_attempts == 0
            assert engine._reload_failures == 0

    def test_reload_parse_failure_is_bounded(self, engine, index_dir):
        """Invalid JSON gets one retry, then waits for another commit signal."""
        import time as time_mod

        old_mtime = engine._index_mtime
        time_mod.sleep(0.05)
        (index_dir / "global_index.json").write_text("not json")
    ...
    def test_reload_scalar_content_embeddings_triggers_bounded_rollback(
        self, engine, index_dir
    ):
        """A loadable scalar .npy cannot crash or bypass retry accounting."""
        import time as time_mod
        ...
        time_mod.sleep(0.05)
        np.save(index_dir / "content_embeddings.npy", np.array(1.0, dtype=np.float32))
        (index_dir / "global_index.json").write_text(
            self._make_index_json(title="Scalar Content")
        )
        ...
        np.testing.assert_array_equal(engine.content_embeddings, old_emb)
        assert engine.index.domains["constitution"].principles[0].title == old_title
        assert all(item is not None and item.title == old_title for item in observed)
        assert engine._reload_attempts == 2
        assert engine._reload_failures == 2

    def test_reload_scalar_domain_embeddings_disables_routing(self, engine, index_dir):
        """A scalar optional routing matrix is discarded without crashing reload."""
        import time as time_mod

        time_mod.sleep(0.05)
        new_emb = np.random.rand(1, 384).astype(np.float32)
        np.save(index_dir / "content_embeddings.npy", new_emb)
        np.save(index_dir / "domain_embeddings.npy", np.array(1.0, dtype=np.float32))
        (index_dir / "global_index.json").write_text(
            self._make_index_json(title="Scalar Domain")
        )

        principle = engine.get_principle_by_id("meta-C1")

        assert principle is not None
        assert principle.title == "Scalar Domain"
        assert engine.index.domains["constitution"].principles[0].title == (
            "Scalar Domain"
        )
        np.testing.assert_array_equal(engine.content_embeddings, new_emb)
        assert engine.domain_embeddings is None
        assert engine.domain_rows == []
...
class _FixedEmbedder:
    """Returns the same fixed unit vector for every text (controls cosine)."""
...
class TestCanaryGate:
    """BACKLOG #58 Fix A — load-time behavioral canary gate in _load_index."""
    ...
    def _index_json(self, title="Test Principle", canaries=None, dims=384):
        import json
        ...
        if canaries is not None:
            body["embedding_canaries"] = canaries
        return json.dumps(body)

    def _write_index(self, index_path, title="Test Principle", canaries=None):
        (index_path / "global_index.json").write_text(
            self._index_json(title=title, canaries=canaries)
        )
        np.save(
            index_path / "content_embeddings.npy",
            np.random.rand(1, self.DIM).astype(np.float32),
        )
        np.save(
            index_path / "domain_embeddings.npy",
            np.random.rand(1, self.DIM).astype(np.float32),
    ...
    def _settings(self, index_path, tmp_path):
        settings = Settings()
        settings.index_path = index_path
        settings.logs_path = tmp_path / "logs"
        settings.logs_path.mkdir(parents=True, exist_ok=True)
    ...
    @pytest.fixture
    def working_engine(self, tmp_path):
        """Engine loaded from a no-canary index → semantic on, embedder unused."""
        from ai_governance_mcp.retrieval import RetrievalEngine

        index_path = tmp_path / "index"
        index_path.mkdir()
        self._write_index(index_path, title="Original", canaries=None)
        engine = RetrievalEngine(self._settings(index_path, tmp_path))
        assert engine.content_embeddings is not None  # cold load OK, gate skipped
        return engine, index_path

    def test_canary_gate_passes(self, working_engine, tmp_path):
        """Matching canaries → new index swaps in (semantic stays on)."""
        import time as time_mod

        engine, index_path = working_engine
        engine._embedder = _FixedEmbedder(self._unit(0))  # matches stored e0

        time_mod.sleep(0.05)
        # Row 0 must equal the canary's stored vector: the extractor writes each
        # canary as a verbatim copy of its row, and the load-time canary-ROW
        # check enforces that (it is what detects a stale JSON paired with a
        # fresh matrix — BACKLOG #217). A random matrix here would be a shape no
        # real build produces.
        new_emb = np.array([self._unit(0)], dtype=np.float32)
        canaries = [{"text": "probe", "vector": self._unit(0)}]
        (index_path / "global_index.json").write_text(
            self._index_json(title="Fresh", canaries=canaries)
        )
        np.save(index_path / "content_embeddings.npy", new_emb)

        engine._check_index_freshness()

        assert engine.index.domains["constitution"].principles[0].title == "Fresh"
        np.testing.assert_array_equal(engine.content_embeddings, new_emb)

    def test_canary_failure_retains_prior_index(self, working_engine, tmp_path):
        """Divergent canaries on reload → RETAIN prior working index (rollback),
        NOT swap to BM25. Fails against any model_mismatch-routed implementation."""
        import time as time_mod

        engine, index_path = working_engine
        old_emb = engine.content_embeddings.copy()
        old_title = engine.index.domains["constitution"].principles[0].title
        engine._embedder = _FixedEmbedder(self._unit(1))  # orthogonal to stored e0

        time_mod.sleep(0.05)
        canaries = [{"text": "probe", "vector": self._unit(0)}]
        (index_path / "global_index.json").write_text(
            self._index_json(title="Divergent", canaries=canaries)
        )
        np.save(
            index_path / "content_embeddings.npy",
            np.random.rand(1, self.DIM).astype(np.float32),
        ...
        # Prior working index retained, not discarded.
        assert engine.content_embeddings is not None
        np.testing.assert_array_equal(engine.content_embeddings, old_emb)
        assert engine.index.domains["constitution"].principles[0].title == old_title
        assert engine._reload_failures >= 1

    def test_canary_failure_coldstart_falls_to_bm25(self, tmp_path):
        """First load with failing canaries → BM25-only (nothing to retain).
        ...
        index_path.mkdir()
        canaries = [{"text": "probe", "vector": self._unit(0)}]
        self._write_index(index_path, canaries=canaries)
        settings = self._settings(index_path, tmp_path)

        with patch.object(
            RetrievalEngine,
            "embedder",
            new_callable=PropertyMock,
            return_value=_FixedEmbedder(self._unit(1)),  # orthogonal → fail
        ):
            engine = RetrievalEngine(settings)

        assert engine.index is not None
        assert engine.content_embeddings is None  # BM25 fallback

    def test_canary_encode_raises_is_fail_safe(self, tmp_path):
        """Encode raising during load must NOT crash __init__ → treated as fail."""
        from unittest.mock import PropertyMock
        ...
        with patch.object(
            ...
            engine = RetrievalEngine(settings)  # must not raise

        assert engine.content_embeddings is None

    def test_canary_backward_compat_no_canaries(self, working_engine):
        """Legacy index without canaries → gate skipped, semantic stays on."""
        engine, _ = working_engine
        assert engine.content_embeddings is not None

    @pytest.mark.parametrize(
        "state, expected_rows, expected_canaries",
        [
            ("healthy", 1, 1),
            ("degraded", 0, 1),
            ("missing_canaries", 1, 0),
        ],
    )
    def test_index_readiness_reports_accepted_state(
        self, tmp_path, state, expected_rows, expected_canaries
    ):
        """Readiness describes accepted live state, not merely loaded .npy rows."""
        from unittest.mock import PropertyMock

        from ai_governance_mcp.retrieval import (
            INDEX_READINESS_FORMAT,
            RetrievalEngine,
            logger,
        )
        ...
        canaries = (
            None
            if state == "missing_canaries"
            else [{"text": "probe", "vector": self._unit(0)}]
        )
        self._write_index(index_path, canaries=canaries)
        np.save(
            index_path / "content_embeddings.npy",
            np.array([self._unit(0)], dtype=np.float32),
        )
        embedder = _FixedEmbedder(self._unit(1 if state == "degraded" else 0))
        with (
            patch.object(
                RetrievalEngine,
                "embedder",
            ...
            patch.object(logger, "info", wraps=logger.info) as info,
        ):
            engine = RetrievalEngine(self._settings(index_path, tmp_path))
        ...
        readiness_calls = [
            ...
            if call.args[0] == INDEX_READINESS_FORMAT
        ]
        assert len(readiness_calls) == 1
        assert readiness_calls[0].args == (
            INDEX_READINESS_FORMAT,
            expected_rows,
            expected_canaries,
        )
        assert engine.bm25_index is not None
        assert (engine.content_embeddings is not None) == bool(expected_rows)

    def test_index_readiness_not_emitted_for_rejected_reload(self, working_engine):
        """A rejected candidate must not advertise newly accepted readiness."""
        from ai_governance_mcp.retrieval import INDEX_READINESS_FORMAT, logger

        engine, index_path = working_engine
        old_index = engine.index
        engine._embedder = _FixedEmbedder(self._unit(1))
        self._write_index(
            index_path, canaries=[{"text": "probe", "vector": self._unit(0)}]
        )

        with patch.object(logger, "info", wraps=logger.info) as info:
            engine._load_index()

        assert engine.index is old_index
        assert not any(
            call.args[0] == INDEX_READINESS_FORMAT for call in info.call_args_list
        )
