...
@pytest.mark.parametrize("marker", ["wt/paused", "origin/wt/paused", "paused"])
@pytest.mark.parametrize("state", ["dirty", "dead", "unknown"])
def test_worktree_keep_matches_real_branch_without_stale_marker(
    repo, tmp_path, marker, state
):
    """A display-name truncation must not discard the user's retention decision.

    Covers: FM-HYGIENE-KEEP-REF-IDENTITY
    """
    wt = tmp_path / "paused"
    _git(repo, "worktree", "add", "-q", "-b", "wt/paused", str(wt))
    assert _git(wt, "branch", "--show-current") == "wt/paused"
    ctx = repo / "_ai-context"
    ctx.mkdir()
    (ctx / "BACKLOG.md").write_text(f"keep: {marker}\n")
    local = rh.collect_local_facts(repo)
    collected = next(w for w in local["worktrees"] if w["path"] == str(wt))
    assert collected["branch_ref"] == "refs/heads/wt/paused"
    collected["ownership"] = state if state != "dirty" else "unknown"
    collected["dirty_files"] = [" M f.txt"] if state == "dirty" else []
    findings = rh.classify(local, None)
    kept = next(f for f in findings if f.check == "stale_worktree")
    assert kept.kept, f"{marker} must suppress the worktree alarm"
    assert kept.severity == "info"
    assert not any(f.check == "stale_keep_marker" for f in findings)
    report = rh.summarize(findings, "high")
    assert report["counts"]["kept"] == 1


def test_worktree_keep_preserves_branch_namespace_identity():
    """An exact wt/ marker must not mute a different branch with the same leaf.

    Covers: FM-HYGIENE-KEEP-REF-IDENTITY
    """
    local = _local(
        keep_markers={"wt/paused": "BACKLOG"},
        worktrees=[
            {
                "path": f"/tmp/{prefix}",
                "branch": "paused",
                "branch_ref": f"refs/heads/{prefix}/paused",
                "ownership": "unknown",
            }
            for prefix in ("wt", "feature")
        ],
    )
    findings = rh.classify(local, None)
    worktrees = {f.evidence["path"]: f for f in findings if f.check == "stale_worktree"}
    assert worktrees["/tmp/wt"].kept
    assert not worktrees["/tmp/feature"].kept
    assert not any(f.check == "stale_keep_marker" for f in findings)


@pytest.mark.parametrize("marker", ["wt/paused", "origin/paused"])
def test_worktree_keep_for_unknown_namespace_remains_stale(marker):
    """Sharing a leaf name is not proof the named branch exists.

    Covers: FM-HYGIENE-KEEP-REF-IDENTITY
    """
    local = _local(
        keep_markers={marker: "BACKLOG"},
        worktrees=[
            {
                "path": "/tmp/feature",
                "branch": "paused",
                "branch_ref": "refs/heads/feature/paused",
                "ownership": "unknown",
            }
        ],
    )
    findings = rh.classify(local, None)
    assert not any(f.kept for f in findings)
    assert any(f.check == "stale_keep_marker" and f.ref == marker for f in findings)
...
def test_landed_on_origin_is_not_reported_as_NEVER_LANDED():
    """BACKLOG #365, reproduced. A stale local default manufactured a false HIGH.

    Measured 2026-09-01: the primary sat one commit behind `origin/main` and this tool
    said "Worktree whose work never landed ... 1 commit(s) not on refs/heads/main"
    about `backlog3`, whose work HAD landed. `resolve_base_ref` prefers the local ref,
    which is the SAFE order (a stale base withholds a removal command) but not a true
    one. Safe and true are different requirements.

    The command must STILL be withheld -- the two bases disagree and `unlanded_commits`
    stays the conservative number -- but the title must stop asserting the false part.

    Covers: FM-HYGIENE-LANDEDNESS-STALE-BASE.
    """
    local = _local(
        default_ref_lag=1,
        origin_base_ref="refs/remotes/origin/main",
        landed_base_ref="refs/heads/main",
        worktrees=[
            {
                "path": "/tmp/wt-landed-on-origin",
                "branch": "wt/backlog3",
                "dirty_files": [],
                "ownership": "dead",
                "owner_pids": [72050],
                "unlanded_commits": 1,
                "unlanded_vs_origin": 0,
            },
        ],
    )
    findings = [f for f in rh.classify(local, None) if f.check == "stale_worktree"]
    assert len(findings) == 1
    f = findings[0]
    assert "never landed" not in f.title, "the false claim is the whole defect"
    assert "has landed on refs/remotes/origin/main" in f.title
    assert "1 commit(s) behind" in f.title
    assert f.command is None, (
        "disagreeing bases must not license removal -- safety is unchanged"
    )
    assert f.evidence["unlanded_vs_origin"] == 0
    assert f.evidence["default_ref_lag"] == 1


def test_unlanded_against_BOTH_refs_still_reads_as_never_landed():
    """The fix must not mute the true positive it sits next to.

    A branch genuinely ahead of both the local and the remote default is unlanded, and
    nothing about a stale primary changes that. Withholding is required either way; the
    title is what must stay accurate.
    ...
    local = _local(
        ...
        worktrees=[
            {
                "path": "/tmp/wt-really-unlanded",
                "branch": "wt/ahead",
                "dirty_files": [],
                "ownership": "dead",
                "owner_pids": [99999],
                "unlanded_commits": 4,
                "unlanded_vs_origin": 3,
            },
        ],
    )
    f = [x for x in rh.classify(local, None) if x.check == "stale_worktree"][0]
    assert "never landed" in f.title
    assert f.command is None
    assert f.severity == "high"


def test_absent_origin_facts_behave_exactly_as_before():
    """classify() stays pure over `local`, and a caller predating these facts is safe.

    DEGRADE TOWARD QUIET: `unlanded_vs_origin` absent, or -1 for "not computed", must
    never be read as "landed on origin". Absence of evidence is not evidence of landing
    -- the same rule the -1 landedness sentinel already enforces one field over.

    Covers: FM-HYGIENE-LANDEDNESS-STALE-BASE.
    """
    for extra in ({}, {"unlanded_vs_origin": -1}):
        wt = {
            "path": "/tmp/wt-old-caller",
            "branch": "wt/old",
            "dirty_files": [],
            "ownership": "dead",
            "owner_pids": [12345],
        ...
        f = [
            x
            for x in rh.classify(_local(worktrees=[wt]), None)
            if x.check == "stale_worktree"
        ][0]
        assert "never landed" in f.title, f"regressed for {extra!r}"
        assert f.command is None


def test_stale_default_ref_is_reported_and_offers_ff_only_from_the_primary():
    """BACKLOG #362 candidate (a): nothing notices when the primary goes stale.

    `~/.claude/skills/*` and `~/.codex/hooks/*` are absolute symlinks into the primary,
    so a stale primary silently changes what every project on the machine runs. WARN,
    never HIGH -- being behind is the ordinary state between a sibling's publish and
    your next fetch. The fast-forward is offered ONLY from the primary, because run
    from a worktree it would move the wrong checkout.
    ...
    from_primary = [
        f
        for f in rh.classify(_local(acting_is_primary=True, **base), None)
        if f.check == "stale_default_ref"
    ]
    assert len(from_primary) == 1
    assert from_primary[0].severity == "warn"
    assert from_primary[0].command == "git merge --ff-only refs/remotes/origin/main"
    assert from_primary[0].evidence["lag_commits"] == 3

    from_worktree = [
        f
        for f in rh.classify(_local(acting_is_primary=False, **base), None)
        if f.check == "stale_default_ref"
    ]
    assert len(from_worktree) == 1
    assert from_worktree[0].command is None, (
        "a fast-forward run from a worktree moves the wrong checkout"
    )

    for lag in (0, -1):
        assert not [
            f
            for f in rh.classify(_local(default_ref_lag=lag), None)
            if f.check == "stale_default_ref"
        ], f"lag={lag} must be silent, not a finding"


def test_default_ref_lag_is_measured_against_the_real_remote(repo_with_remote):
    """The lag fact itself, on a real repo rather than a dict.

    Distinct from the classify tests above: those prove what the report SAYS, this
    proves the number is computed from git and not from a fixture's imagination.
    ...
    facts = rh.collect_local_facts(repo)
    assert facts["default_ref_lag"] in (0, -1), (
        "a primary level with its remote has no lag"
    )
