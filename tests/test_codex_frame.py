...
def run_hook(
    ...
    *,
    ...
    env = {**os.environ}
    # Point the subprocess at THIS checkout's src, matching the pattern in
    # test_semantic_rank.py. Without it the subprocess resolves whatever editable
    # install is on the machine (the primary checkout), so in a linked worktree the
    # test compared the worktree's in-process FRAME against the PRIMARY's emitted
    # one and failed on a difference that was never a defect. cwd is "/" here, so
    # this must be absolute.
    #
    # WHAT THIS ASSERTION IS AND IS NOT, stated because the first version of this
    # comment overclaimed and a mutation check caught it: these tests compare the
    # hook's emitted string against the `FRAME` constant imported in-process. That
    # is a CONSISTENCY check on the emit path — it proves the hook ships the module's
    # FRAME rather than some other string. It is NOT a content check: corrupting
    # FRAME moves both sides together and every test here still passes (verified by
    # mutation). FRAME's content is guarded elsewhere — `test_frame_ssot.py` asserts
    # byte-identity with the .sh copy and the ASCII-only invariant.
