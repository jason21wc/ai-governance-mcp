...
class TestMethodRetrievalQuality:
    ...
    def test_eval_driven_development_retrievable(self, real_settings):
        """EDD method (§5.2.8) must be discoverable — Layer-B output-quality verification.

        RED before the method is authored + index rebuilt; GREEN after.
        """
        from ai_governance_mcp.retrieval import RetrievalEngine

        engine = RetrievalEngine(real_settings)
        result = engine.retrieve(
            "verify probabilistic AI feature outputs golden cases judge threshold eval-driven",
            include_methods=True,
        )

        target_id = "coding-method-eval-driven-development"
        target_method = next(
            (m for m in result.methods if m.method.id == target_id), None
        )

        if target_method is None:
            print("\n=== Eval-Driven Development Test ===")
            print("Found: NO. Top 5 methods returned:")
            for i, m in enumerate(result.methods[:5]):
                print(f"  {i + 1}. {m.method.id}: {m.combined_score:.3f}")
            pytest.fail(f"Method {target_id} not found in results")

        print(f"\nEDD method found, combined score: {target_method.combined_score:.3f}")
        assert target_method.combined_score >= 0.0
