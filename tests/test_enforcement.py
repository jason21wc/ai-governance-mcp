...
class TestActionProvenance:
    """The recency gate records THAT governance was called, never WHAT FOR.

    One evaluation therefore licenses every governed tool until it expires, so
    "evaluated A, then did B" is invisible. These tests pin the reporting that
    makes it visible. Reporting ONLY — nothing here may change whether a call is
    allowed; binding the gate to a capability was declined as disproportionate
    for an advisory layer (PROJECT-MEMORY.md:83).
    """
    ...
    def _msg(self, tool, arguments, req_id=7):
        return json.dumps(
            {
                "jsonrpc": "2.0",
                "id": req_id,
    ...
    # ---- capture ----
    def test_record_call_captures_planned_action(self):
        e = GovernanceEnforcer()
        e.record_call("evaluate_governance", {"planned_action": "update the README"})
        assert e._last_action_preview == "update the README"
        assert e._last_action_digest is not None
        assert len(e._last_action_digest) == 16

    def test_record_call_without_arguments_captures_nothing(self):
        e = GovernanceEnforcer()
        e.record_call("evaluate_governance")
        assert e._last_action_preview is None
        assert e.action_provenance_note("send_email") is None

    def test_preview_is_clamped_and_whitespace_collapsed(self):
        e = GovernanceEnforcer()
        e.record_call(
            "evaluate_governance", {"planned_action": "a\n\n  b " + "x" * 500}
        )
        assert e._last_action_preview.startswith("a b ")
        assert len(e._last_action_preview) <= 120

    def test_non_satisfier_call_does_not_capture(self):
        """Only a satisfier's arguments are provenance; an action tool's are not."""
        e = GovernanceEnforcer(govern_all=True)
        e.record_call("send_email", {"planned_action": "not a satisfier"})
        assert e._last_action_preview is None

    # ---- reporting ----
    def test_note_emitted_once_per_tool_and_evaluation(self):
        e = GovernanceEnforcer()
        e.record_call("evaluate_governance", {"planned_action": "update the README"})

        first = e.action_provenance_note("send_email")
        assert first is not None
        assert "send_email" in first and "update the README" in first
        assert e.action_provenance_note("send_email") is None, "should not repeat"
        assert e.action_provenance_note("delete_mailbox") is not None, "new tool"

    def test_new_evaluation_re_arms_the_note(self):
        e = GovernanceEnforcer()
        e.record_call("evaluate_governance", {"planned_action": "first"})
        assert e.action_provenance_note("send_email") is not None
        assert e.action_provenance_note("send_email") is None

        e.record_call("evaluate_governance", {"planned_action": "second"})
        note = e.action_provenance_note("send_email")
        assert note is not None and "second" in note

    # ---- cross-process ----
    def test_provenance_written_to_shared_state(self, state_dir):
        state_file = os.path.join(state_dir, "state.json")
        writer = GovernanceEnforcer(state_file=state_file)
        writer.record_call(
            "evaluate_governance", {"planned_action": "update the README"}
        )

        with open(state_file) as f:
            state = json.load(f)
        assert state["action_preview"] == "update the README"
        assert state["action_digest"] == writer._last_action_digest

    def test_reader_adopts_writer_provenance(self, state_dir):
        state_file = os.path.join(state_dir, "state.json")
        writer = GovernanceEnforcer(state_file=state_file, state_ttl=300)
        writer.record_call(
            "evaluate_governance", {"planned_action": "update the README"}
        ...
        assert reader.check_precondition("delete_mailbox").allowed
        note = reader.action_provenance_note("delete_mailbox")
        assert note is not None and "update the README" in note

    def test_hostile_state_file_preview_is_ignored_not_trusted(self, state_dir):
        """Provenance is written by another process — treat it as untrusted text."""
        state_file = os.path.join(state_dir, "state.json")
        with open(state_file, "w") as f:
            json.dump(
                {
                    ...
                    "pid": os.getpid(),
                    "project": None,
                    "action_digest": {"not": "a string"},
                    "action_preview": ["not", "a", "string"],
                },
                f,
            )
        ...
        assert reader.check_precondition("delete_mailbox").allowed
        assert reader.action_provenance_note("delete_mailbox") is None

    def test_oversized_preview_from_state_file_is_reclamped(self, state_dir):
        state_file = os.path.join(state_dir, "state.json")
        with open(state_file, "w") as f:
            json.dump(
                {
        ...
        assert reader.check_precondition("delete_mailbox").allowed
        assert len(reader._last_action_preview) <= 120

    # ---- protocol level ----
    @pytest.mark.asyncio
    async def test_governed_call_emits_provenance_note_to_stderr(self, capsys):
        enforcer = GovernanceEnforcer(govern_all=True)
        proxy = StdioProxy(server_cmd=["echo", "test"], enforcer=enforcer)

        await proxy._handle_client_message(
            self._msg("evaluate_governance", {"planned_action": "update the README"})
        )
        capsys.readouterr()  # discard anything from the satisfier itself

        result = await proxy._handle_client_message(
            self._msg("send_email", {"body": "hello"})
        )
        assert result is None, "call must still be forwarded — reporting only"
        err = capsys.readouterr().err
        assert "send_email" in err
        assert "update the README" in err

    @pytest.mark.asyncio
    async def test_satisfier_itself_emits_no_note(self, capsys):
        enforcer = GovernanceEnforcer(govern_all=True)
        proxy = StdioProxy(server_cmd=["echo", "test"], enforcer=enforcer)
        ...
        assert "proceeded on governance" not in capsys.readouterr().err

    @pytest.mark.asyncio
    async def test_provenance_does_not_change_allow_decision(self):
        """A governed tool with no prior governance is still blocked."""
        enforcer = GovernanceEnforcer(govern_all=True)
        proxy = StdioProxy(server_cmd=["echo", "test"], enforcer=enforcer)
        ...
        assert "GOVERNANCE REQUIRED" in text


class TestActGateNotOverridableByConfig:
    """The act-intrinsic gate is the only non-model-satisfiable tier and is hard
    even in soft mode. A config file that could switch it off would be a config
    file that disables the one control the model cannot talk past."""

    def test_enforce_acts_rejected_as_config_override(self):
        with tempfile.NamedTemporaryFile("w", suffix=".yaml", delete=False) as f:
            f.write("govern_all: true\n")
            config_path = f.name
        try:
            with pytest.raises(ValueError, match="security-critical"):
                GovernanceEnforcer.from_config(config_path, enforce_acts=False)
        finally:
