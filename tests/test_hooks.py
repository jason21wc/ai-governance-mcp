...
class TestBypassAuditLog:
    ...
    def _red_record(self, repo, sha, **extra):
        """A red check-run record at `sha`, plus whatever attribution fields."""
        import json as _json
        ...
        rec = {
            ...
            "seconds": 1,
            "checks": [
                {"check": "alpha", "status": "fail", "seconds": 1, "detail": ""},
                {"check": "beta", "status": "fail", "seconds": 1, "detail": ""},
            ],
        }
        rec.update(extra)
        (repo / "logs").mkdir(exist_ok=True)
        (repo / "logs" / "check-runs.jsonl").write_text(_json.dumps(rec) + "\n")
    ...
    def test_check11_does_not_block_on_a_dirty_tree_record(self, tmp_path):
        """A run over a dirty tree did not test this commit's content.

        check.sh stamps the head the run STARTED at. If the tree was dirty, the
        results describe uncommitted content, so blocking the push asserts more
        than was measured -- and a false deny trains QUALITY_GATE_SKIP=true, which
        disables the secret scanner in Check 6. Observed 2026-09-01: a run begun
        before a formatting fix recorded against the commit containing the fix.
        """
        repo = self._push_repo(tmp_path / "wt", "import sys\nsys.exit(0)\n")
        self._red_record(repo, self._head(repo), dirty=True, head_moved=False)
        reason = self._deny_reason(self._gate(tmp_path, repo))
        assert reason is None or "local check run" not in reason, (
            f"blocked on a record that does not describe this commit: {reason}"
        )

    def test_check11_does_not_block_when_head_moved_mid_run(self, tmp_path):
        """Same defect, other cause: HEAD moved under a long run."""
        repo = self._push_repo(tmp_path / "wt", "import sys\nsys.exit(0)\n")
        self._red_record(repo, self._head(repo), dirty=False, head_moved=True)
        reason = self._deny_reason(self._gate(tmp_path, repo))
        assert reason is None or "local check run" not in reason, (
            f"blocked on a record whose HEAD moved mid-run: {reason}"
        )

    def test_check11_still_blocks_on_a_clean_attributed_record(self, tmp_path):
        """The signal must survive the fix.

        A record that explicitly says the tree was clean and HEAD held still DOES
        describe this commit, and a red one must still deny -- otherwise the
        attribution fields would have bought precision by deleting the gate.
        """
        repo = self._push_repo(tmp_path / "wt", "import sys\nsys.exit(0)\n")
        self._red_record(repo, self._head(repo), dirty=False, head_moved=False)
        reason = self._deny_reason(self._gate(tmp_path, repo))
        assert reason is not None, "an attributed known-red record did not deny"
        assert "local check run" in reason, f"denied for the wrong reason: {reason}"
