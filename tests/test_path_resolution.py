...
class TestIsWithinAllowedScope:
    ...
    def test_home_subdir_allowed(self, tmp_path, monkeypatch):
        """Exercises the HOME branch specifically.

        Under conftest.isolate_home, `$HOME` lives inside the system temp root,
        so `is_within_allowed_scope` would be satisfied by its `tmp` entry and
        this assertion would pass with the `home` entry deleted — vacuous with
        respect to the branch it was written for. Pointing HOME outside the temp
        root restores the distinction.
        """
        outside = pathlib.Path(__file__).resolve().parent.parent / ".pytest-home"
        outside.mkdir(exist_ok=True)
        monkeypatch.setenv("HOME", str(outside))
        assert is_within_allowed_scope(Path(outside) / "some_project") is True
