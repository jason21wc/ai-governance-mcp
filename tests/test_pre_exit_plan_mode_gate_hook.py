...
class TestAuditLog:
    ...
    def test_allow_is_logged(self, tmp_path):
        """The ALLOW path must log, or V-012's rate has no denominator.

        The log recorded only deny and bypass, so a rate over plan-mode
        invocations was not computable from the source V-012 names. Without this
        test, deleting the `_audit_log "allow"` call leaves the suite green — a
        guard nothing verifies, which is the class this whole change is about.
        """
        transcript = create_transcript(
            [make_exit_plan_entry(), make_task_entry("contrarian-reviewer")]
        )
        try:
            run_hook(transcript, env_overrides={"HOME": str(tmp_path)})
        finally:
            os.unlink(transcript)
        log = tmp_path / ".context-engine" / "plan-contrarian-denies.log"
        assert log.exists(), "no audit log written on the allow path"
        assert "allow" in log.read_text(), (
            "allow path did not record an `allow` line — V-012's denominator is "
            "missing again"
        )
...
class TestTimeoutBinaryFallback:
    ...
    def test_warning_when_no_timeout_binary(self, tmp_path):
        # Build a deterministic PATH containing the hook's ordinary dependencies
        # but deliberately not timeout/gtimeout. `/usr/bin:/bin` is insufficient:
        # Linux ships timeout there, so the test skipped on every CI runner and the
        # public skip registry correctly rejected the unregistered skip.
        ...
        for command in required:
            ...
            assert target, f"test requires {command} to be discoverable"
        ...
        try:
            _, _, stderr = run_hook(
                path,
                env_overrides={"PATH": str(bin_dir), "HOME": str(tmp_path)},
            )
