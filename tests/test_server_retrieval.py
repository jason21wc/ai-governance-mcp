...
@pytest.fixture
def oversized_retrieval_result(sample_retrieval_result, scored_principle):
    """Maximum pool cardinality plus long metadata, independent of live retrieval."""
    result = sample_retrieval_result.model_copy(deep=True)
    result.query = "unicode 界 🧭 " * 500
    result.domains_detected = ["domain-" + "界" * 2000] * 50
    result.domain_scores = {"domain-" + "x" * 40000: 0.9}
    result.constitution_principles = []
    result.domain_principles = []
    for n in range(74):
        sp = scored_principle.model_copy(deep=True)
        sp.principle.id = f"test-principle-{n}"
        sp.principle.title = "界🧭 title " * 1000
        sp.principle.content = f"BODY-{n}:" + "content " * 80
        sp.principle.series_code = "S" if n == 73 else "C"
        sp.combined_score = 0.1 if n == 73 else 0.9 - n / 100
        sp.match_reasons = ["reason " * 10000]
        target = result.constitution_principles if n < 24 else result.domain_principles
        target.append(sp)
    ...
    result.references = []
    for n in range(50):
        sm = method.model_copy(deep=True)
        sm.method.id = f"test-method-{n}"
        sm.method.title = "method " * 1000
        result.methods.append(sm)
        sr = reference.model_copy(deep=True)
        sr.reference.id = f"test-reference-{n}"
        sr.reference.title = "reference " * 1000
        sr.reference.summary = "summary " * 1000
        sr.reference.status = "deprecated" if n == 0 else "current"
        result.references.append(sr)
    return result


class TestCompleteQueryResponseBudget:
    @pytest.mark.parametrize("health", ["healthy", "keyword", "missing"])
    @pytest.mark.asyncio
    async def test_dispatch_bounds_complete_text_and_preserves_warnings(
        self, oversized_retrieval_result, reset_server_state, monkeypatch, health
    ):
        from ai_governance_mcp.server import _app
        from ai_governance_mcp.server._constants import (
            GOVERNANCE_REMINDER,
            QUERY_RESPONSE_MAX_CHARS,
        )
        from ai_governance_mcp.server.handlers import retrieval
        from unittest.mock import AsyncMock

        result = oversized_retrieval_result
        result.index_loaded = health != "missing"
        result.semantic_available = health == "healthy"
        original = result.model_dump()
        engine = Mock()
        engine.index.domains = _MOCK_DOMAINS
        ...
        response = await _app.call_tool("query_governance", {"query": result.query})
        text = response[0].text
        assert len(response) == 1
        assert len(text) <= QUERY_RESPONSE_MAX_CHARS
        assert text.endswith(GOVERNANCE_REMINDER)
        assert "Compact response" in text
        assert "S-SERIES TRIGGERED" in text
        if health == "keyword":
            assert "DEGRADED" in text
        elif health == "missing":
            assert "GOVERNANCE INDEX MISSING" in text
        assert "get_principle('<id>')" in text
        assert "Result entries omitted for space:" in text
        assert "deprecated" in text
        assert result.model_dump() == original
        assert (
            text
            == (await _app.call_tool("query_governance", {"query": result.query}))[
                0
            ].text
        )

    def test_compact_metadata_keeps_every_recoverable_id_when_it_fits(
        self, oversized_retrieval_result
    ):
        from ai_governance_mcp.server import _format_retrieval_result

        result = oversized_retrieval_result
        for sp in result.constitution_principles + result.domain_principles:
            sp.principle.title = "short title"
        for sm in result.methods:
            sm.method.title = "short method"
        for sr in result.references:
            sr.reference.title = "short reference"
        text = _format_retrieval_result(result)
        assert "Result entries omitted for space: 0" in text
        for prefix, count in (("principle", 74), ("method", 50), ("reference", 50)):
            for n in range(count):
                assert f"`test-{prefix}-{n}`" in text
        assert "BODY-73:" in text, (
            "low-scoring S-Series body retains allocation priority"
        )
        assert "clipped at 300 chars" in text

    def test_unbounded_ids_are_omitted_whole_with_honest_recovery_guidance(
        ...
        result.methods[0].method.id = "oversized-id-" + "x" * 40000
        text = _format_retrieval_result(result)
        assert len(text + GOVERNANCE_REMINDER) <= QUERY_RESPONSE_MAX_CHARS
        assert "Result entries omitted for space: 1" in text
        assert "narrow the query or specify a domain" in text
        assert "oversized-id-" not in text, "never present a clipped, unresolvable ID"

    def test_compact_empty_body_keeps_extraction_warning(self, sample_retrieval_result):
        from ai_governance_mcp.server import _format_retrieval_result

        result = sample_retrieval_result
        result.constitution_principles[0].principle.content = ""
        result.constitution_principles[0].match_reasons = ["x" * 40000]
        text = _format_retrieval_result(result)
        assert "indexed body is EMPTY" in text
        assert "not withheld, not truncated" in text

    @pytest.mark.asyncio
    async def test_invalid_domain_response_is_also_bounded(self, reset_server_state):
        from ai_governance_mcp.server import _handle_query_governance
        from ai_governance_mcp.server._app import _append_governance_reminder
        from ai_governance_mcp.server._constants import QUERY_RESPONSE_MAX_CHARS

        engine = Mock()
        engine.index.domains = {"valid-" + "x" * 40000: None}
        response = _append_governance_reminder(
            await _handle_query_governance(
                engine, {"query": "test", "domain": "invalid-" + "x" * 40000}
            )
        )
        assert len(response[0].text) <= QUERY_RESPONSE_MAX_CHARS
        assert "Error: Invalid domain" in response[0].text
        assert "clipped" in response[0].text
        engine.retrieve.assert_not_called()

    @pytest.mark.asyncio
    async def test_compact_pointer_recovers_full_source(self, sample_retrieval_result):
        ...
        result = sample_retrieval_result
        unit = result.constitution_principles[0].principle
        unit.content = "source-content " * 4000
        unit.title = "long title " * 4000
        text = _format_retrieval_result(result)
        assert "Compact response" in text
        assert f"get_principle('{unit.id}')" in text
        engine = Mock()
        engine.get_principle_by_id.return_value = unit
        detail = await _handle_get_principle(engine, {"principle_id": unit.id})
        assert json.loads(detail[0].text)["content"] == unit.content

    def test_exact_complete_limit_then_one_character_over(
        self, sample_retrieval_result, monkeypatch
    ):
        from ai_governance_mcp.server.handlers import retrieval
