...
6. **Bounded recovery** (BACKLOG #354) — the JSON mtime records the last generation actually accepted into memory, not merely observed on disk. A refused generation is retried once on the next query; after two failed attempts it is suppressed until `global_index.json` changes again. This repairs transient load failures without turning every query into another load attempt.
...
publishes the exact topic `HEAD` only after `scripts/check.sh --full` passes on
...
- Gated: `ai-governance-proxy` (proxy + server, Layers 1+3) — call-order gate plus, with `--enforce-acts`, the act-intrinsic content deny
...
| **Stale governance index** | A changed on-disk generation is not accepted by the loader's integrity gates | Query returns content from the last known-good generation; reload warning/error identifies the rejected generation | Auto-reload checks JSON mtime on each query, preserves the last known-good index, retries one refused generation once, then waits for the next JSON commit signal. A restart is not required; a new JSON commit signal rearms reload. |
